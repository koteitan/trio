import Pair.Cofinality

/-! **Imported from the sibling project `lean-yapss`** (pair sequences, 2 rows).
The pair theorem is on trio's critical path: the `|R| = 1` base of
`TowerExp2Root` is the pair diagonal `[(k*e, v+k*f, 0)] ∈ W 0`
(GRAFTALL-PLAN 4.2).  Namespace `YAPSS`, so nothing clashes with `TRIO`. -/
