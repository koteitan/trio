...
def NF : Set Three := {t | ∃ M, ST_TS M ∧ translate M = t}
...
(the pair `(T, M)` with `M ∈ ST_TS` and `step M T`). -/
def stepRel (T M : TrioSeq) : Prop := ST_TS M ∧ step M T

/-- Conditional termination: decrease + well-foundedness on the image give
well-foundedness of the one-step relation on `ST_TS`. -/
theorem step_terminates_cond
    (dec : ∀ M n, ST_TS M → 1 < M.length → 1 ≤ n →
