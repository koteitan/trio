...
### ⚠⚠ 2026-08-29: **計器 `inW` は退化していた（課題 L45）**

`inW` は `Wset.lean:171` の `Aop` の**節 3（段の本体、`graft` の節）を落としている**:

```lean
def Aop (Wfam) (u) (X) (M) : Prop :=
  (M.length ≤ 1 ∧ lev M 0 = 0)                                   -- 節 1 … 計器は lev <= a
  ∨ (∀ n : ℕ, 1 ≤ n → M⟦n⟧ ∈ X)                                  -- 節 2 … 計器は NS=(1,2)
  ∨ (∃ m < u, domT M m ∧ ∀ z ∈ Wfam m, based z → graft M z ∈ X)  -- **節 3 が無い**
```

第 1 列は展開で変わらないので、再帰は必ず `S[0]` に着地する。⟹ **判定できたときは
必ず `inW(S,a) = (lev(S[0]) <= a)`**。`tools/inw_audit.py`: 標本 20000、判定 15609、
**一致 15609 / 食い違い 0**。

⟹ **「`X ∈ W u ⟹ Y ∈ W u`」型の命題を `inW` で測ると、
`lev(X[0]) <= u ⟹ lev(Y[0]) <= u` を測っていることになる。
`Y[0] = X[0]` なら恒真で、違反 0 は自動的に出る。**

**「違反 0」を根拠に方針を決める前に、下の「計器」の列を見ること。**

| 計器 | 意味 |
|---|---|
| **⛔ 空虚** | `inW` を使い、かつ結論の第 1 列が仮定の第 1 列と同じ（違反 0 は自動） |
| **⚠ 要確認** | `inW` を使うが第 1 列が変わりうる。非退化かどうか未確認 |
| **✅ 無傷** | `inW` を使わない（`Le1` / `Lift1` / 構文の突合せなど） |
| **✅ 破れ実績** | その計器で実際に違反が出たことがある ⟹ **非退化の証明** |

| 命題 | 例数 | 違反 | **計器** |
|---|---|---|---|
| `(SUBST1g)` 網羅 | 210201 | 0 | ⚠ 要確認 |
| `(SUBST1g)` 非厳格 | 165768 | 0 | ⚠ 要確認 |
| `(SUBST1)` | 62151 | 0 | ⚠ 要確認 |
| 残差のみ・反証型乱択 | 78885 | 0 | ⚠ 要確認 |
| 残差のみ・**狭めた核の領域**（行2 が `R.dropLast` に、別シード・広レンジ） | 13616 | 0 | ⚠ 要確認 |
| `(LOW)` 末尾レベル下げ | 5544711 | 0 | ⛔ **空虚**（末尾を触るだけ ⟹ `S[0]` 不変）|
| 段＝根レベル | 211880 | 0 | ⛔ **空虚**（これはまさに `lev(S[0])` の主張）|
| 接尾辞閉 / 区間閉 | 237099 / 470712 | 0 | ⚠ 要確認（`S[0]` が変わる）|
| `(REPL)` 接尾辞差し替え | 1097675 | 0 | ⛔ **空虚**（接尾辞のみ ⟹ `S[0]` 不変）|
| **`(TOW)` 段なし形**（残核の最もきれいな顔） | **1642293** | 0 | ⛔ **空虚**（結論の頭は `(0,v,z)`、仮定が `2v+z ≤ a`）|
| `(GC)` graft 閉包 | 12217217 | 0 | ⚠ 要確認（`probe_gc.py` は `graft` を実装。ただし所属判定は `inW`）|
| **行 0 の順序型だけで決まるか**（rank/stretch/affine/random） | **3290952 + 134200** | 0 |
| `le1` 錐 ⊆ `amin` 錐（無条件） | 64808 | 0 | ✅ 無傷（`inW` 不使用）|
| `Lift1` と `oper` の可換性・孤児枝 | 210204 | 0 | ✅ 無傷 |
| `Lift1` と `oper` の可換性・親あり枝 | 192996 | **47718 不可換** | ✅ **破れ実績** |
...
| 既知不変量だけでタイが消えるか | 9611 | **1552（消えない）** | ✅ **破れ実績** |
| 残核 `Subst1gReviveSelf` 再監査（小型網羅・別実装） | 1201307 | 0（未判定 273562） | ⚠ 要確認（`Wself` は `lev(M[0])` そのもの ⟹ 空虚の疑いが濃い）|
| **(INS) 1 列挿入**（位置・列とも無制限＝`(SNOC)` の最短形） | **1603817** | 0 | ⚠ 位置 0 のときだけ有効かも。位置 ≥ 1 は ⛔ 空虚 |
| 同・`S[p]` の下に挿入する制限版 | 1268993 | 0 | ⚠ 同上 |
| **(ROW1MONO) 行 1 引き下げ**・多列同時 | **369068** | 0 | ⛔ **空虚**（行 1 を下げると `lev(S[0])` は下がるだけ）|
| 同・単列 / **反証型**（悪い根・その錐・末尾を狙う） | 106763 / **773483** | 0 / 0 | ⛔ **空虚** |
| 同・**塔型ホスト**（行0ずらし×行1リフトのコピー塔、末尾列あり／なし） | **258507** | 0 | ⛔ **空虚** |
| (C) ゲート: `take`/`drop-rebase`/**`oper`** 閉包でタイは出るか | 72561 | 0 |
| ⚠ 行 0 の任意の上げ下げで `W` は閉じるか（＝定理と同値） | 390293 / 337510 | 0 / 0 | ⛔ **空虚** |
| (ROW1MONO) の打ち切り検証（`n≤2` vs `n≤3` 突合） | 44692 | 不一致 0 |
| **サンドイッチ左 `Lift1 (X⟦n⟧) d ≤₁ (Lift1 X d)⟦n⟧`** | **1920294** | **0** | ✅ **無傷**（`inW` 不使用。しかも Lean で証明ずみ）|
| **サンドイッチ右 `(Lift1 X d)⟦n⟧ ≤₁ shiftr01 0 d (X⟦n⟧)`** | **1920294** | **0** | ✅ **無傷**（同上）|
| 内訳: 構造化 based / 敵対的（行2崩壊末尾・タイ・深さ9） / **完全乱択（`based` 無し）** | 368424 / 831870 / **720000** | 0 / 0 / 0 |
| 証明済み ST_TS 不変量 `cnf` はタイを排除するか | — | **⛔ 25 例でタイ** | ✅ **破れ実績** |

⚠ 表に無い `(SNOC)`（14455 例・違反 0、`tools/probe_snoc.py`）は **⛔ 空虚**である
（`C ++ [p]` の第 1 列は `C` の第 1 列と同じ ⟹ 恒真）。`(WCONV-STEP)`（60 万例、
**4.3〜9.4% 破れ**、`probe_wconvex_step.py`）は `inW` を使わないので **✅ 破れ実績**。

**⟹ 再測定が要るもの（優先順）**: `(SNOC)` / `(TOW)` / `Subst1gReviveSelf` / `(ROW1MONO)`。
どれも「たぶん真」の根拠が空虚なので、**いまは真偽どちらの証拠も無い**。
節 3 を入れた `inW`（`graft` 付き）を書くのが先決。`probe_gc.py` に `graft` の実装がある。
...
長さが崩れない理由（**2026-08-10 訂正済み**。当初「`Lift1` も `shiftr01` も分岐
データを全部保存するから」と書いたが、それは**偽**である ── 大域的な分岐データ
一致率は `Lift1` が 540822/600000（90%）、`shiftr01 0 d` が 379947/600000（63%）
にすぎない。行 1 を一部だけ上げれば上げなかった列同士の行 1 大小が変わるので、
`nextrel1_Lift1` 等の補題は「`X` に対する相対的な保存」であって大域一致ではない）:

* **`Lift1 X d` は `X` と展開パスが完全に一致する** ── 末尾列の `srow` と、その行
  における親。**600000/600000 で一致**（測定）。ゆえに `|(Lift1 X d)⟦n⟧| = |X⟦n⟧|`
* 右端は `oper` の**後に** `shiftr01` をかけたもの（`shiftr01 0 d (X⟦n⟧)`）なので
  長さは自明に `|X⟦n⟧|`

⟹ 3 つとも長さ `|X⟦n⟧|`。行 0・行 2 も一致し、行 1 の持ち上げ量だけが違う ──
左は最初のコピーのみ、中央は全コピー、右は全列。

⚠ `shiftr01 0 d` の側は**展開と可換とは限らない**: `path: shift == X` は
540411/600000（90%）で、破れるのは末尾列の `srow` が 0 から 1 に変わる場合
（`oper_shiftr1` の仮説 `1 ≤ srow` が要るのはこのため）。実際
`|(Lift1 X d)⟦n⟧| = |(shiftr01 0 d X)⟦n⟧|` は 4.2% で**偽**。サンドイッチの右端が
`(shiftr01 0 d X)⟦n⟧` ではなく `shiftr01 0 d (X⟦n⟧)` なのは本質的である。
...
### ⛔ `Le1s` / `Le1sd`（分岐データ一致を足す）も反証（2026-08-10）

上の反例が全部「長さ不一致＝分岐データの違い」だったので、`Le1` に
**分岐データ一致**（全 `(i,j)` の `parent` と全 `j` の `srow` が一致）を足した
関係を試した。`tools/probe_le1s.py` / `probe_le1sd.py`:

```
Le1s  A B := Le1 A B ∧ 分岐データ一致
Le1sd A B := Le1s A B ∧ ミラー差分が順序どおり（D1_A ≤ D1_B）
```

* `Le1s` 単独の `oper` 単調性 … 1015980 例中 1977 違反（0.19%、全部 `row1`）
* **`Le1sd` の `oper` 単調性 … 1012280 例 0 違反**（`len` 破れは 1 件も無い）

ここまでは有望に見えた（もし成り立てば凸性すら不要で
`(ROW1MONO-S)` が `A2'` でそのまま閉じる）。しかし決定的な 2 点が**両方とも偽**:

* ⛔ **(WL) の実例が関係に入らない**: `Le1sd (Lift1 X d) (shiftr01 0 d X)` は
  `based` で 20103/177174（11%）、自由で 131760/422826（31%）が `branch` で破れる。
  関係が目標を含まないので使えない
* ⛔ **連鎖しない**: `Le1sd A B ⟹ Le1sd (A⟦n⟧) (B⟦n⟧)` は 759268 例中 1996（0.26%）
  が `delta` で破れる。差分条件が次段で保たれない

⟹ **「行 1 を動かしても木が変わらない」という形の制限では (WL) に届かない。**
⛔ この族の修理も続けないこと。
