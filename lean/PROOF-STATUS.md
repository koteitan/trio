...
### ⛔ `(WCONVEX)` の素直な帰納は反証（2026-08-10、`tools/probe_wconvex_step.py`）

`(WCONVEX)` は**形の上では** `(ROW1MONO)` より弱い（下の witness も使える）が、
**同じ帰納法で楽になるわけではない**ことが測定で確定した。`C` の `W` 導出で
帰納すると、節 2 で要るのは

```
(WCONV-STEP)  A ≤₁ B ≤₁ C ⟹ ∀n ∃n' n''.  A⟦n'⟧ ≤₁ B⟦n⟧ ≤₁ C⟦n''⟧
```

（`A⟦n'⟧ ∈ W a` は `oper_mem_of_mem` で無料）。60 万例 × 2 母集団で測った:

| | 一般の鎖 | サンドイッチ形の鎖 |
|---|---|---|
| (M) 単調形 `n' = n'' = n` | **543589 / 600000（9.4% 失敗）** | **556835 / 600000（7.2% 失敗）** |
| (E) 存在形（`n', n''` を 1..6 で探索） | **564639 / 600000（5.9% 失敗）** | **574348 / 600000（4.3% 失敗）** |

破れ方はほぼ全部 `len`（長さ不一致）で、機構は `(ROW1MONO)` を殺したのと**同一**:
行 1 を動かすと分岐データが変わり、**親が生えたり消えたりする**ので展開後の長さが
ずれる。反例 `A=B=[…,(3,0,0)]` / `C=[…,(3,3,0)]` — 末尾列の行 1 を 0→3 に上げると
親ができ、`C⟦n⟧` だけ短くなる。

⟹ **`(WCONVEX)` は `(ROW1MONO)` より弱い命題だが、`oper` 帰納で攻める限り同じ壁**。
下の witness は「長さが合わない」問題を一切救わない。次の入力は `oper` 帰納の
外から来る必要がある。⛔ この 2 形の修理を続けないこと。
...
### 📐 メタ原則: どの単調性が「レバー」でどれが「定理そのもの」か

現在**無条件に証明済みの自由クラス**は 2 つ:

| 自由クラス | 根拠 |
|---|---|
| 行 2 ≡ 0 | `zeroRow2_mem_Wself`（ペア定理を取り込み済み） |
| 行 0 ≡ 0 | `flat_mem_W`（`nextrel0` が空 ⟹ 全列孤児） |

単調性 `R` が**レバーになる条件**は「自由クラスが `R` で全体を生成しない」こと:

* **行 2 の引き上げ** — 任意の `M` はその非マーク版（行 2 ≡ 0、自由）から得られる
  ⟹ **定理と同値**。⛔ 使えない。
* **行 0 の自由変更** — 任意の `M` は平坦化版（行 0 ≡ 0、自由）から得られる
  ⟹ **定理と同値**（上記）。⛔ 使えない。
* **行 1 の引き下げ `(ROW1MONO)`** — 行 1 を下げても行 0・行 2 は動かないので
  自由クラスから出られない ⟹ **生成しない ⟹ 真のレバー**。✅

新しい単調性を思いついたら、まずこの生成テストにかけること。
