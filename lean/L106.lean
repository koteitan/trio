...
### ⛔ 事実 2: 親が接頭辞にあると、窓の長さが **`n` に比例して伸びます**

親が `c < P.length` にあるとき、悪い部分は **`[c, P.length + j)`** です。
**⟹ ⛔ `P = A ++ mTower Q d e n` で `|mTower| = n·|Q|` なので、**窓の長さ ≥ n·|Q| − |A|**。**
**⟹ ⟹ ⛔⛔ `towerMeas` は `3·|V| + rankDE`（`rankDE ≤ 2`）なので、⟹ **`|V|` が支配**します。**
**⟹ ⟹ ⟹ ⛔⛔⛔ ですから **測度が増えます** ⟹ **`tower_of_measure_step2` の帰納法が回りません**。**
