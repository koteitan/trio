...
/-! ## 2. ★★★★★★ H12 の成果（`lean/H12H2.lean` から**署名を逐語で**写したもの）

⚠ **`H12H2.lean` は H12 の作業ファイルで、いまも編集されている。**
**⟹ `import` せず、ここに写す**（team-lead の規則）。
⚠ **依存閉包を手で追うと漏れるので、いったん**本体をまるごと**写した。**
**⟹ 使わないものは、合成が終わってから削る。**

⚠ **名前衝突は 1 件だけだった**（`h2_cone`。私は §178 で H12 の版を `L105Cap` に入れている）。
**⟹ ここでは H12 の版を落とし、`L105.h2_cone` を使う。** -/
