/-
HbS.lean: F のタイごとに荷の子を持つ遠い語の族の第 3 部: 状態の層を持つ文脈の族 RA0（HbH の写し）。

    RA0 A0 k0 H Lds b X := ∀ g b', b ≤ b' → okWA0 A0 k0 (addF H g) Lds b' (reliftX b' H g A0 (mlift X b (b' - b)))

RA0 は錨の列 A0・上限 k0 の文脈の族（CtxP）。空の中身（RA0_nil）だけは頭 Lds の空の語の規則を仮定に取る。
FarP の場で語の頭 FTL0 ++ 中身の BotGe は、中身が空なら d = 1（BotGe_one）、空でなければ中身の BotGe（BotGe_append_left）。
-/
import HaF
import HbR
...
open HaA HaC HaD HaE HaF HbD HbM HbP HbQ HbR

/-! ## BotGe の補助 -/

theorem BotGe_one {P : TrioSeq} (hP : Fr P) (v : ℕ) : BotGe P 1 v := by
  intro c y hc hy hr
  exfalso
  have hw := window_of_rtg0 hr (by simp) P.length hy (le_refl _)
  rw [Small.entry_append_left hy, show P.length = P.length + 0 from rfl, entry_append_right] at hw
  have := getD_row0_ge hP hy
  have hc1 : entry [c] 0 0 = 1 := by
    obtain ⟨c1, c2, c3⟩ := c
    simp only at hc; subst hc; rfl
  rw [hc1] at hw
  omega

theorem BotGe_append_left {X M : TrioSeq} (hX : Fr X) (hM : Hd M) (hMne : M ≠ []) {d v : ℕ}
    (hbot : BotGe M d v) : BotGe (X ++ M) d v := by
  intro c y hc hy hr
  have hMpos : 0 < M.length := List.length_pos_iff.mpr hMne
  rw [List.append_assoc, List.length_append] at hr
  rw [List.length_append] at hy
  rcases Nat.lt_or_ge y X.length with hyX | hyX
  · exfalso
    have hw := window_of_rtg0 hr (by simp) X.length hyX (by omega)
    rw [Small.entry_append_left hyX, show X.length = X.length + 0 from rfl, entry_append_right,
      Small.entry_append_left hMpos, hM hMne] at hw
    have := getD_row0_ge hX hyX
    omega
  · obtain ⟨w, rfl⟩ : ∃ w, y = X.length + w := ⟨y - X.length, by omega⟩
    have hr1 := rtg0_append_unlift (Nat.le_add_right _ _) hr M.length rfl
    rw [show X.length + w - X.length = w by omega] at hr1
    rw [entry_append_right]
    exact hbot c w hc (by omega) hr1

/-! ## 状態の取り替えと再持ち上げ -/

theorem FarCA0_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a) {b0 : ℕ}
    {ws : List (List TrioSeq × ℕ × TrioSeq)} (hC : FarCA0 A0 k0 H b0 ws) : FarCA0 A0 k0 H' b0 ws := by
  ...
  have hR' : RawWsA0 A0 k0 H b ws := fun w hw => by
    have := hR w hw
    exact ⟨this.1, this.2.1, this.2.2.1, this.2.2.2.1, by rw [reOff_congrH hH]; exact this.2.2.2.2⟩
  have eK := reOff_congrF hH g k0
  have := hC g S o f b hf' hb hR' hSA hA hA1 ho (by rw [eK]; exact hK) (by rw [eK]; exact hKo)
  rwa [relWs0_congr hH] at this

theorem okWA0_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a)
    {Lds : List TrioSeq} {u : ℕ} {X : TrioSeq} (h : okWA0 A0 k0 H Lds u X) : okWA0 A0 k0 H' Lds u X := by
  refine ⟨h.1, by rw [← reOff_congrH hH]; exact h.2.1, fun b0 ws hC => ?_⟩
  exact FarCA0_congr hH (h.2.2 b0 ws (FarCA0_congr (fun a ha => (hH a ha).symm) hC))

theorem FarCA0_relift {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {b0 : ℕ}
    {ws : List (List TrioSeq × ℕ × TrioSeq)} (hC : FarCA0 A0 k0 H b0 ws)
    (hRw : ∀ w ∈ ws, Fr w.2.2 ∧ Hd w.2.2 ∧ LowC (w.2.1 + reOff (fun _ => 0) H A0 k0) w.2.2)
    (g2 : ℕ → ℕ) : FarCA0 A0 k0 (addF H g2) b0 (relWs0 A0 H g2 ws) := by
  intro g S o f b hf hb hR hSA hA hA1 ho hK hKo
  have eH : addF (addF H g2) g = addF H (addF g2 g) := by funext a; simp [addF]; omega
  rw [eH] at hf hK hKo
  have hR0 : RawWsA0 A0 k0 H b ws := fun w hw => by
    have h1 := hR (w.1, w.2.1, reliftX w.2.1 H g2 A0 w.2.2) (List.mem_map.mpr ⟨w, hw, rfl⟩)
    exact ⟨h1.1, h1.2.1, (hRw w hw).1, (hRw w hw).2.1, (hRw w hw).2.2⟩
...
def RA0 (A0 : List ℕ) (k0 : ℕ) (H : ℕ → ℕ) (Lds : List TrioSeq) (b : ℕ) (X : TrioSeq) : Prop :=
  ∀ g b', b ≤ b' → okWA0 A0 k0 (addF H g) Lds b' (reliftX b' H g A0 (mlift X b (b' - b)))

theorem RA0_lift {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {Lds : List TrioSeq} {b : ℕ} {X : TrioSeq}
    (h : RA0 A0 k0 H Lds b X) (g : ℕ → ℕ) : RA0 A0 k0 (addF H g) Lds b (reliftX b H g A0 X) := by
  intro g' b' hb'
  rw [mlift_reliftX, show b + (b' - b) = b' by omega, reliftX_comp, addF_assoc]
  exact h (addF g g') b' hb'

theorem RA0_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a)
    {Lds : List TrioSeq} {b : ℕ} {X : TrioSeq} (h : RA0 A0 k0 H Lds b X) : RA0 A0 k0 H' Lds b X := by
  intro g b' hb'
  have e : reliftX b' H' g A0 (mlift X b (b' - b)) = reliftX b' H g A0 (mlift X b (b' - b)) :=
    reliftX_congr b' (fun a ha => (hH a ha).symm) (fun _ _ => rfl) _
  rw [e]
  exact okWA0_congr (fun a ha => by simp [addF, hH a ha]) (h g b' hb')

theorem RA0_nil {A0 : List ℕ} {k0 : ℕ} {Lds : List TrioSeq}
    (hnil : ∀ (H' : ℕ → ℕ) (b0 c0 : ℕ), GTWA0 A0 k0 H' b0 Lds c0 []) (H : ℕ → ℕ) (b : ℕ) :
...
theorem RA0_okWA {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {Lds : List TrioSeq} {b : ℕ} {X : TrioSeq}
    (h : RA0 A0 k0 H Lds b X) : okWA0 A0 k0 H Lds b X := by
  have := h (fun _ => 0) b le_rfl
  rwa [addF_zero, Nat.sub_self, mlift_zero, reliftX_zero] at this

/-- ★ 状態の層を持つ族の差し込み口の公理。 -/
theorem RA0_ax {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) (H : ℕ → ℕ)
    (Lds : List TrioSeq) : SlotAx (RA0 A0 k0 H Lds) where
  lift := by
  ...
  oper := by
    ...
    refine (okWA0_ax hA01 hk1 (addF H g) Lds).oper b' _ _
  ...
  orph := by
    ...
    have eU2 : reliftX b' H g A0 (mlift (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]) u (b' - u))
      ...
      rw [eU]; rfl
    rw [eU2]
    refine (okWA0_ax hA01 hk1 (addF H g) Lds).orph b' _ _ h j
  ...
  tie := by
    ...
    refine (okWA0_ax hA01 hk1 (addF H g) Lds).tie b' _ _ x
  ...
  flat := by
    ...
    rw [e]
    exact (okWA0_ax hA01 hk1 (addF H g) Lds).flat b' _ (Fr_reliftX (Fr_mlift hW _ _) _ _ _ _)
      (h g b' hb')

/-! ## FarP の場 -/

theorem FarP_RA0 {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) {H : ℕ → ℕ} {Lds : List TrioSeq}
    {s : ℕ} (hs2 : 2 ≤ s) (hs : s ≤ liftOff H A0 k0) :
    FarP (GC A0) (fun H' => RA0 A0 k0 H' Lds) A0 H s := by
  ...
  have hP1ok : okWA0 A0 k0 H1 Lds b1 P1 := by
    have := RA0_okWA (h1 g1 b1 hb1); rwa [← hP1, ← hH1] at this
  ...
  have h2' : ∀ (g2 : ℕ → ℕ) b', b1 ≤ b' → ∀ τ L, 1 ≤ τ → τ < reOff H1 g2 A0 s1 → Fr L →
      ...
      okWA0 A0 k0 (addF H1 g2) Lds b' (reliftX b' H1 g2 A0 (mlift P1 b1 (b' - b1)) ++
    ...
    rw [eH] at hGL ⊢
    have := RA0_okWA (h2 (addF g1 g2) b' (by omega) τ L h1τ hτ' hL hGL)
  ...
  have hw : RawWA0 A0 k0 H1 bb (Lds, b1, P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]) :=
    hR _ (List.mem_append_right _ (List.mem_singleton_self _))
  have hR0 : RawWsA0 A0 k0 H1 bb ws := fun w' h' => hR w' (List.mem_append_left _ h')
  have hb1bb : b1 ≤ bb := hw.1
  have hLds : ∀ Ld ∈ Lds, Fr Ld ∧ LowC bb Ld := hw.2.1
  ...
  rw [← hr]
  have eF : fwH bb r (FTL0 r Lds) b1
        (reliftX b1 H1 g A0 P1 ++ [((d, b1 + reOff H1 g A0 s1, 0) : ℕ × ℕ × ℕ)])
      = fwH bb r (FTL0 r Lds) b1 (reliftX b1 H1 g A0 P1) ++
  ...
  have hQF : Fr (farW0 bb r (relWs0 A0 H1 g ws) ++ fwH bb r (FTL0 r Lds) b1 (reliftX b1 H1 g A0 P1)) :=
    Fr_append (Fr_farW0 _ _ _) (Fr_fwH _ _ _ _ _)
  have hRP : ∀ b', bb ≤ b' → RawWsA0 A0 k0 H1 b' (ws ++ [(Lds, b1, P1)]) := fun b' hb' =>
    RawWsA0_snoc (RawWsA0_mono hb' hR0)
      ⟨show b1 ≤ b' by omega, fun Ld hLd => ⟨(hLds Ld hLd).1, LowC_mono hb' (hLds Ld hLd).2⟩,
        hP1Fr, hP1ok.1, hP1ok.2.1⟩
  have hC1 : FarCA0 A0 k0 H1 b0 (ws ++ [(Lds, b1, P1)]) := hP1ok.2.2 b0 ws hC
  have eP : ∀ b', bb ≤ b' →
      mlift (farW0 bb r (relWs0 A0 H1 g ws) ++ fwH bb r (FTL0 r Lds) b1 (reliftX b1 H1 g A0 P1)) bb (b' - bb)
        = farW0 b' (b' + liftOff f (S ++ A0) o + 1) (relWs0 A0 H1 g (ws ++ [(Lds, b1, P1)])) := by
    intro b' hb'
    have hRk := RawWsk0_relWs0 (hRP bb le_rfl) g
    rw [relWs0_snoc] at hRk
    have := mlift_farW0_basek (show bb < r by omega) (b' - bb) _ hRk
    rw [farW0_snoc] at this
    rw [show bb + (b' - bb) = b' by omega,
      show r + (b' - bb) = b' + liftOff f (S ++ A0) o + 1 by omega] at this
    rw [relWs0_snoc]
    exact this
  have hQbot : BotGe (FTL0 r Lds ++ mlift (reliftX b1 H1 g A0 P1) b1 (bb - b1)) d
      (bb + reOff H1 g A0 s1) := by
    ...
    · have hd1 : d = 1 := by have := hHd (by simp); simpa [entry] using this
      ...
      exact BotGe_one (Fr_FTL0 _ _) _
    · refine BotGe_append_left (Fr_FTL0 _ _) (Hd_mlift (Hd_reliftX hP1ok.1 _ _ _ _) _ _) ?_ hbotP3u
      intro h0
  ...
  · unfold fwH
    exact BotGe_node (Fr_farW0 _ _ _) (by omega) hQbot
  · -- h1
    obtain ⟨hfG, hKG, hKoG⟩ := FarCA_conds_lift hSA hA hf hK hKo g'
    rw [eP b' hb', reliftX_farW0A hA hSA b' hf g' hK _ (hRP b' hb')]
    exact hC1 (addF g g') S o (addF f g') b' hfG (by omega) (hRP b' hb') hSA hA hA1 ho hKG hKoG
  · -- h2
    obtain ⟨hfG, hKG, hKoG⟩ := FarCA_conds_lift hSA hA hf hK hKo g'
    rw [eP b' hb', reliftX_farW0A hA hSA b' hf g' hK _ (hRP b' hb'), relWs0_snoc]
    ...
    have hRw : ∀ w ∈ ws, Fr w.2.2 ∧ Hd w.2.2 ∧ LowC (w.2.1 + reOff (fun _ => 0) H1 A0 k0) w.2.2 :=
      fun w hw => (hR0 w hw).2.2
    have hC3 := FarCA0_relift hC hRw (addF g g')
    ...
    have hR3 := RawWsA0_snoc (RawWsA0_relWs0 (RawWsA0_mono (show bb ≤ b' by omega) hR0) (addF g g'))
      (show RawWA0 A0 k0 (addF H1 (addF g g')) b' (Lds, b', reliftX b' H1 (addF g g') A0
        (mlift P1 b1 (b' - b1)) ++ shiftr01 (d - 1) 0 (((1, b' + τ, 0) : ℕ × ℕ × ℕ) ::
          shiftr01 1 0 L)) from
        ⟨le_rfl, fun Ld hLd => ⟨(hLds Ld hLd).1, LowC_mono (by omega) (hLds Ld hLd).2⟩, hFrY, hY.1,
          hY.2.1⟩)
    ...
    rw [relWs0_zero, farW0_snoc] at this
    dsimp only at this
    have eY : fwH b' (b' + liftOff (addF f g') (S ++ A0) o + 1)
        ...
        = fwH b' (b' + liftOff (addF f g') (S ++ A0) o + 1)
            (FTL0 (b' + liftOff (addF f g') (S ++ A0) o + 1) Lds) b1
    ...
    rw [eY] at this
    rw [farW0_snoc, List.append_assoc, show d + 1 - 1 = d by omega]
    exact this

/-- ★ RA0 は錨の列 A0・上限 k0 の文脈の族。 -/
theorem RA0_ctx {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) (Lds : List TrioSeq) :
    CtxP (GC A0) A0 k0 (fun H => RA0 A0 k0 H Lds) := fun H =>
  ⟨RA0_ax hA01 hk1 H Lds, fun _ _ _ hH h => RA0_congr hH h, fun g _ _ _ h => RA0_lift h g,
    fun _ hs2 hs => FarP_RA0 hA01 hs2 hs⟩

/-- ★ 頭 Lds の語の錨つきの中身に、子の級 GC A0 H τ の並びを持つ節点を置く。 -/
theorem RA0_node {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0)
    (hAk : ∀ a ∈ A0, a < k0) {Lds : List TrioSeq} {H : ℕ → ℕ} {u τ : ℕ} {X L : TrioSeq} (hX : Fr X)
    (h : RA0 A0 k0 H Lds u X) (hL : GC A0 H τ u L) (hτ : τ ≤ liftOff H A0 k0) :
    RA0 A0 k0 H Lds u (X ++ ((1, u + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L) :=
  R_child (R := fun H => RA0 A0 k0 H Lds) (RA0_ctx hA01 hk1 Lds) hAk hX h hL hτ
