...
/-! ## 136. ★★★★★★★★★★ (W85): **`e = 0` ∧「行 1 で最小」⟹ 塔でも孤児**（(W84) の一般化）

「孤児 ⟹ 塔でも孤児」の正確な条件は、**2 つが 1 つに畳めます**:
⟹ ★ **`Q` の中で孤児**（`amin Q j = entry Q 1 j`）
⟹ ★ **非祖先の低い列も無い**（`cross_needs_nonancestor_low`、(W76)）
⟹ ⟹ ★★★★★★★★★★ **合わせると「`j` が `Q` の中で行 1 の最小」**（祖先も非祖先も無いので）。 -/
...
/-! ## 138. ★★★★★★★★★★ (W87): **`d ≤ 段差` ∧ `srow = 2` ⟹ ブロック根は孤児**

`d ≤ 段差`（＝ `∀ r > 0, entry Q 0 0 + d ≤ entry Q 0 r`）なら、
**ブロック根の `nextrel0` の親は「1 つ前のブロックの根」ちょうど**（(W79)(b)）。
⟹ ★ **`nextrel0` の始点は一意** ⟹ ⟹ ★★★★★ **ブロック根の `le0` 祖先は、前のブロック根だけ**。
⟹ ⟹ ★★★★★★★★★★ **どのブロック根も行 2 は `entry Q 2 0`（同じ）**
⟹ ⟹ ⟹ ★ **`nextrel2` は狭義に小さい行 2 を要求** ⟹ ⛔ **候補ゼロ** ⟹ **孤児**。 -/
...
/-! ## 142. ★★★★★★★★★★ (W90c) の道具: **行 2 版の「候補で縛る」**

(W90) の芯（行 1）は「**`le0` 祖先で行 1 が低いものは、`nextrel1` の候補になる**」でした。
⟹ ★ **行 2 版は「`le1` 祖先で行 2 が低いもの」**です（`nextrel2` の最小性が `le1` 祖先の上なので）。 -/
...
/-! ## 144. ★★★★★★★★★★ (W91a) の還元: **「直前ブロックに低い `le0` 祖先がある」に落ちます**

`e = 0` では **ブロック根の行 1 が的と同じ** ⟹ ★ **根は候補になりません**。
⟹ ★★★★★ ですから **「直前ブロックの中に、行 1 が的より低い `le0` 祖先」**が要ります。
⟹ ⟹ ★ それさえあれば、私の `src_ge_of_ancestor_low` が下界を出します。 -/
