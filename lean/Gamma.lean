...
/-- **The generated family's peel IS the next tower level.**  `gpow`'s peel at
`k` is literally `tow v z R (k+1)`, so any statement about the family's peels is
a statement about the tower itself. -/
theorem gpow_dropLast_eq_tow {v z : ℕ} {R : TrioSeq} (hRne : R ≠ []) (k : ℕ) :
    ((0, v, z) : ℕ × ℕ × ℕ)
      :: (gpow R (((0, v, z) : ℕ × ℕ × ℕ) :: R) k).dropLast
      = Wset.tow v z R (k + 1) := by
  have hg := graft_tow_gpow (v := v) (z := z) (N := R) hRne k 0
  rw [Nat.zero_add] at hg
  rw [← hg]
  rfl

/-- **Core α, generated-family form** (v0.120): the peels of the context family
generated by the *lifted composite block* `Rt = ltail v z (graft M Y) t` are
`W`-packages at the lifted root.
...
the row-1 tower is a `W`-package", **not a reduction** — `tower1_mem2_fam` /
`tower1_mem2_gpow` only re-index `k`.  What it does buy is a `GX`-free,
stage-free formulation of α's obligation: no `∀ y ∈ W m` interface, no `GX`
membership of the tower, no `CorePlantCtxLift`.  The content that used to
discharge it (`tow_mem_GX` + `GX_full`) is unchanged and still lives in
`coreGpowPeel_of_plantctx`. -/
def CoreGpowPeel : Prop :=
  ∀ (Y M : TrioSeq), Y.dropLast ∈ GXs → based Y → Y ≠ [] →
    argOK M → 1 ≤ M.length →
    ∀ v z : ℕ, z ≤ 1 → CtxOK M v z →
    ∀ t k a : ℕ, 2 * (v + t) + z ≤ a →
      (((0, v + t, z) : ℕ × ℕ × ℕ)
        :: (gpow (Wset.ltail v z (graft M Y) t)
              (((0, v + t, z) : ℕ × ℕ × ℕ) :: Wset.ltail v z (graft M Y) t)
              k).dropLast) ∈ W a
...
/-! ### v0.120: α の `GX`-free な言い換え（⚠ 還元ではない）

`coreT1L_of_plantctx` は塔を `tow_mem_GX` で回し `GX_full` で潰していたので、
「∀ 装備つき文脈」の `GX` と核 `CorePlantCtxLift` を経由していた。
`tower1_mem2_gpow` を使うと塔は生成族の peel だけで閉じ、`GX` も `GX_full` も
`CorePlantCtxLift` も statement から消える。

⚠ **ただしこれは還元ではない**: `gpow_dropLast_eq_tow` の通り生成族の peel は
塔そのものなので、`CoreGpowPeel` は「塔の全段が `W` package」の言い換えであり、
`coreT1L_of_gpow` は `A1_intro` の節 2 に `oper_cons_tower1` を噛ませただけ。
中身は `coreGpowPeel_of_plantctx`（= 旧 `coreT1L_of_plantctx` の中身）に移る。 -/

open Classical in
/-- **α restated on the generated-family peel core** (v0.120).  The row-1 tower
is closed by `tower1_mem2_gpow`, so the residue is `CoreGpowPeel`: no
`GX`-membership of the tower, no `GX_full`, no `CorePlantCtxLift` in the
statement.  ⚠ Not a reduction — see `CoreGpowPeel`'s honesty note. -/
theorem coreT1L_of_gpow (hg : CoreGpowPeel) : CoreT1L := by
  classical
  intro u Y M hYd hbased hy hMarg hM2 hs1 hdG v z hz1 hctx a t hva hpN
  obtain ⟨m, hd⟩ := hdG
  have hMne : M ≠ [] := List.length_pos_iff.mp (by omega)
  ...
  rw [hNbdef, Wset.lift_cons, ← hRtdef,
    oper_cons_tower1 hRtOK hRtne hdRt hsrRt hpMt]
  refine tower1_mem2_gpow hRtne (by omega) ?_ n
  intro k a' ha'
  rw [Lift1_zero]
  exact hg Y M hYd hbased hy hMarg hM2 v z hz1 hctx t k a' (by omega)

/-- **No regression**: the old equipment core still supplies the new one.  The
generated family's peel is exactly the composite `graft Rt (tow … k)` that
`GX_full` used to discharge (`graft_tow_gpow`), so `CoreGpowPeel` is bounded
above by `CorePlantCtxLift`. -/
theorem coreGpowPeel_of_plantctx (hp : CorePlantCtxLift) : CoreGpowPeel := by
  classical
  intro Y M hYd hbased hy hMarg hM2 v z hz1 hctx t k a hva
  have hMne : M ≠ [] := List.length_pos_iff.mp (by omega)
  have hylen : 0 < Y.length := List.length_pos_iff.mpr hy
  have hR : argOK (graft M Y) := argOK_graft hMne hMarg Y
  have hRne : graft M Y ≠ [] :=
  ...
  have hres := GX_full (hTow k) (Wset.based_tow (v + t) z Rt k) hRtOK hRt2
    hctxRt hz1 (t := 0) (by omega)
  rw [Lift1_zero] at hres
  have heq : graft Rt (Wset.tow (v + t) z Rt k)
      = (gpow Rt (((0, v + t, z) : ℕ × ℕ × ℕ) :: Rt) k).dropLast := by
    have hg := graft_tow_gpow (v := v + t) (z := z) (N := Rt) hRtne k 0
    rw [Nat.zero_add] at hg
    exact hg
  rwa [heq] at hres
