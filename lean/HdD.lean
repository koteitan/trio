/-
HdD.lean: 木の単位の遠い語の中身の、状態の層と文脈の公理（HcD の写し）。

    RAt A0 k0 H b X := ∀ g b', b ≤ b' → okWAt A0 k0 (H+g) b' (reliftX b' H g A0 (mlift X b (b' − b)))

- 子の並びの良さ GoodCht は状態の持ち上げ（GoodCht_relift）と段の持ち上げ（HcC.GoodCht_lift）で閉じる。
- FarP の場は、中身を先に語の段 u' へ持ち上げてから（FarP_RAt）、語の段が中身の段と同じ場合（FarP_RAt_core）に帰着する。
-/
import HdC
...
open HaA HaC HaE HaF HbD HbM HbP HbS HcA HcI HdA HdB HdC

/-! ## 状態の取り替えと再持ち上げ -/

theorem FarCAt_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a) {b0 : ℕ}
    {ws : List (List (List UT) × ℕ × TrioSeq)} (hC : FarCAt A0 k0 H b0 ws) : FarCAt A0 k0 H' b0 ws := by
  ...
  have hR' : RawWsAt A0 k0 H b ws := fun w hw => by
    have := hR w hw
  ...
  rwa [relWst_congr hH] at this

theorem GoodCht_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a) {u : ℕ}
    {Lds : List (List UT)} (h : GoodCht A0 k0 H u Lds) : GoodCht A0 k0 H' u Lds := by
  refine ⟨fun us hus => by rw [← reOff_congrH hH]; exact h.1 us hus,
    fun g b0 ws hC => ?_⟩
  have hHg : ∀ a ∈ A0, addF H g a = addF H' g a := fun a ha => by simp [addF, hH a ha]
  have e : relTs A0 H' g u = relTs A0 H g u :=
    funext (fun us => relTs_congr (fun a ha => (hH a ha).symm) g u us)
  rw [e]
  exact FarCAt_congr hHg (h.2 g b0 ws (FarCAt_congr (fun a ha => (hHg a ha).symm) hC))

theorem okWAt_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a)
    {u : ℕ} {X : TrioSeq} (h : okWAt A0 k0 H u X) : okWAt A0 k0 H' u X := by
  refine ⟨h.1, by rw [← reOff_congrH hH]; exact h.2.1, fun u' hu' Lds hG b0 ws hC => ?_⟩
  exact FarCAt_congr hH (h.2.2 u' hu' Lds (GoodCht_congr (fun a ha => (hH a ha).symm) hG) b0 ws
    (FarCAt_congr (fun a ha => (hH a ha).symm) hC))

theorem FarCAt_relift {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {b0 : ℕ}
    {ws : List (List (List UT) × ℕ × TrioSeq)} (hC : FarCAt A0 k0 H b0 ws)
    (hRw : ∀ w ∈ ws, (∀ us ∈ w.1, RawTs (w.2.1 + reOff (fun _ => 0) H A0 k0) us) ∧
      Fr w.2.2 ∧ Hd w.2.2 ∧ LowC (w.2.1 + reOff (fun _ => 0) H A0 k0) w.2.2)
    (g2 : ℕ → ℕ) : FarCAt A0 k0 (addF H g2) b0 (relWst A0 H g2 ws) := by
  intro g S o f b hf hb hR hSA hA hA1 ho hK hKo
  have eH : addF (addF H g2) g = addF H (addF g2 g) := by funext a; simp [addF]; omega
  rw [eH] at hf hK hKo
  have hR0 : RawWsAt A0 k0 H b ws := fun w hw => by
    have h1 := hR (w.1.map (relTs A0 H g2 w.2.1), w.2.1,
...
theorem GoodCht_relift {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {u : ℕ} {Lds : List (List UT)}
    (h : GoodCht A0 k0 H u Lds) (g : ℕ → ℕ) :
    GoodCht A0 k0 (addF H g) u (Lds.map (relTs A0 H g u)) := by
  refine ⟨RawLdst_relift h.1 g, fun g' b0 => ?_⟩
  have e : (Lds.map (relTs A0 H g u)).map
        (relTs A0 (addF H g) g' u)
      = Lds.map (relTs A0 H (addF g g') u) := by
    simp only [List.map_map, Function.comp_def, relTs_comp]
  rw [e, addF_assoc]
  exact h.2 (addF g g') b0

theorem GoodCht_base {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {u : ℕ} {Lds : List (List UT)}
    (hG : GoodCht A0 k0 H u Lds) (b0 : ℕ) : GTWAt A0 k0 H b0 Lds u [] := by
  have e2 : relTs A0 H (fun _ => 0) u = id := funext (fun us => relTs_zero A0 H u us)
  have := hG.2 (fun _ => 0) b0
  simpa [addF_zero, e2] using this

/-! ## 状態の層を持つ族 -/

def RAt (A0 : List ℕ) (k0 : ℕ) (H : ℕ → ℕ) (b : ℕ) (X : TrioSeq) : Prop :=
  ∀ g b', b ≤ b' → okWAt A0 k0 (addF H g) b' (reliftX b' H g A0 (mlift X b (b' - b)))

theorem RAt_lift {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {b : ℕ} {X : TrioSeq}
    (h : RAt A0 k0 H b X) (g : ℕ → ℕ) : RAt A0 k0 (addF H g) b (reliftX b H g A0 X) := by
  intro g' b' hb'
  rw [mlift_reliftX, show b + (b' - b) = b' by omega, reliftX_comp, addF_assoc]
  exact h (addF g g') b' hb'

theorem RAt_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a)
    {b : ℕ} {X : TrioSeq} (h : RAt A0 k0 H b X) : RAt A0 k0 H' b X := by
  intro g b' hb'
  have e : reliftX b' H' g A0 (mlift X b (b' - b)) = reliftX b' H g A0 (mlift X b (b' - b)) :=
    reliftX_congr b' (fun a ha => (hH a ha).symm) (fun _ _ => rfl) _
  rw [e]
  exact okWAt_congr (fun a ha => by simp [addF, hH a ha]) (h g b' hb')

theorem RAt_nil (A0 : List ℕ) (k0 : ℕ) (H : ℕ → ℕ) (b : ℕ) : RAt A0 k0 H b [] := by
  intro g b' _
  rw [mlift_nil]
  ...
  rw [mlift_nil]
  exact GoodCht_base hG b0

theorem RAt_okWA {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {b : ℕ} {X : TrioSeq}
    (h : RAt A0 k0 H b X) : okWAt A0 k0 H b X := by
  have := h (fun _ => 0) b le_rfl
  rwa [addF_zero, Nat.sub_self, mlift_zero, reliftX_zero] at this

/-- ★ 状態の層を持つ族の差し込み口の公理。 -/
theorem RAt_ax {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) (H : ℕ → ℕ) :
    SlotAx (RAt A0 k0 H) where
  lift := by
  ...
  oper := by
    ...
    refine (okWAt_ax hA01 hk1 (addF H g)).oper b' _ _
  ...
  orph := by
    ...
    have eU2 : reliftX b' H g A0 (mlift (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]) u (b' - u))
      ...
      rw [eU]; rfl
    rw [eU2]
    refine (okWAt_ax hA01 hk1 (addF H g)).orph b' _ _ h j
  ...
  tie := by
    ...
    refine (okWAt_ax hA01 hk1 (addF H g)).tie b' _ _ x
  ...
  flat := by
    ...
    rw [e]
    exact (okWAt_ax hA01 hk1 (addF H g)).flat b' _ (Fr_reliftX (Fr_mlift hW _ _) _ _ _ _)
      (h g b' hb')

/-! ## FarP の場 -/

/-- 語の段が中身の段 b1 と同じ場合。 -/
theorem FarP_RAt_core {A0 : List ℕ} {k0 : ℕ}
    {P1 : TrioSeq} {b1 s1 d : ℕ} {H1 : ℕ → ℕ} (hP1Fr : Fr P1) (hd : 1 ≤ d)
    (hbotP1 : BotGe P1 d (b1 + s1)) (hs12 : 2 ≤ s1) (hsK : s1 ≤ reOff (fun _ => 0) H1 A0 k0)
    (hP1ok : okWAt A0 k0 H1 b1 P1) (hHd : Hd (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]))
    (h2' : ∀ (g2 : ℕ → ℕ) b', b1 ≤ b' → ∀ τ L, 1 ≤ τ → τ < reOff H1 g2 A0 s1 → Fr L →
      GC A0 (addF H1 g2) τ b' L →
      okWAt A0 k0 (addF H1 g2) b' (reliftX b' H1 g2 A0 (mlift P1 b1 (b' - b1)) ++
        shiftr01 (d - 1) 0 (((1, b' + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L)))
    (Lds : List (List UT)) (hG : GoodCht A0 k0 H1 b1 Lds) (b0 : ℕ) :
    GTWAt A0 k0 H1 b0 Lds b1 (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]) := by
  intro ws hC g S o f bb hf hb hR hSA hA hA1 ho hK hKo
  have hw := hR _ (List.mem_append_right _ (List.mem_singleton_self _))
  have hR0 : RawWsAt A0 k0 H1 bb ws := fun w' h' => hR w' (List.mem_append_left _ h')
  have hb1bb : b1 ≤ bb := hw.1
  have hLds : ∀ us ∈ Lds, RawTs (b1 + reOff (fun _ => 0) H1 A0 k0) us := hG.1
  ...
  rw [← hr]
  have eF : fwH bb r (FTLt bb r b1 (Lds.map (relTs A0 H1 g b1))) b1
        (reliftX b1 H1 g A0 P1 ++ [((d, b1 + reOff H1 g A0 s1, 0) : ℕ × ℕ × ℕ)])
      = fwH bb r (FTLt bb r b1 (Lds.map (relTs A0 H1 g b1))) b1 (reliftX b1 H1 g A0 P1) ++
  ...
  have hQF : Fr (farWt bb r (relWst A0 H1 g ws) ++
      fwH bb r (FTLt bb r b1 (Lds.map (relTs A0 H1 g b1))) b1 (reliftX b1 H1 g A0 P1)) :=
    Fr_append (Fr_farWt _ _ _) (Fr_fwH _ _ _ _ _)
  have hRP : ∀ b', bb ≤ b' → RawWsAt A0 k0 H1 b' (ws ++ [(Lds, b1, P1)]) := fun b' hb' =>
    RawWsAt_snoc (RawWsAt_mono hb' hR0) ⟨show b1 ≤ b' by omega, hLds, hP1Fr, hP1ok.1, hP1ok.2.1⟩
  have hC1 : FarCAt A0 k0 H1 b0 (ws ++ [(Lds, b1, P1)]) := by
    have := hP1ok.2.2 b1 le_rfl Lds hG b0 ws hC
    rwa [Nat.sub_self, mlift_zero] at this
  have eP : ∀ b', bb ≤ b' →
      mlift (farWt bb r (relWst A0 H1 g ws) ++
          fwH bb r (FTLt bb r b1 (Lds.map (relTs A0 H1 g b1))) b1 (reliftX b1 H1 g A0 P1)) bb (b' - bb)
        = farWt b' (b' + liftOff f (S ++ A0) o + 1) (relWst A0 H1 g (ws ++ [(Lds, b1, P1)])) := by
    intro b' hb'
    have hRk := RawWskt_relWst (hRP bb le_rfl) g
    rw [relWst_snoc] at hRk
    have := mlift_farWt_basek (show bb < r by omega) (b' - bb) _ hRk
    rw [farWt_snoc] at this
    rw [show bb + (b' - bb) = b' by omega,
      show r + (b' - bb) = b' + liftOff f (S ++ A0) o + 1 by omega] at this
    rw [relWst_snoc]
    exact this
  have hQbot : BotGe (FTLt bb r b1 (Lds.map (relTs A0 H1 g b1)) ++
    ...
    · have hd1 : d = 1 := by have := hHd (by simp); simpa [entry] using this
      ...
      exact BotGe_one (Fr_FTLt _ _ _ _) _
    · refine BotGe_append_left (Fr_FTLt _ _ _ _) (Hd_mlift (Hd_reliftX hP1ok.1 _ _ _ _) _ _) ?_
        hbotP3u
      intro h0
  ...
  · unfold fwH
    exact BotGe_node (Fr_farWt _ _ _) (by omega) hQbot
  · -- h1
    obtain ⟨hfG, hKG, hKoG⟩ := FarCA_conds_lift hSA hA hf hK hKo g'
    rw [eP b' hb', reliftX_farWtA hA hSA b' hf g' hK _ (hRP b' hb')]
    exact hC1 (addF g g') S o (addF f g') b' hfG (by omega) (hRP b' hb') hSA hA hA1 ho hKG hKoG
  · -- h2
    obtain ⟨hfG, hKG, hKoG⟩ := FarCA_conds_lift hSA hA hf hK hKo g'
    rw [eP b' hb', reliftX_farWtA hA hSA b' hf g' hK _ (hRP b' hb'), relWst_snoc]
    ...
    have hRw : ∀ w ∈ ws, (∀ us ∈ w.1, RawTs (w.2.1 + reOff (fun _ => 0) H1 A0 k0) us) ∧
        Fr w.2.2 ∧ Hd w.2.2 ∧ LowC (w.2.1 + reOff (fun _ => 0) H1 A0 k0) w.2.2 :=
      fun w hw => (hR0 w hw).2
    have hC3 := FarCAt_relift hC hRw (addF g g')
    ...
    have hG3 : GoodCht A0 k0 (addF H1 (addF g g')) b'
        ((Lds.map (relTs A0 H1 (addF g g') b1)).map
          (mlTs b1 (b' - b1))) :=
      GoodCht_lift (GoodCht_relift hG (addF g g')) (by omega)
    have hR3 : RawWsAt A0 k0 (addF H1 (addF g g')) b'
        (relWst A0 H1 (addF g g') ws ++
          [((Lds.map (relTs A0 H1 (addF g g') b1)).map
          (mlTs b1 (b' - b1)), b',
      ...
      exact RawWsAt_snoc (RawWsAt_relWst (RawWsAt_mono (show bb ≤ b' by omega) hR0) (addF g g'))
    ...
    rw [relWst_zero, farWt_snoc, Nat.sub_self, mlift_zero] at this
    dsimp only at this
    rw [FTLt_rebase (show b1 ≤ b' by omega) le_rfl] at this
    have eY : fwH b' (b' + liftOff (addF f g') (S ++ A0) o + 1)
          (FTLt b' (b' + liftOff (addF f g') (S ++ A0) o + 1) b1
            (Lds.map (relTs A0 H1 (addF g g') b1))) b'
          (reliftX b' H1 (addF g g') A0 (mlift P1 b1 (b' - b1)) ++
            shiftr01 (d - 1) 0 (((1, b' + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L))
        = fwH b' (b' + liftOff (addF f g') (S ++ A0) o + 1)
            (FTLt b' (b' + liftOff (addF f g') (S ++ A0) o + 1) b1
              (Lds.map (relTs A0 H1 (addF g g') b1))) b1
    ...
    rw [eY] at this
    rw [farWt_snoc, List.append_assoc, show d + 1 - 1 = d by omega]
    exact this

theorem FarP_RAt {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) {H : ℕ → ℕ}
    {s : ℕ} (hs2 : 2 ≤ s) (hs : s ≤ liftOff H A0 k0) :
    FarP (GC A0) (fun H' => RAt A0 k0 H') A0 H s := by
  ...
  have hP1ok : okWAt A0 k0 H1 b1 P1 := by
    have := RAt_okWA (h1 g1 b1 hb1); rwa [← hP1, ← hH1] at this
  ...
  have h2' : ∀ (g2 : ℕ → ℕ) b', b1 ≤ b' → ∀ τ L, 1 ≤ τ → τ < reOff H1 g2 A0 s1 → Fr L →
    ...
    rw [eH] at hGL ⊢
    have := RAt_okWA (h2 (addF g1 g2) b' (by omega) τ L h1τ hτ' hL hGL)
  ...
  rw [eM]
  refine FarP_RAt_core (Fr_mlift hP1Fr _ _) hd ?_ (by omega) hsK
    ((okWAt_ax hA01 hk1 H1).lift b1 P1 hP1Fr hP1ok u' hu') ?_ ?_ Lds hG b0
...
/-- ★ RAt は錨の列 A0・上限 k0 の文脈の族。 -/
theorem RAt_ctx {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) :
    CtxP (GC A0) A0 k0 (fun H => RAt A0 k0 H) := fun H =>
  ⟨RAt_ax hA01 hk1 H, fun _ _ _ hH h => RAt_congr hH h, fun g _ _ _ h => RAt_lift h g,
    fun _ hs2 hs => FarP_RAt hA01 hk1 hs2 hs⟩

/-- ★ 語の錨つきの中身に、子の級 GC A0 H τ の並びを持つ節点を置く。 -/
theorem RAt_node {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0)
    (hAk : ∀ a ∈ A0, a < k0) {H : ℕ → ℕ} {u τ : ℕ} {X L : TrioSeq} (hX : Fr X)
    (h : RAt A0 k0 H u X) (hL : GC A0 H τ u L) (hτ : τ ≤ liftOff H A0 k0) :
    RAt A0 k0 H u (X ++ ((1, u + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L) :=
  R_child (R := fun H => RAt A0 k0 H) (RAt_ctx hA01 hk1) hAk hX h hL hτ
