...
`GX E := {y | … ∀ M, argOK M, E M v z → …}` は `E` に**反単調**、
`F(E) := (W 部分) ∧ (植えブロックが GX E に入る)` は `E` に**単調**。よって
`F` の不動点が存在する（`lfpS` と同じ構成でよい）。装備を強くしたい（`E` を
小さくしたい）のは核の証明側、弱くしたい（`E` を大きくしたい）のは装備供給側
なので、**どちらの不動点を取るかが設計判断**。これが v0.119 の最初の分岐。

3（現状維持: `CorePlantCtxLift` を核のまま）も依然として有効な選択肢。
