...
さらに `N_n` の接頭辞は `N_{n-1}` の接頭辞か `graft N_{n-1} (P.take i)` で、

```
|graft N_{n-1} (P.take i)| = |N_{n-1}| - 1 + i  <  |N_n| = |N_{n-1}| - 1 + |P|
```

なので **複合列長が真に降下する**。γ 枝でも複合列の展開は
`graft (M.take (p+1)) (コピー塊)` で文脈が真に縮む。

⟹ **測度 = 複合列長**（`gx_graft` の再結合 `graft N (graft E w) =
graft (graft N E) w` で不変、`graft_assoc`）。§1.9.39 で複合列長が却下された
