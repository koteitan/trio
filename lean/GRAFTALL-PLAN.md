...
## 4.0005 ★★★★★ `W` は**接尾辞閉**だった（それ自身の根のレベルで）（v0.118.135）

```
Wset.oper_one_eq_dropLast : 1 < |M| → M⟦1⟧ = M.dropLast
W_drop     : M ∈ W u → M.drop j ∈ W (lev M j)
W_segment  : M ∈ W u → (M.drop j).take k ∈ W (lev M j)
drop_mem_Wself : M ∈ W u → M.drop j ∈ Wself
```

旧メモの「接尾辞閉包は偽」は**段を固定していた**せい
（`[(0,0,0),(1,1,0)] ∈ W 0` だが `[(1,1,0)] ∉ W 0`）。`W_root_stage` の後は
「接尾辞自身の根のレベル」が正しい段で、そこでは成立する
（`[(1,1,0)] ∈ W 2`）。証明はデータ帰納:
- `T := N.drop j` の末尾列が **`T` 内に**親を持つ → `oper_append_inner` で
  `N⟦n⟧ = N.take j ++ T⟦n⟧`、`T⟦n⟧` は `N⟦n⟧` の接尾辞 → データ
- そうでなければ `oper` が剥離し `T⟦n⟧` は `N.dropLast = N⟦1⟧` の接尾辞
  （`oper_one_eq_dropLast` を新証明）
- 短い場合は `singleton_mem_self` / `W_nil`

計測 drop 237099 例・区間 470712 例、失敗 0。

**帰結**: `W_take`（接頭辞）と合わせて **`Wself` は任意の連続区間で閉じている**。
つまりブロック `C` も尾部 `D` も「自分の根のレベルで `W` 元」であることが
無料で分かり、**残差は純粋な貼り合わせ命題**になった。

### (REPL) 接尾辞の差し替え — 位置の正規化（計測のみ）

```
(REPL)  A ++ X ∈ W u,  Y ∈ W (lev X 0),  entry Y 0 0 = entry X 0 0,
        Y の全列が深さ >= それ  ==>  A ++ Y ∈ W u
```
計測 1097675 例 0 違反（未判定 33490）。`W_drop` により `X = S.drop p` は
自動で `W (lev S p)` に入るので、(REPL) があれば残差の位置 `p` を **`p = 0`
（根に植わった `Wstar` 形）に正規化できる**。ただし核が 1→2 に増えるので未採用。
⚠ (REPL) 自身をデータ帰納すると同じ復活枝で止まる（節1 が (LOW) 相当）。

## 4.001 ★★★★★ 段の階層は**根のレベルに潰れる** — 核から `u` が消えた（v0.118.132）
