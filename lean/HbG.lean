/-
HbG.lean: F のタイの本数つきの錨つきの中身の遠い語の族の第 2 部: 本数 n を固定した中身の族 okWAn と差し込み口の公理（HaX の写し）。

    GTWAn A0 k0 H b0 n u X := ∀ ws, FarCAn A0 k0 H b0 ws → FarCAn A0 k0 H b0 (ws ++ [(n, u, X)])
    okWAn A0 k0 H n u X    := Hd X ∧ LowC (u + reOff (fun _ => 0) H A0 k0) X ∧ ∀ b0, GTWAn A0 k0 H b0 n u X
-/
import HbE
import HbF
...
open HaA HaC HbD HbE HbF

def GTWAn (A0 : List ℕ) (k0 : ℕ) (H : ℕ → ℕ) (b0 n u : ℕ) (X : TrioSeq) : Prop :=
  ∀ ws, FarCAn A0 k0 H b0 ws → FarCAn A0 k0 H b0 (ws ++ [(n, u, X)])

def okWAn (A0 : List ℕ) (k0 : ℕ) (H : ℕ → ℕ) (n u : ℕ) (X : TrioSeq) : Prop :=
  Hd X ∧ LowC (u + reOff (fun _ => 0) H A0 k0) X ∧ ∀ b0, GTWAn A0 k0 H b0 n u X

theorem GTWAn_nil0 (A0 : List ℕ) (k0 : ℕ) (H : ℕ → ℕ) (b0 c0 : ℕ) : GTWAn A0 k0 H b0 0 c0 [] :=
  fun _ hC => farWAn_collapse hC

theorem GTWAn_rep {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {b0 n u : ℕ} {X : TrioSeq}
    (h : GTWAn A0 k0 H b0 n u X) {ws : List (ℕ × ℕ × TrioSeq)} (hC : FarCAn A0 k0 H b0 ws) :
    ∀ m, FarCAn A0 k0 H b0 (ws ++ List.replicate m (n, u, X))
  | 0 => by simpa using hC
  | m + 1 => by
      have := h _ (GTWAn_rep h hC m)
      rwa [List.append_assoc, ← List.replicate_succ'] at this

/-- ★ F のタイの本数 n の語の中身の族の差し込み口の公理。 -/
theorem okWAn_ax {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) (H : ℕ → ℕ)
    (n : ℕ) : SlotAx (okWAn A0 k0 H n) where
  lift := by
    intro u W hW h u' hu
    ...
    · have := LowC_mliftk h.2.1 (u' - u)
      rwa [show u + reOff (fun _ => 0) H A0 k0 + (u' - u) = u' + reOff (fun _ => 0) H A0 k0 by omega]
        at this
    · have hw : RawWAn A0 k0 H b (n, u', mlift W u (u' - u)) :=
        hR _ (List.mem_append_right _ (List.mem_singleton_self _))
      have hub' : u' ≤ b := hw.1
      have hR0 : RawWsAn A0 k0 H b ws := fun w' h' => hR w' (List.mem_append_left _ h')
      have := h.2.2 b0 ws hC g S o f b hf hb
        (RawWsAn_snoc hR0 ⟨show u ≤ b by omega, hW, h.1, h.2.1⟩) hSA hA hA1 ho hK hKo
      simp only [relWsn_snoc, farWn_snoc] at this ⊢
      have e : reliftX u' H g A0 (mlift W u (u' - u)) = mlift (reliftX u H g A0 W) u (u' - u) := by
        rw [mlift_reliftX, show u + (u' - u) = u' by omega]
      rw [e, fwWn_lift_eq _ _ _ hu hub']
      exact this
  oper := by
    ...
    · have hw : RawWAn A0 k0 H b (n, u, W ++ U) :=
        hR _ (List.mem_append_right _ (List.mem_singleton_self _))
      have hub : u ≤ b := hw.1
      have hR0 : RawWsAn A0 k0 H b ws := fun w' h' => hR w' (List.mem_append_left _ h')
      simp only [relWsn_snoc, farWn_snoc]
      rw [reliftX_app hW hH]
      refine fwWn_oper_step hA hA1 ho f (Fr_farWn _ _ _) (Fr_reliftX hW _ _ _ _)
      ...
      · have hIm := hIH m hm
        have := hIm.2.2 b0 ws hC g S o f b hf hb
          (RawWsAn_snoc hR0 ⟨hub, Fr_append hW (Fr_oper hU m), hIm.1, hIm.2.1⟩) hSA hA hA1 ho hK hKo
        simp only [relWsn_snoc, farWn_snoc] at this
  ...
  orph := by
    ...
    · have hw : RawWAn A0 k0 H b (n, u, W ++ (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)])) :=
        hR _ (List.mem_append_right _ (List.mem_singleton_self _))
      have hR0 : RawWsAn A0 k0 H b ws := fun w' h' => hR w' (List.mem_append_left _ h')
      ...
      refine fwWn_orph_step hA hA1 ho f hub (show b < b + liftOff f (S ++ A0) o + 1 by omega)
        (Fr_farWn _ _ _) (Fr_reliftX hW _ _ _ _)
      ...
      · have hzW : z ∈ Wg (2 * j) := Wg_mono (by omega) hz'
        ...
        have := hzz.2.2 b0 ws hC g S o f b hf hb
          (RawWsAn_snoc hR0 ⟨hub, Fr_append hW hFrz, hzz.1, hzz.2.1⟩) hSA hA hA1 ho hK hKo
        simp only [relWsn_snoc, farWn_snoc] at this
  ...
  tie := by
    ...
    · have hw : RawWAn A0 k0 H b (n, u, W ++ (U ++ [((x, u + 1, 0) : ℕ × ℕ × ℕ)])) :=
        hR _ (List.mem_append_right _ (List.mem_singleton_self _))
      have hR0 : RawWsAn A0 k0 H b ws := fun w' h' => hR w' (List.mem_append_left _ h')
      have hUc : Fr U := fun y hy => hU y (List.mem_append_left _ hy)
      have hHU : Hd U := by
      ...
      simp only [relWsn_snoc, farWn_snoc]
      rw [reliftX_app hW hH, eU]
      refine fwWn_tie_step hA hA1 ho f hub (Fr_farWn _ _ _)
        (fun b'' => farWn b'' (b'' + liftOff f (S ++ A0) o + 1) (relWsn A0 H g ws))
      ...
      · show mlift (farWn b (b + liftOff f (S ++ A0) o + 1) (relWsn A0 H g ws)) b (b'' - b)
          = farWn b'' (b'' + liftOff f (S ++ A0) o + 1) (relWsn A0 H g ws)
        rw [mlift_farWn_basek (show b < b + liftOff f (S ++ A0) o + 1 by omega) (b'' - b) _
            (RawWskn_relWsn hR0 g), show b + (b'' - b) = b'' by omega,
      ...
      · have hFrWU : Fr (W ++ U) := Fr_append hW hUc
        ...
        have := hL.2.2 b0 ws hC g S o f b'' hf (by omega)
          (RawWsAn_snoc (RawWsAn_mono hb'' hR0) ⟨le_rfl, hFrZ, hL.1, hL.2.1⟩) hSA hA hA1 ho hK hKo
        simp only [relWsn_snoc, farWn_snoc] at this
  ...
  flat := by
    ...
    · have hw : RawWAn A0 k0 H b (n, u, W ++ [((1, 0, 0) : ℕ × ℕ × ℕ)]) :=
        hR _ (List.mem_append_right _ (List.mem_singleton_self _))
      have hub : u ≤ b := hw.1
      have hR0 : RawWsAn A0 k0 H b ws := fun w' h' => hR w' (List.mem_append_left _ h')
      ...
      simp only [relWsn_snoc, farWn_snoc]
      rw [e]
      refine fwWn_flat_step hA hA1 ho f (Fr_farWn _ _ _) (Fr_reliftX hW _ _ _ _) (fun m => ?_)
      have hRm : RawWsAn A0 k0 H b (ws ++ List.replicate m (n, u, W)) := by
        intro w' h'
        rcases List.mem_append.mp h' with h' | h'
        · exact hR0 w' h'
        · rw [List.eq_of_mem_replicate h']; exact ⟨hub, hW, h.1, h.2.1⟩
      have := GTWAn_rep (h.2.2 b0) hC m g S o f b hf hb hRm hSA hA hA1 ho hK hKo
      rw [relWsn_rep, farWn_rep] at this
      exact this
