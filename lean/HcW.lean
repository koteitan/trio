/-
HcW.lean: 接頭辞を述語 P（段ごと）に限った字の中身 LC1R（GyH の写し）。

    LC1R P A o H b Y := ∀ W, Fr W → P b W → PVP A o H b W → PVP A o H b (W ++ (1, b + liftOff H A o + 1, 1) :: Y↑1)

- oper / orph は同じ接頭辞のまま（P の条件なし）。
- tie は接頭辞の持ち上げ（P の段の持ち上げで閉じる hPl）。
- flat は字の複写（P が語 Y を足して閉じる hPw）。
-/
import HcV
...
def LC1R (P : ℕ → TrioSeq → Prop) (A : List ℕ) (o : ℕ) (H : ℕ → ℕ) (b : ℕ) (Y : TrioSeq) : Prop :=
  ∀ W, Fr W → P b W → PVP A o H b W →
    PVP A o H b (W ++ ((1, b + liftOff H A o + 1, 1) : ℕ × ℕ × ℕ) :: shiftr01 1 0 Y)

theorem LC1R_oper {A : List ℕ} {o : ℕ} (hA : ∀ a ∈ A, a < o) (hA1 : ∀ a ∈ A, 1 ≤ a) (ho : 1 ≤ o)
    {P : ℕ → TrioSeq → Prop} {H : ℕ → ℕ} {b : ℕ} {Y U : TrioSeq} (hY : Fr Y) (hU : Fr U) (hH : Hd U)
    (hlen : 2 ≤ U.length) (hp : hasParent U (srow U (U.length - 1)) (U.length - 1))
    (hIH : ∀ m, 1 ≤ m → LC1R P A o H b (Y ++ U⟦m⟧)) : LC1R P A o H b (Y ++ U) := by
  intro W hW hPW hPV t
  ...
  have h := hIH m hm W hW hPW hPV t
  rwa [mlift_letterU hW (Fr_append hY (Fr_oper hU m))
    (show b + liftOff H A o < b + liftOff H A o + 1 by omega)] at h

theorem LC1R_flat {A : List ℕ} {o : ℕ} (hA : ∀ a ∈ A, a < o) (hA1 : ∀ a ∈ A, 1 ≤ a) (ho : 1 ≤ o)
    {P : ℕ → TrioSeq → Prop} {H : ℕ → ℕ} {b : ℕ} {Y : TrioSeq} (hY : Fr Y) (h : LC1R P A o H b Y)
    (hPw : ∀ W, Fr W → P b W → PVP A o H b W →
      P b (W ++ ((1, b + liftOff H A o + 1, 1) : ℕ × ℕ × ℕ) :: shiftr01 1 0 Y)) :
    LC1R P A o H b (Y ++ [((1, 0, 0) : ℕ × ℕ × ℕ)]) := by
  intro W hW hPW hPV t
  have hrep : ∀ m, PVP A o H b (W ++ (List.range m).flatMap
      (fun _ => ((1, b + liftOff H A o + 1, 1) : ℕ × ℕ × ℕ) :: shiftr01 1 0 Y)) ∧
      Fr (W ++ (List.range m).flatMap
        (fun _ => ((1, b + liftOff H A o + 1, 1) : ℕ × ℕ × ℕ) :: shiftr01 1 0 Y)) ∧
      P b (W ++ (List.range m).flatMap
        (fun _ => ((1, b + liftOff H A o + 1, 1) : ℕ × ℕ × ℕ) :: shiftr01 1 0 Y)) := by
    intro m
    induction m with
    | zero => exact ⟨by simpa using hPV, by simpa using hW, by simpa using hPW⟩
    | succ m ih =>
        rw [List.range_succ, List.flatMap_append]
        simp only [List.flatMap_cons, List.flatMap_nil, List.append_nil]
        rw [← List.append_assoc]
        exact ⟨h _ ih.2.1 ih.2.2 ih.1, Fr_append ih.2.1 (Fr_letter _ _), hPw _ ih.2.1 ih.2.2 ih.1⟩
...
theorem LC1R_orph {A : List ℕ} {o : ℕ} (hA : ∀ a ∈ A, a < o) (hA1 : ∀ a ∈ A, 1 ≤ a) (ho : 1 ≤ o)
    {P : ℕ → TrioSeq → Prop} {H : ℕ → ℕ} {b : ℕ} {Y U : TrioSeq} {h j : ℕ} (hY : Fr Y)
    (hU : Fr (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)])) (hH : Hd (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]))
    (hj1 : 1 ≤ j) (hj : j ≤ b)
    (hnp : ¬ hasParent (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]) 1 U.length)
    (hz : ∀ z ∈ Wg (2 * j - 1), based z → LC1R P A o H b (Y ++ (U ++ shiftr01 h 0 z))) :
    LC1R P A o H b (Y ++ (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)])) := by
  intro W hW hPW hPV t
  ...
  · have hzW : z ∈ Wg (2 * j) := Wg_mono (by omega) hz'
    ...
    have hFrz : Fr (U ++ shiftr01 h 0 z) := by
      ...
      obtain ⟨p, -, rfl⟩ := hy
      dsimp only; omega
    have hzz := hz z hz' hbz W hW hPW hPV t
...
theorem LC1R_tie {A : List ℕ} {o : ℕ} (hA : ∀ a ∈ A, a < o) (hA1 : ∀ a ∈ A, 1 ≤ a) (ho : 1 ≤ o)
    {P : ℕ → TrioSeq → Prop} {H : ℕ → ℕ} {b : ℕ} {Y U : TrioSeq} {x : ℕ} (hY : Fr Y)
    (hU : Fr (U ++ [((x, b + 1, 0) : ℕ × ℕ × ℕ)])) (hH : Hd (U ++ [((x, b + 1, 0) : ℕ × ℕ × ℕ)]))
    (hc : coneV (U ++ [((x, b + 1, 0) : ℕ × ℕ × ℕ)]) b U.length)
    (hPl : ∀ W, P b W → ∀ b'', b ≤ b'' → P b'' (mlift W b (b'' - b)))
    (hload : ∀ b', b ≤ b' → ∀ Z ∈ Wg (2 * b'), based Z →
      LC1R P A o H b' (mlift (Y ++ U) b (b' - b) ++ shiftr01 x 0 Z)) :
    LC1R P A o H b (Y ++ (U ++ [((x, b + 1, 0) : ℕ × ℕ × ℕ)])) := by
  intro W hW hPW hPV t
  ...
  · have hFrYU : Fr (Y ++ U) := Fr_append hY hUc
    have hFrZ : Fr (mlift (Y ++ U) b (b'' - b) ++ shiftr01 x 0 Z) := by
      ...
      obtain ⟨p, -, rfl⟩ := hy
      dsimp only; omega
    have h1 := hload b'' hb'' Z hZ hbZ (mlift W b (b'' - b)) (Fr_mlift hW _ _) (hPl W hPW b'' hb'')
