/-
HcL.lean: 子を塊と F の位置のタイの並びにした遠い語の中身の、状態の層と文脈の公理（HcD の写し）。

    RAu A0 k0 H b X := ∀ g b', b ≤ b' → okWAu A0 k0 (H+g) b' (reliftX b' H g A0 (mlift X b (b' − b)))

- 子の並びの良さ GoodChu は状態の持ち上げ（GoodChu_relift）と段の持ち上げ（HcC.GoodChu_lift）で閉じる。
- FarP の場は、中身を先に語の段 u' へ持ち上げてから（FarP_RAu）、語の段が中身の段と同じ場合（FarP_RAu_core）に帰着する。
-/
import HcK
...
open HaA HaC HaE HaF HbD HbM HbP HbS HcA HcI HcJ HcK

/-! ## 状態の取り替えと再持ち上げ -/

theorem FarCAu_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a) {b0 : ℕ}
    {ws : List (List (List (Option TrioSeq)) × ℕ × TrioSeq)} (hC : FarCAu A0 k0 H b0 ws) : FarCAu A0 k0 H' b0 ws := by
  ...
  have hR' : RawWsAu A0 k0 H b ws := fun w hw => by
    have := hR w hw
    exact ⟨this.1, fun us hus => by rw [reOff_congrH hH]; exact this.2.1 us hus,
      this.2.2.1, this.2.2.2.1, by rw [reOff_congrH hH]; exact this.2.2.2.2⟩
  have eK := reOff_congrF hH g k0
  have := hC g S o f b hf' hb hR' hSA hA hA1 ho (by rw [eK]; exact hK) (by rw [eK]; exact hKo)
  rwa [relWsu_congr hH] at this

theorem GoodChu_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a) {u : ℕ}
    {Lds : List (List (Option TrioSeq))} (h : GoodChu A0 k0 H u Lds) : GoodChu A0 k0 H' u Lds := by
  refine ⟨fun us hus => by rw [← reOff_congrH hH]; exact h.1 us hus,
    fun g b0 ws hC => ?_⟩
  have hHg : ∀ a ∈ A0, addF H g a = addF H' g a := fun a ha => by simp [addF, hH a ha]
  have e : reliftX u H' g A0 = reliftX u H g A0 :=
    funext (fun X => reliftX_congr u (fun a ha => (hH a ha).symm) (fun _ _ => rfl) X)
  rw [e]
  exact FarCAu_congr hHg (h.2 g b0 ws (FarCAu_congr (fun a ha => (hHg a ha).symm) hC))

theorem okWAu_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a)
    {u : ℕ} {X : TrioSeq} (h : okWAu A0 k0 H u X) : okWAu A0 k0 H' u X := by
  refine ⟨h.1, by rw [← reOff_congrH hH]; exact h.2.1, fun u' hu' Lds hG b0 ws hC => ?_⟩
  exact FarCAu_congr hH (h.2.2 u' hu' Lds (GoodChu_congr (fun a ha => (hH a ha).symm) hG) b0 ws
    (FarCAu_congr (fun a ha => (hH a ha).symm) hC))

theorem FarCAu_relift {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {b0 : ℕ}
    {ws : List (List (List (Option TrioSeq)) × ℕ × TrioSeq)} (hC : FarCAu A0 k0 H b0 ws)
    (hRw : ∀ w ∈ ws, (∀ us ∈ w.1, RawUc (w.2.1 + reOff (fun _ => 0) H A0 k0) us) ∧
      Fr w.2.2 ∧ Hd w.2.2 ∧ LowC (w.2.1 + reOff (fun _ => 0) H A0 k0) w.2.2)
    (g2 : ℕ → ℕ) : FarCAu A0 k0 (addF H g2) b0 (relWsu A0 H g2 ws) := by
  intro g S o f b hf hb hR hSA hA hA1 ho hK hKo
  have eH : addF (addF H g2) g = addF H (addF g2 g) := by funext a; simp [addF]; omega
  rw [eH] at hf hK hKo
  have hR0 : RawWsAu A0 k0 H b ws := fun w hw => by
    have h1 := hR (w.1.map (fun us => us.map (Option.map (reliftX w.2.1 H g2 A0))), w.2.1,
      reliftX w.2.1 H g2 A0 w.2.2)
      (List.mem_map.mpr ⟨w, hw, rfl⟩)
    exact ⟨h1.1, (hRw w hw).1, (hRw w hw).2.1, (hRw w hw).2.2.1, (hRw w hw).2.2.2⟩
  have := hC (addF g2 g) S o f b hf hb hR0 hSA hA hA1 ho hK hKo
  rw [relWsu_comp]
  exact this

theorem GoodChu_relift {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {u : ℕ} {Lds : List (List (Option TrioSeq))}
    (h : GoodChu A0 k0 H u Lds) (g : ℕ → ℕ) :
    GoodChu A0 k0 (addF H g) u (Lds.map (fun us => us.map (Option.map (reliftX u H g A0)))) := by
  refine ⟨RawLdsu_relift h.1 g, fun g' b0 => ?_⟩
  have e : (Lds.map (fun us => us.map (Option.map (reliftX u H g A0)))).map
        (fun us => us.map (Option.map (reliftX u (addF H g) g' A0)))
      = Lds.map (fun us => us.map (Option.map (reliftX u H (addF g g') A0))) := by
    simp [List.map_map, Function.comp_def, Option.map_map, reliftX_comp]
  rw [e, addF_assoc]
  exact h.2 (addF g g') b0

theorem GoodChu_base {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {u : ℕ} {Lds : List (List (Option TrioSeq))}
    (hG : GoodChu A0 k0 H u Lds) (b0 : ℕ) : GTWAu A0 k0 H b0 Lds u [] := by
  have e2 : reliftX u H (fun _ => 0) A0 = id := by funext Z; simp [reliftX_zero]
  have := hG.2 (fun _ => 0) b0
  simpa [addF_zero, e2] using this

/-! ## 状態の層を持つ族 -/

def RAu (A0 : List ℕ) (k0 : ℕ) (H : ℕ → ℕ) (b : ℕ) (X : TrioSeq) : Prop :=
  ∀ g b', b ≤ b' → okWAu A0 k0 (addF H g) b' (reliftX b' H g A0 (mlift X b (b' - b)))

theorem RAu_lift {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {b : ℕ} {X : TrioSeq}
    (h : RAu A0 k0 H b X) (g : ℕ → ℕ) : RAu A0 k0 (addF H g) b (reliftX b H g A0 X) := by
  intro g' b' hb'
  rw [mlift_reliftX, show b + (b' - b) = b' by omega, reliftX_comp, addF_assoc]
  exact h (addF g g') b' hb'

theorem RAu_congr {A0 : List ℕ} {k0 : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A0, H a = H' a)
    {b : ℕ} {X : TrioSeq} (h : RAu A0 k0 H b X) : RAu A0 k0 H' b X := by
  intro g b' hb'
  have e : reliftX b' H' g A0 (mlift X b (b' - b)) = reliftX b' H g A0 (mlift X b (b' - b)) :=
    reliftX_congr b' (fun a ha => (hH a ha).symm) (fun _ _ => rfl) _
  rw [e]
  exact okWAu_congr (fun a ha => by simp [addF, hH a ha]) (h g b' hb')

theorem RAu_nil (A0 : List ℕ) (k0 : ℕ) (H : ℕ → ℕ) (b : ℕ) : RAu A0 k0 H b [] := by
  intro g b' _
  rw [mlift_nil]
  have e : reliftX b' H g A0 [] = [] := by unfold reliftX; exact slift_nil _
  rw [e]
  refine ⟨fun h => absurd rfl h, LowC_nil _, fun u' _ Lds hG b0 => ?_⟩
  rw [mlift_nil]
  exact GoodChu_base hG b0

theorem RAu_okWA {A0 : List ℕ} {k0 : ℕ} {H : ℕ → ℕ} {b : ℕ} {X : TrioSeq}
    (h : RAu A0 k0 H b X) : okWAu A0 k0 H b X := by
  have := h (fun _ => 0) b le_rfl
  rwa [addF_zero, Nat.sub_self, mlift_zero, reliftX_zero] at this

/-- ★ 状態の層を持つ族の差し込み口の公理。 -/
theorem RAu_ax {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) (H : ℕ → ℕ) :
    SlotAx (RAu A0 k0 H) where
  lift := by
  ...
  oper := by
    ...
    refine (okWAu_ax hA01 hk1 (addF H g)).oper b' _ _
  ...
  orph := by
    ...
    have eU2 : reliftX b' H g A0 (mlift (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]) u (b' - u))
      ...
      rw [eU]; rfl
    rw [eU2]
    refine (okWAu_ax hA01 hk1 (addF H g)).orph b' _ _ h j
  ...
  tie := by
    ...
    refine (okWAu_ax hA01 hk1 (addF H g)).tie b' _ _ x
  ...
  flat := by
    ...
    rw [e]
    exact (okWAu_ax hA01 hk1 (addF H g)).flat b' _ (Fr_reliftX (Fr_mlift hW _ _) _ _ _ _)
      (h g b' hb')

/-! ## FarP の場 -/

/-- 語の段が中身の段 b1 と同じ場合。 -/
theorem FarP_RAu_core {A0 : List ℕ} {k0 : ℕ}
    {P1 : TrioSeq} {b1 s1 d : ℕ} {H1 : ℕ → ℕ} (hP1Fr : Fr P1) (hd : 1 ≤ d)
    (hbotP1 : BotGe P1 d (b1 + s1)) (hs12 : 2 ≤ s1) (hsK : s1 ≤ reOff (fun _ => 0) H1 A0 k0)
    (hP1ok : okWAu A0 k0 H1 b1 P1) (hHd : Hd (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]))
    (h2' : ∀ (g2 : ℕ → ℕ) b', b1 ≤ b' → ∀ τ L, 1 ≤ τ → τ < reOff H1 g2 A0 s1 → Fr L →
      GC A0 (addF H1 g2) τ b' L →
      okWAu A0 k0 (addF H1 g2) b' (reliftX b' H1 g2 A0 (mlift P1 b1 (b' - b1)) ++
        shiftr01 (d - 1) 0 (((1, b' + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L)))
    (Lds : List (List (Option TrioSeq))) (hG : GoodChu A0 k0 H1 b1 Lds) (b0 : ℕ) :
    GTWAu A0 k0 H1 b0 Lds b1 (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]) := by
  intro ws hC g S o f bb hf hb hR hSA hA hA1 ho hK hKo
  have hw := hR _ (List.mem_append_right _ (List.mem_singleton_self _))
  have hR0 : RawWsAu A0 k0 H1 bb ws := fun w' h' => hR w' (List.mem_append_left _ h')
  have hb1bb : b1 ≤ bb := hw.1
  have hLds : ∀ us ∈ Lds, RawUc (b1 + reOff (fun _ => 0) H1 A0 k0) us := hG.1
  ...
  rw [← hr]
  have eF : fwH bb r (FTLu bb r b1 (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A0))))) b1
        (reliftX b1 H1 g A0 P1 ++ [((d, b1 + reOff H1 g A0 s1, 0) : ℕ × ℕ × ℕ)])
      = fwH bb r (FTLu bb r b1 (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A0))))) b1 (reliftX b1 H1 g A0 P1) ++
        [((d + 1, bb + reOff H1 g A0 s1, 0) : ℕ × ℕ × ℕ)] := by
    unfold fwH; rw [eL]; simp [shiftr01]
  ...
  have hQF : Fr (farWu bb r (relWsu A0 H1 g ws) ++
      fwH bb r (FTLu bb r b1 (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A0))))) b1 (reliftX b1 H1 g A0 P1)) :=
    Fr_append (Fr_farWu _ _ _) (Fr_fwH _ _ _ _ _)
  have hRP : ∀ b', bb ≤ b' → RawWsAu A0 k0 H1 b' (ws ++ [(Lds, b1, P1)]) := fun b' hb' =>
    RawWsAu_snoc (RawWsAu_mono hb' hR0) ⟨show b1 ≤ b' by omega, hLds, hP1Fr, hP1ok.1, hP1ok.2.1⟩
  have hC1 : FarCAu A0 k0 H1 b0 (ws ++ [(Lds, b1, P1)]) := by
    have := hP1ok.2.2 b1 le_rfl Lds hG b0 ws hC
    rwa [Nat.sub_self, mlift_zero] at this
  have eP : ∀ b', bb ≤ b' →
      mlift (farWu bb r (relWsu A0 H1 g ws) ++
          fwH bb r (FTLu bb r b1 (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A0))))) b1 (reliftX b1 H1 g A0 P1)) bb (b' - bb)
        = farWu b' (b' + liftOff f (S ++ A0) o + 1) (relWsu A0 H1 g (ws ++ [(Lds, b1, P1)])) := by
    intro b' hb'
    have hRk := RawWsku_relWsu (hRP bb le_rfl) g
    rw [relWsu_snoc] at hRk
    have := mlift_farWu_basek (show bb < r by omega) (b' - bb) _ hRk
    rw [farWu_snoc] at this
    rw [show bb + (b' - bb) = b' by omega,
      show r + (b' - bb) = b' + liftOff f (S ++ A0) o + 1 by omega] at this
    rw [relWsu_snoc]
    exact this
  have hQbot : BotGe (FTLu bb r b1 (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A0)))) ++
      mlift (reliftX b1 H1 g A0 P1) b1 (bb - b1)) d (bb + reOff H1 g A0 s1) := by
    rcases P1 with _ | ⟨x, P1t⟩
    · have hd1 : d = 1 := by have := hHd (by simp); simpa [entry] using this
      subst hd1
      have e0 : mlift (reliftX b1 H1 g A0 []) b1 (bb - b1) = [] := by
        unfold reliftX; rw [slift_nil, mlift_nil]
      rw [e0, List.append_nil]
      exact BotGe_one (Fr_FTLu _ _ _ _) _
    · refine BotGe_append_left (Fr_FTLu _ _ _ _) (Hd_mlift (Hd_reliftX hP1ok.1 _ _ _ _) _ _) ?_
        hbotP3u
      intro h0
  ...
  · unfold fwH
    exact BotGe_node (Fr_farWu _ _ _) (by omega) hQbot
  · -- h1
    obtain ⟨hfG, hKG, hKoG⟩ := FarCA_conds_lift hSA hA hf hK hKo g'
    rw [eP b' hb', reliftX_farWuA hA hSA b' hf g' hK _ (hRP b' hb')]
    exact hC1 (addF g g') S o (addF f g') b' hfG (by omega) (hRP b' hb') hSA hA hA1 ho hKG hKoG
  · -- h2
    obtain ⟨hfG, hKG, hKoG⟩ := FarCA_conds_lift hSA hA hf hK hKo g'
    rw [eP b' hb', reliftX_farWuA hA hSA b' hf g' hK _ (hRP b' hb'), relWsu_snoc]
    ...
    have hRw : ∀ w ∈ ws, (∀ us ∈ w.1, RawUc (w.2.1 + reOff (fun _ => 0) H1 A0 k0) us) ∧
        Fr w.2.2 ∧ Hd w.2.2 ∧ LowC (w.2.1 + reOff (fun _ => 0) H1 A0 k0) w.2.2 :=
      fun w hw => (hR0 w hw).2
    have hC3 := FarCAu_relift hC hRw (addF g g')
    ...
    have hG3 : GoodChu A0 k0 (addF H1 (addF g g')) b'
        ((Lds.map (fun us => us.map (Option.map (reliftX b1 H1 (addF g g') A0)))).map
          (fun us => us.map (Option.map (fun X => mlift X b1 (b' - b1))))) :=
      GoodChu_lift (GoodChu_relift hG (addF g g')) (by omega)
    have hR3 : RawWsAu A0 k0 (addF H1 (addF g g')) b'
        (relWsu A0 H1 (addF g g') ws ++
          [((Lds.map (fun us => us.map (Option.map (reliftX b1 H1 (addF g g') A0)))).map
          (fun us => us.map (Option.map (fun X => mlift X b1 (b' - b1)))), b',
            mlift (reliftX b' H1 (addF g g') A0 (mlift P1 b1 (b' - b1)) ++
              shiftr01 (d - 1) 0 (((1, b' + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L)) b' (b' - b'))]) := by
      rw [Nat.sub_self, mlift_zero]
      exact RawWsAu_snoc (RawWsAu_relWsu (RawWsAu_mono (show bb ≤ b' by omega) hR0) (addF g g'))
        ⟨le_rfl, hG3.1, hFrY, hY.1, hY.2.1⟩
    have e0 : ∀ a ∈ A0, addF f g' a = addF (addF H1 (addF g g')) (fun _ => 0) a := fun a ha => by
      rw [addF_zero]; exact hfG a ha
    have := hY.2.2 b' le_rfl _ hG3 b0 _ hC3 (fun _ => 0) S o (addF f g') b' e0 (by omega) hR3 hSA hA
      hA1 ho (by rw [addF_zero]; exact hKG) (by rw [addF_zero]; exact hKoG)
    rw [relWsu_zero, farWu_snoc, Nat.sub_self, mlift_zero] at this
    dsimp only at this
    rw [FTLu_rebase (show b1 ≤ b' by omega) le_rfl] at this
    have eY : fwH b' (b' + liftOff (addF f g') (S ++ A0) o + 1)
          (FTLu b' (b' + liftOff (addF f g') (S ++ A0) o + 1) b1
            (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 (addF g g') A0))))) b'
          (reliftX b' H1 (addF g g') A0 (mlift P1 b1 (b' - b1)) ++
            shiftr01 (d - 1) 0 (((1, b' + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L))
        = fwH b' (b' + liftOff (addF f g') (S ++ A0) o + 1)
            (FTLu b' (b' + liftOff (addF f g') (S ++ A0) o + 1) b1
              (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 (addF g g') A0))))) b1
            (reliftX b1 H1 (addF g g') A0 P1) ++
    ...
    rw [eY] at this
    rw [farWu_snoc, List.append_assoc, show d + 1 - 1 = d by omega]
    exact this

theorem FarP_RAu {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) {H : ℕ → ℕ}
    {s : ℕ} (hs2 : 2 ≤ s) (hs : s ≤ liftOff H A0 k0) :
    FarP (GC A0) (fun H' => RAu A0 k0 H') A0 H s := by
  ...
  have hP1ok : okWAu A0 k0 H1 b1 P1 := by
    have := RAu_okWA (h1 g1 b1 hb1); rwa [← hP1, ← hH1] at this
  ...
  have h2' : ∀ (g2 : ℕ → ℕ) b', b1 ≤ b' → ∀ τ L, 1 ≤ τ → τ < reOff H1 g2 A0 s1 → Fr L →
    ...
    rw [eH] at hGL ⊢
    have := RAu_okWA (h2 (addF g1 g2) b' (by omega) τ L h1τ hτ' hL hGL)
  ...
  have hHd : Hd (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]) := by
    ...
    · exact h0
  refine ⟨hHd, LowC_snoc hP1ok.2.1 _ (by show b1 + s1 ≤ b1 + reOff (fun _ => 0) H1 A0 k0; omega),
    fun u' hu' Lds hG b0 => ?_⟩
  have eM : mlift (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]) b1 (u' - b1)
      = mlift P1 b1 (u' - b1) ++ [((d, u' + s1, 0) : ℕ × ℕ × ℕ)] := by
    rw [mlift_snoc_bottom hbotP1, if_pos (by omega), show b1 + s1 + (u' - b1) = u' + s1 by omega]
  rw [eM]
  refine FarP_RAu_core (Fr_mlift hP1Fr _ _) hd ?_ (by omega) hsK
    ((okWAu_ax hA01 hk1 H1).lift b1 P1 hP1Fr hP1ok u' hu') ?_ ?_ Lds hG b0
  · have := BotGe_slift hbotP1 (stair_step b1 (u' - b1))
    rw [← mlift_eq_slift, if_pos (by omega), show b1 + s1 + (u' - b1) = u' + s1 by omega] at this
    exact this
  · rw [← eM]; exact Hd_mlift hHd _ _
  · intro g2 b' hb' τ L h1τ hτ hL hGL
    have := h2' g2 b' (by omega) τ L h1τ hτ hL hGL
    have e := mlift_mlift P1 b1 (u' - b1) (b' - u')
    rw [show b1 + (u' - b1) = u' by omega, show u' - b1 + (b' - u') = b' - b1 by omega] at e
    rw [e]; exact this

/-- ★ RAu は錨の列 A0・上限 k0 の文脈の族。 -/
theorem RAu_ctx {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0) :
    CtxP (GC A0) A0 k0 (fun H => RAu A0 k0 H) := fun H =>
  ⟨RAu_ax hA01 hk1 H, fun _ _ _ hH h => RAu_congr hH h, fun g _ _ _ h => RAu_lift h g,
    fun _ hs2 hs => FarP_RAu hA01 hk1 hs2 hs⟩

/-- ★ 語の錨つきの中身に、子の級 GC A0 H τ の並びを持つ節点を置く。 -/
theorem RAu_node {A0 : List ℕ} {k0 : ℕ} (hA01 : ∀ a ∈ A0, 1 ≤ a) (hk1 : 1 ≤ k0)
    (hAk : ∀ a ∈ A0, a < k0) {H : ℕ → ℕ} {u τ : ℕ} {X L : TrioSeq} (hX : Fr X)
    (h : RAu A0 k0 H u X) (hL : GC A0 H τ u L) (hτ : τ ≤ liftOff H A0 k0) :
    RAu A0 k0 H u (X ++ ((1, u + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L) :=
  R_child (R := fun H => RAu A0 k0 H) (RAu_ctx hA01 hk1) hAk hX h hL hτ
