/-
HdM.lean: 高さ s の子の並びの最後の低い塊の族 NXs と、その差し込み口の lift / oper / orph。

    NXs s A k H c X := Hd X ∧ LowC ∧ ∀ c' ≥ c, ∀ us, GT s c' us → GT s c' (us ++ [ch (X を c' へ)])
- 最後の語は farWt ws ++ (fwH (FTLt (Lds ++ [plugQ Q []])) u [] ++ (chT us の像 ++ 塊の像)↑(|Q|+2)）（farWt_botT）。
- 接頭辞の道のタイの頭と F のタイは行 1 が r > b（PathCone_fwQ）なので、HdJ の tstep_* が使える。
-/
import HdL
...
open HaA HaC HaE HaF HbD HbM HbP HbS HcA HcE HcF HcI HcM HdA HdB HdC HdD HdE HdF HdG HdH HdI HdJ HdL
...
theorem shift_chTQ_cons (b r u m : ℕ) (us : List UT) (q : List (List UT)) :
    shiftr01 m 0 (chTQ b r u (us :: q))
      = shiftr01 m 0 (chT b r u us) ++ [((m + 1, r, 0) : ℕ × ℕ × ℕ)] ++
...
theorem PathCone_chTQ {b r : ℕ} (hbr : b < r) (bb u K : ℕ) :
    ∀ (Q : List (List UT)) (m : ℕ) (Y : TrioSeq), (∀ us ∈ Q, RawTs K us) → PathCone b m Y →
  ...
  | us :: q, m, Y, hQ, h => by
      ...
      have h1 := PathCone_tieSnoc hbr h (V := shiftr01 m 0 (chT bb r u us))
        (shift_ge (Fr_chT _ (hQ us (by simp))) m)
      have := PathCone_chTQ hbr bb u K q (m + 1) _ (fun us' h' => hQ us' (by simp [h'])) h1
      rwa [show m + 1 + q.length = m + (us :: q).length by simp; omega] at this

theorem fwQ_eq (bb r u : ℕ) (Lds Q : List (List UT)) :
...
theorem PathCone_fwQ {b r : ℕ} (hbr : b < r) (bb u K : ℕ) (Lds Q : List (List UT))
    (hQ : ∀ us ∈ Q, RawTs K us) :
...
theorem botY_facts {b r : ℕ} (hbr : b < r) (u K : ℕ) (Lds Q : List (List UT))
    (hQ : ∀ us ∈ Q, RawTs K us) {V : TrioSeq} (hV : Fr V) :
...
def NXs (s : ℕ) (A : List ℕ) (k : ℕ) (H : ℕ → ℕ) (c : ℕ) (X : TrioSeq) : Prop :=
  Hd X ∧ LowC (c + reOff (fun _ => 0) H A k) X ∧
    ∀ c', c ≤ c' → ∀ us, GT s A k H c' us → GT s A k H c' (us ++ [UT.ch (mlift X c (c' - c))])

theorem NXs_snoc_of {s : ℕ} {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} {c' : ℕ} {us : List UT}
    (hus : GT s A k H c' us) {c : ℕ} (hcc : c ≤ c') {X : TrioSeq}
    (hXF : Fr X) (hXH : Hd X) (hXL : LowC (c + reOff (fun _ => 0) H A k) X)
    (hstep : ∀ (G : ℕ → ℕ) (S : List ℕ) (o : ℕ) (f : ℕ → ℕ), EmbU A k H G S o f → ∀ u, c' ≤ u →
      ∀ Lds, GoodChtX (S ++ A) o f u Lds → ∀ Q, PG s (S ++ A) o f u Q → ∀ b, u ≤ b →
    ...
    GT s A k H c' (us ++ [UT.ch (mlift X c (c' - c))]) := by
...
theorem NXs_snoc_elim {s : ℕ} {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} {c' : ℕ} {us : List UT} {c : ℕ}
    {X : TrioSeq} (h : GT s A k H c' (us ++ [UT.ch (mlift X c (c' - c))])) (hcc : c ≤ c')
    (G : ℕ → ℕ) (S : List ℕ) (o : ℕ) (f : ℕ → ℕ) (hE : EmbU A k H G S o f) (u : ℕ) (hcu : c' ≤ u)
    (Lds : List (List UT)) (hL : GoodChtX (S ++ A) o f u Lds) (Q : List (List UT))
    (hQ : PG s (S ++ A) o f u Q) (b : ℕ) (hub : u ≤ b) (ws : List (List (List UT) × ℕ × TrioSeq))
...
theorem NXs_lift {s : ℕ} {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} (c : ℕ) (W : TrioSeq)
    (h : NXs s A k H c W) (c1 : ℕ) (hc1 : c ≤ c1) : NXs s A k H c1 (mlift W c (c1 - c)) := by
...
theorem NXs_oper {s : ℕ} {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} (c : ℕ) (W U : TrioSeq)
    (hW : Fr W) (hU : Fr U) (hH : Hd U) (hlen : 2 ≤ U.length)
    (hp : hasParent U (srow U (U.length - 1)) (U.length - 1))
    (hIH : ∀ m, 1 ≤ m → NXs s A k H c (W ++ U⟦m⟧)) : NXs s A k H c (W ++ U) := by
  ...
  obtain ⟨hYF, hYH, hYne, -⟩ := botY_facts hbr u _ Lds Q (PG_raw hQ) hV
...
theorem NXs_orph {s : ℕ} {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} (c : ℕ) (W U : TrioSeq) (h j : ℕ)
    (hW : Fr W) (hU : Fr (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)])) (hH : Hd (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]))
    (hj1 : 1 ≤ j) (hj : j ≤ c) (hnp : ¬ hasParent (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]) 1 U.length)
    (hz : ∀ z ∈ Wg (2 * j - 1), based z → NXs s A k H c (W ++ (U ++ shiftr01 h 0 z))) :
    NXs s A k H c (W ++ (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)])) := by
