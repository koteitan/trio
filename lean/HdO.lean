/-
HdO.lean: 高さ s の子の位置の節点の族 NXs の状態の層 RNs と文脈の公理（HdG の写し）。

    RNs s A k H b X := ∀ g b', b ≤ b' → NXs s A k (H+g) b' (reliftX b' H g A (mlift X b (b' − b)))
- FarP の場: 塊の底の節点は、埋め込み先の級で FarP_GpT_ge（行 0 の深さ d + |Q| + 2）。
  底の祖先は道のタイの頭・F のタイ・語の頭で行 1 が r（BotGe_path）。
  h2 の子の節点は、再持ち上げした族 H+G+g' の NXs に、埋め込んで持ち上げた GT s の並びと道を渡す。
-/
import HdN
...
open HaA HaC HaE HaF HbD HbM HbP HbS HcA HcE HcF HcI HcM HdA HdB HdC HdD HdE HdF HdG HdH HdI HdJ HdL HdM HdN

/-! ## 補題 -/

theorem GT_congrH {t : ℕ} {A : List ℕ} {k : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A, H a = H' a) {c : ℕ}
    {x : List UT} (h : GT t A k H c x) : GT t A k H' c x := by
...
theorem NXs_congr {s : ℕ} {A : List ℕ} {k : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A, H a = H' a) {c : ℕ}
    {X : TrioSeq} (h : NXs s A k H c X) : NXs s A k H' c X :=
  ⟨h.1, by rw [← reOff_congrH hH]; exact h.2.1, fun c' hc' us hus =>
    GT_congrH hH (h.2.2 c' hc' us (GT_congrH (fun a ha => (hH a ha).symm) hus))⟩

theorem NXs_nil {s : ℕ} {A : List ℕ} {k : ℕ} (H : ℕ → ℕ) (c : ℕ) : NXs s A k H c [] := by
...
theorem BotGe_chTQ {b r u : ℕ} {V : TrioSeq} {d v : ℕ} (hvr : v ≤ r) (hV : BotGe V d v) :
    ∀ Q : List (List UT), (∀ us ∈ Q, ∃ K, RawTs K us) →
      BotGe (chTQ b r u Q ++ shiftr01 Q.length 0 V) (d + Q.length) v
  | [], _ => by simpa [chTQ, shiftr01] using hV
  | us :: q, hQ => by
      obtain ⟨K, hK⟩ := hQ us (by simp)
      have ih := BotGe_chTQ (b := b) (u := u) hvr hV q (fun us' h' => hQ us' (by simp [h']))
      have e : chTQ b r u (us :: q) ++ shiftr01 (us :: q).length 0 V
          = chT b r u us ++ ((1, r, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (chTQ b r u q ++ shiftr01 q.length 0 V) := by
        simp only [chTQ, List.length_cons, shiftr01_append0, shiftr01_add0, List.append_assoc,
          List.cons_append]
      rw [e, show d + (us :: q).length = d + q.length + 1 by simp; omega]
      exact BotGe_node (Fr_chT _ hK) hvr ih

/-- 道の下の塊の底の節点の祖先は、行 1 が r の道の頭か、塊の中。 -/
theorem BotGe_path {b r u : ℕ} (ws : List (List (List UT) × ℕ × TrioSeq)) (Lds Q : List (List UT))
    (hQ : ∀ us ∈ Q, ∃ K, RawTs K us) {V : TrioSeq} {d v : ℕ} (hvr : v ≤ r) (hV : BotGe V d v) :
...
def RNs (s : ℕ) (A : List ℕ) (k : ℕ) (H : ℕ → ℕ) (b : ℕ) (X : TrioSeq) : Prop :=
  ∀ g b', b ≤ b' → NXs s A k (addF H g) b' (reliftX b' H g A (mlift X b (b' - b)))

theorem RNs_lift {s : ℕ} {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} {b : ℕ} {X : TrioSeq}
    (h : RNs s A k H b X) (g : ℕ → ℕ) : RNs s A k (addF H g) b (reliftX b H g A X) := by
  intro g' b' hb'
  rw [mlift_reliftX, show b + (b' - b) = b' by omega, reliftX_comp, addF_assoc]
  exact h (addF g g') b' hb'

theorem RNs_congr {s : ℕ} {A : List ℕ} {k : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A, H a = H' a)
    {b : ℕ} {X : TrioSeq} (h : RNs s A k H b X) : RNs s A k H' b X := by
  intro g b' hb'
...
theorem RNs_nil {s : ℕ} {A : List ℕ} {k : ℕ} (H : ℕ → ℕ) (b : ℕ) : RNs s A k H b [] := by
  intro g b' _
  rw [mlift_nil]
...
theorem RNs_okW {s : ℕ} {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} {b : ℕ} {X : TrioSeq}
    (h : RNs s A k H b X) : NXs s A k H b X := by
  have := h (fun _ => 0) b le_rfl
  rwa [addF_zero, Nat.sub_self, mlift_zero, reliftX_zero] at this

/-- ★ 状態の層の差し込み口の公理。 -/
theorem RNs_ax {s : ℕ} {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (H : ℕ → ℕ) :
    SlotAx (RNs s A k H) where
  lift := by
  ...
  oper := by
    ...
    refine (NXs_ax (s := s) hA01 hk1 (addF H g)).oper b' _ _
  ...
  orph := by
    ...
    have eU2 : reliftX b' H g A (mlift (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]) u (b' - u))
      ...
      rw [eU]; rfl
    rw [eU2]
    refine (NXs_ax (s := s) hA01 hk1 (addF H g)).orph b' _ _ h j
  ...
  tie := by
    ...
    refine (NXs_ax (s := s) hA01 hk1 (addF H g)).tie b' _ _ x
  ...
  flat := by
    ...
    rw [e]
    exact (NXs_ax (s := s) hA01 hk1 (addF H g)).flat b' _ (Fr_reliftX (Fr_mlift hW _ _) _ _ _ _)
      (h g b' hb')

/-! ## FarP の場 -/

/-- 子の段が塊の段 b1 と同じ場合。 -/
theorem FarP_RNs_core {s : ℕ} {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k)
    (hAk : ∀ a ∈ A, a < k)
    {P1 : TrioSeq} {b1 s1 d : ℕ} {H1 : ℕ → ℕ} (hP1Fr : Fr P1) (hd : 1 ≤ d)
    (hbotP1 : BotGe P1 d (b1 + s1)) (hs12 : 2 ≤ s1) (hsK : s1 ≤ reOff (fun _ => 0) H1 A k)
    (hP1ok : NXs s A k H1 b1 P1) (hHd : Hd (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]))
    (h2' : ∀ (g2 : ℕ → ℕ) b', b1 ≤ b' → ∀ τ L, 1 ≤ τ → τ < reOff H1 g2 A s1 → Fr L →
      GC A (addF H1 g2) τ b' L →
      NXs s A k (addF H1 g2) b' (reliftX b' H1 g2 A (mlift P1 b1 (b' - b1)) ++
        shiftr01 (d - 1) 0 (((1, b' + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L)))
    (us : List UT) (hus : GT s A k H1 b1 us) :
    GT s A k H1 b1 (us ++ [UT.ch (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)])]) := by
  ...
  rw [← e00]
  have hP1T : GT s A k H1 b1 (us ++ [UT.ch P1]) := by
  ...
  have hbotQQ := BotGe_path (b := bb) (r := r) (u := u) ws Lds Q
    (fun us' h' => ⟨_, PG_raw hQ us' h'⟩) (show bb + s' ≤ r by omega) hbotC
  -- 最後の語を木の単位で書く
  ...
  have hW1R : ∀ us' ∈ Lds ++ [plugQ Q (imgT A H1 G b1 u (us ++ [UT.ch P1]))],
    ...
    · exact hL.1 us' hus'
    · simp at hus'; subst hus'; exact RawTs_plugQ Q _ (PG_raw hQ) hTraw
  ...
  have eP : ∀ (g' : ℕ → ℕ) b', bb ≤ b' →
      ...
      = farWt b' (b' + liftOff (addF f g') (S ++ A) o + 1) (relWst (S ++ A) f g' ws ++
          [(Lds.map (relTs (S ++ A) f g' u) ++
            [plugQ (Q.map (relTs (S ++ A) f g' u)) (imgT A H1 (addF G g') b1 u (us ++ [UT.ch P1]))], u, [])]) := by
    intro g' b' hb'
  ...
  · -- h1
    ...
    have := hP1T.2 (addF G g') S o (addF f g') hE' u hcu (Lds.map (relTs (S ++ A) f g' u))
      (GoodChtX_emb hL hEs) (Q.map (relTs (S ++ A) f g' u)) (PG_emb hQ hEs) b' (by omega)
  ...
  · -- h2
    ...
    have hus3 : GT s A k (addF H1 (addF G g')) b' (mlTs b1 (b' - b1) (relTs A H1 (addF G g') b1 us)) :=
      GT_lift (GT_emb hus (EmbU_self hA01 hk1 hAk H1 (addF G g'))) (show b1 ≤ b' by omega)
    ...
    rw [farWt_botT, eC, List.length_map]
...
theorem FarP_RNs {s : ℕ} {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k)
    {H : ℕ → ℕ} {s0 : ℕ} (hs2 : 2 ≤ s0) (hs : s0 ≤ liftOff H A k) :
    FarP (GC A) (fun H' => RNs s A k H') A H s0 := by
  ...
  have hP1ok : NXs s A k H1 b1 P1 := by
  ...
  rw [eM]
  refine FarP_RNs_core hA01 hk1 hAk (Fr_mlift hP1Fr _ _) hd ?_ (by omega) hsK
    ((NXs_ax hA01 hk1 H1).lift b1 P1 hP1Fr hP1ok c' hc') ?_ ?_ us hus
...
theorem RNs_ctx {s : ℕ} {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k) :
    CtxP (GC A) A k (fun H => RNs s A k H) := fun H =>
  ⟨RNs_ax hA01 hk1 H, fun _ _ _ hH h => RNs_congr hH h, fun g _ _ _ h => RNs_lift h g,
    fun _ hs2 hs => FarP_RNs hA01 hk1 hAk hs2 hs⟩

/-- ★ 子の位置の塊に、子の級 GC A H τ の並びを持つ節点を置く。 -/
theorem RNs_node {s : ℕ} {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k)
    {H : ℕ → ℕ} {u τ : ℕ} {X L : TrioSeq} (hX : Fr X)
    (h : RNs s A k H u X) (hL : GC A H τ u L) (hτ : τ ≤ liftOff H A k) :
    RNs s A k H u (X ++ ((1, u + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L) :=
  R_child (R := fun H => RNs s A k H) (RNs_ctx hA01 hk1 hAk) hAk hX h hL hτ
