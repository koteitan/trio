...
theorem FLC_congr {A : List ℕ} {k : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A, H a = H' a) {b0 : ℕ}
  ...
  have := h W hW (by rw [el]; exact hL) (PVE_congr (fun a ha => (hH a ha).symm) hP)
  rw [el] at this
...
theorem FRLC_congr {A : List ℕ} {k : ℕ} {H1 H2 : ℕ → ℕ} (hH : ∀ a ∈ k :: A, H1 a = H2 a) {b : ℕ}
  ...
  have hA : ∀ a ∈ A, H1 a = H2 a := fun a ha => hH a (by simp [ha])
  have el : liftOff H1 A (k + H1 k) = liftOff H2 A (k + H2 k) := by
    rw [hk]; unfold liftOff; rw [stepSum_congr 0 _ hA]
  refine ⟨by rw [← el]; exact h.1, fun g b' hb' => ?_⟩
...
theorem FRLC_lift {A : List ℕ} {k : ℕ} (hAk : ∀ a ∈ A, a < k) {H : ℕ → ℕ} {b : ℕ} {V : TrioSeq}
    (h : FRLC A k H b V) (g : ℕ → ℕ) : FRLC A k (addF H g) b (reliftX b H g (k :: A) V) := by
  refine ⟨?_, fun g' b' hb' => ?_⟩
  · have := LowC_reliftX (K := liftOff H A (k + H k) + 1) (by rw [← Nat.add_assoc]; exact h.1) H g (k :: A)
    rwa [← liftOff_cons_k hAk H, reOff_liftOff, liftOff_cons_k hAk (addF H g), ← Nat.add_assoc] at this
  · rw [mlift_reliftX, show b + (b' - b) = b' by omega, reliftX_comp, addF_assoc]
    exact h.2 (addF g g') b' hb'

theorem FRLC_okF {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} {b : ℕ} {V : TrioSeq} (h : FRLC A k H b V) :
    FLC A (k + H k) H b V := by
  have := h.2 (fun _ => 0) b le_rfl
...
theorem FRLC_far {A : List ℕ} {k : ℕ} (hAk : ∀ a ∈ A, a < k) (H : ℕ → ℕ) (b : ℕ) :
  ...
  refine ⟨?_, fun g b' hb' => ?_⟩
  · have := LowC_snoc (LowC_nil (b + liftOff H A (k + H k) + 1))
      ((1, b + liftOff H A (k + H k) + 1, 1) : ℕ × ℕ × ℕ) le_rfl
    simpa using this
...
theorem FRLC_ax {A : List ℕ} {k : ℕ} (hAk : ∀ a ∈ A, a < k) (hA1 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k)
  ...
  lift := by
    ...
    refine ⟨?_, fun g b' hb' => ?_⟩
    · have := LowC_mliftk (c := u) (k := liftOff H A (k + H k) + 1) (by rw [← Nat.add_assoc]; exact h.1)
        (u1 - u)
      rwa [show u + (liftOff H A (k + H k) + 1) + (u1 - u) = u1 + liftOff H A (k + H k) + 1 by omega]
        at this
    · have e := mlift_mlift W u (u1 - u) (b' - u1)
      rw [show u + (u1 - u) = u1 by omega, show u1 - u + (b' - u1) = b' - u by omega] at e
      rw [e]
      exact h.2 g b' (le_trans hu1 hb')
  oper := by
    intro u W U hW hU hH hlen hp hIH
    have hUne : U ≠ [] := by intro h; rw [h] at hlen; simp at hlen
    refine ⟨?_, fun g b' hb' => ?_⟩
    · have hI1' := (hIH 1 le_rfl).1
  ...
  orph := by
    ...
    refine ⟨?_, fun g b' hb' => ?_⟩
    · have hz0 := (hz [] (Wg_nil _) based_nil).1
      have e0 : W ++ (U ++ shiftr01 h 0 []) = W ++ U := by simp [shiftr01]
      rw [e0] at hz0
      rw [← List.append_assoc]
      exact LowC_snoc hz0 _ (by show j ≤ u + liftOff H A (k + H k) + 1; omega)
  ...
  tie := by
    ...
    refine ⟨?_, fun g b' hb' => ?_⟩
    · have h0 := (hload u le_rfl [] (Wg_nil _) based_nil).1
      have e0 : mlift (W ++ U) u (u - u) ++ shiftr01 x 0 [] = W ++ U := by
        simp [shiftr01, mlift_zero]
      rw [e0] at h0
      rw [← List.append_assoc]
      exact LowC_snoc h0 _ (by show u + 1 ≤ u + liftOff H A (k + H k) + 1; omega)
    · have hA'1 : ∀ a ∈ k :: A, 1 ≤ a := by
      ...
      · rw [eU, reliftX_length, mlift_length]
        ...
        omega
      · have h1 := (hload b'' (le_trans hb' hb'') Z hZ hbZ).2 g b'' le_rfl
  ...
  flat := by
    intro u W hW h
    refine ⟨LowC_snoc h.1 _ (by show 0 ≤ u + liftOff H A (k + H k) + 1; omega), fun g b' hb' => ?_⟩
