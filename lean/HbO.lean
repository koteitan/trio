/-
HbO.lean: F のタイごとに低い子を持つ遠い語の錨つきの族の核（HbE の写し）。

    FarCAL A0 k0 h b0 ws := ∀ g S o f b, 条件（HbE.FarCAn と同じ）→
      GpT (S ++ A0) o f b (farWL b r (relWsL A0 h g ws))      （r = b + liftOff f (S ++ A0) o + 1）
-/
import HbN
...
open HaA HaC HbD HbM HbN

def FarCAL (A0 : List ℕ) (k0 : ℕ) (h : ℕ → ℕ) (b0 : ℕ) (ws : List (List TrioSeq × ℕ × TrioSeq)) :
    Prop :=
  ∀ (g : ℕ → ℕ) (S : List ℕ) (o : ℕ) (f : ℕ → ℕ) (b : ℕ), (∀ a ∈ A0, f a = addF h g a) →
    b0 ≤ b → RawWsAL A0 k0 h b ws → (∀ s ∈ S, ∀ a ∈ A0, a < s) → (∀ a ∈ S ++ A0, a < o) →
    (∀ a ∈ S ++ A0, 1 ≤ a) → 1 ≤ o →
    (∀ s ∈ S, reOff (fun _ => 0) (addF h g) A0 k0 ≤ liftVal f (S ++ A0) s) →
    reOff (fun _ => 0) (addF h g) A0 k0 ≤ liftOff f (S ++ A0) o →
    GpT (S ++ A0) o f b (farWL b (b + liftOff f (S ++ A0) o + 1) (relWsL A0 h g ws))

theorem FarCAL_nil (A0 : List ℕ) (k0 : ℕ) (h : ℕ → ℕ) (b0 : ℕ) : FarCAL A0 k0 h b0 [] := by
  intro g S o f b _ _ _ _ hA hA1 ho _ _
  rw [show farWL b (b + liftOff f (S ++ A0) o + 1) (relWsL A0 h g []) = [] from rfl]
...
theorem farWAL_PVP {A0 : List ℕ} {k0 : ℕ} {h : ℕ → ℕ} {b0 : ℕ}
    {ws : List (List TrioSeq × ℕ × TrioSeq)}
    (hC : FarCAL A0 k0 h b0 ws) (g : ℕ → ℕ) {S : List ℕ} {o : ℕ} {f : ℕ → ℕ}
    (hf : ∀ a ∈ A0, f a = addF h g a) {b : ℕ} (hb : b0 ≤ b) (hR : RawWsAL A0 k0 h b ws)
    ...
    PVP (S ++ A0) o f b (farWL b (b + liftOff f (S ++ A0) o + 1) (relWsL A0 h g ws)) := by
  intro t
  rw [mlift_farWL_highk
...
noncomputable def towWL (b : ℕ) (ws : List (List TrioSeq × ℕ × TrioSeq)) (r : ℕ) : ℕ → TrioSeq
  | 0 => farWL b r ws ++ [((1, r, 1) : ℕ × ℕ × ℕ)]
  | m + 1 => farWL b r ws ++ ((1, r, 1) : ℕ × ℕ × ℕ) ::
      shiftr01 1 0 (((1, r, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (towWL b ws (r + 1) m))

theorem Fr_towWL (b : ℕ) (ws : List (List TrioSeq × ℕ × TrioSeq)) : ∀ m r, Fr (towWL b ws r m)
  | 0, r => by rw [towWL]; exact Fr_append (Fr_farWL _ _ ws) (GzF.Fr_single le_rfl _ _)
  | m + 1, r => by rw [towWL]; exact Fr_append (Fr_farWL _ _ ws) (Fr_letter _ _)

theorem towWL_GpT {A0 : List ℕ} {k0 : ℕ} {h : ℕ → ℕ} {b0 : ℕ}
    {ws : List (List TrioSeq × ℕ × TrioSeq)} (hC : FarCAL A0 k0 h b0 ws) (g : ℕ → ℕ) :
    ∀ (m : ℕ) (S : List ℕ) (o : ℕ) (f : ℕ → ℕ) (b : ℕ), (∀ a ∈ A0, f a = addF h g a) →
    b0 ≤ b → RawWsAL A0 k0 h b ws → (∀ s ∈ S, ∀ a ∈ A0, a < s) → (∀ a ∈ S ++ A0, a < o) →
    (∀ a ∈ S ++ A0, 1 ≤ a) → 1 ≤ o →
    (∀ s ∈ S, reOff (fun _ => 0) (addF h g) A0 k0 ≤ liftVal f (S ++ A0) s) →
    reOff (fun _ => 0) (addF h g) A0 k0 ≤ liftOff f (S ++ A0) o →
    GpT (S ++ A0) o f b (towWL b (relWsL A0 h g ws) (b + liftOff f (S ++ A0) o + 1) m)
  | 0, S, o, f, b, hf, hb, hR, hSA, hA, hA1, ho, hK, hKo => by
      rw [towWL]
      exact PVP_to_GpT (PVP_snocz hA hA1 ho (Fr_farWL _ _ _)
        (farWAL_PVP hC g hf hb hR hSA hA hA1 ho hK hKo))
  | m + 1, S, o, f, b, hf, hb, hR, hSA, hA, hA1, ho, hK, hKo => by
      ...
      have hK' : ∀ s ∈ o :: S, reOff (fun _ => 0) (addF h g) A0 k0 ≤ liftVal H ((o :: S) ++ A0) s := by
        ...
        · rw [liftVal_cons_low hA (List.mem_append_left _ hs)]
          ...
          rw [e]; exact hK s hs
      have hL := towWL_GpT hC g m (o :: S) (o + 1) H b hf' hb hR hSA' hAo' hA1' (by omega) hK'
      ...
      have hGC : GC (o :: (S ++ A0)) H (liftOff f (S ++ A0) o + 1) b
          (towWL b (relWsL A0 h g ws) (b + liftOff f (S ++ A0) o + 1 + 1) m) := by
      ...
      have := hLC _ (Fr_farWL _ _ _) (farWAL_PVP hC g hf hb hR hSA hA hA1 ho hK hKo)
      rw [show b + (liftOff f (S ++ A0) o + 1) = b + liftOff f (S ++ A0) o + 1 by omega] at this
      rw [towWL]
      exact PVP_to_GpT this

theorem mlift_farWLt {k b c : ℕ} (hbc : b + k ≤ c) {ws : List (List TrioSeq × ℕ × TrioSeq)}
    (hR : RawWskL k b ws) (j : ℕ) :
    mlift (farWL b (c + 1) ws ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)]) c j
      = farWL b (c + 1 + j) ws ++ [((1, c + 1 + j, 1) : ℕ × ℕ × ℕ)] := by
  rw [mlift_app (Fr_farWL _ _ ws) (fun _ => rfl), mlift_farWL_highk hbc (by omega) j ws hR,
    mlift_one (show c < c + 1 by omega)]

theorem farWL_flat {k b : ℕ} {ws : List (List TrioSeq × ℕ × TrioSeq)} (hR : RawWskL k b ws) :
    ∀ m c, b + k ≤ c → (List.range (m + 1)).flatMap (fun j =>
      shiftr01 (j * 2) 0 (((0, c + j, 0) : ℕ × ℕ × ℕ) ::
        mlift (farWL b (c + 1) ws ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)]) c j))
      = ((0, c, 0) : ℕ × ℕ × ℕ) :: towWL b ws (c + 1) m
  | 0, c, hc => by simp [mlift_zero, towWL]
  | m + 1, c, hc => by
      rw [GzF.flatMap_range_cons _ (m + 1)]
      have e : ∀ j, shiftr01 ((j + 1) * 2) 0 (((0, c + (j + 1), 0) : ℕ × ℕ × ℕ) ::
          mlift (farWL b (c + 1) ws ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)]) c (j + 1))
          = shiftr01 2 0 (shiftr01 (j * 2) 0 (((0, c + 1 + j, 0) : ℕ × ℕ × ℕ) ::
            mlift (farWL b (c + 1 + 1) ws ++ [((1, c + 1 + 1, 1) : ℕ × ℕ × ℕ)]) (c + 1) j)) := by
        intro j
        rw [mlift_farWLt hc hR, mlift_farWLt (c := c + 1) (by omega) hR, GzF.shift_shift,
      ...
      rw [← GzF.shift_flatMap 2 (fun j => shiftr01 (j * 2) 0 (((0, c + 1 + j, 0) : ℕ × ℕ × ℕ) ::
            mlift (farWL b (c + 1 + 1) ws ++ [((1, c + 1 + 1, 1) : ℕ × ℕ × ℕ)]) (c + 1) j)),
        farWL_flat hR m (c + 1) (by omega)]
      have eT : shiftr01 1 0 (((1, c + 1, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (towWL b ws (c + 1 + 1) m))
          = shiftr01 2 0 (((0, c + 1, 0) : ℕ × ℕ × ℕ) :: towWL b ws (c + 1 + 1) m) := by
        rw [show shiftr01 2 0 (((0, c + 1, 0) : ℕ × ℕ × ℕ) :: towWL b ws (c + 1 + 1) m)
            = shiftr01 (1 + 1) 0 (((0, c + 1, 0) : ℕ × ℕ × ℕ) :: towWL b ws (c + 1 + 1) m) from rfl,
          ← GzF.shift_shift]
        rfl
      rw [show towWL b ws (c + 1) (m + 1) = farWL b (c + 1) ws ++
          ((1, c + 1, 1) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (((1, c + 1, 0) : ℕ × ℕ × ℕ) ::
            shiftr01 1 0 (towWL b ws (c + 1 + 1) m)) from rfl, eT]
      simp [mlift_zero]

theorem farWL_P (b c : ℕ) (ws : List (List TrioSeq × ℕ × TrioSeq)) :
    Fr (farWL b (c + 1) ws ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)]) ∧
    coneV ((farWL b (c + 1) ws ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)]) ++ [((2, c + 1, 1) : ℕ × ℕ × ℕ)]) c
      (farWL b (c + 1) ws ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)]).length := by
  refine ⟨Fr_append (Fr_farWL _ _ ws) (GzF.Fr_single le_rfl _ _), ?_⟩
  have hbot : BotGe (farWL b (c + 1) ws ++ ((1, c + 1, 1) : ℕ × ℕ × ℕ) :: shiftr01 1 0 [])
      (0 + 1 + 1) (c + 1) :=
    BotGe_node (Fr_farWL _ _ ws) le_rfl (BotGe_top Fr_nil _)
  have := coneV_of_BotGe (z := 1) hbot (show c < c + 1 by omega)
  simpa [shiftr01] using this

/-- ★ 遠い語の並びの潰れ（中身も F のタイもない遠い語を足す）。 -/
theorem farWAL_collapse {A0 : List ℕ} {k0 : ℕ} {h : ℕ → ℕ} {b0 c0 : ℕ}
    {ws : List (List TrioSeq × ℕ × TrioSeq)} (hC : FarCAL A0 k0 h b0 ws) :
    FarCAL A0 k0 h b0 (ws ++ [([], c0, [])]) := by
  intro g S o f b hf hb hR hSA hA hA1 ho hK hKo
  have hR0 : RawWsAL A0 k0 h b ws := fun w hw => hR w (List.mem_append_left _ hw)
  refine GpT_intro hA (fun R hR' g' => ?_)
  rw [reliftX_farWLA hA hSA b hf g' hK _ hR]
  ...
  intro u' hu X hX hRX
  have ers : relWsL A0 h (addF g g') (ws ++ [([], c0, [])])
      = relWsL A0 h (addF g g') ws ++ [([], c0, [])] := by
    simp [relWsL, reliftX, slift_nil]
  have hRk : RawWskL (reOff (fun _ => 0) (addF h (addF g g')) A0 k0) b
      (relWsL A0 h (addF g g') ws ++ [([], c0, [])]) := by
    have := RawWskL_relWsL hR (addF g g'); rwa [ers] at this
  rw [ers]
  obtain ⟨ws', hws'⟩ : ∃ ws', ws' = relWsL A0 h (addF g g') ws := ⟨_, rfl⟩
  rw [← hws'] at hRk ⊢
  rw [mlift_farWL_basek (show b < b + kk + 1 by omega) (u' - b) _ hRk,
    show b + kk + 1 + (u' - b) = u' + kk + 1 by omega, show b + (u' - b) = u' by omega, farWL_snoc]
  have hRu : RawWskL (reOff (fun _ => 0) (addF h (addF g g')) A0 k0) u' ws' := by
    rw [hws']; exact RawWskL_mono hu (RawWskL_relWsL hR0 (addF g g'))
  obtain ⟨c, hc⟩ : ∃ c, c = u' + kk := ⟨_, rfl⟩
  rw [← hc]
  have hbc : u' + reOff (fun _ => 0) (addF h (addF g g')) A0 k0 ≤ c := by rw [hc]; omega
  obtain ⟨hP, hcone⟩ := farWL_P u' c ws'
  obtain ⟨U0, hU0⟩ : ∃ U0 : TrioSeq, U0 = ((0, c, 0) : ℕ × ℕ × ℕ) ::
      ((farWL u' (c + 1) ws' ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)]) ++ [((2, c + 1, 1) : ℕ × ℕ × ℕ)]) :=
    ⟨_, rfl⟩
  have eU : ((1, c, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (farWL u' (c + 1) ws' ++
      fwH u' (c + 1) (FTL u' (c + 1) (([] : List TrioSeq), c0, ([] : TrioSeq)).2.1
        (([] : List TrioSeq), c0, ([] : TrioSeq)).1)
        (([] : List TrioSeq), c0, ([] : TrioSeq)).2.1 (([] : List TrioSeq), c0, ([] : TrioSeq)).2.2)
      = shiftr01 1 0 U0 := by
    rw [hU0]; simp [shiftr01, fwH, FTL, mlift_nil]
  rw [eU]
  ...
  have hpV : hasParent U0 (srow U0 (U0.length - 1)) (U0.length - 1) := by
    ...
    · exfalso
      have hl : U0.length - 1 = (((0, c, 0) : ℕ × ℕ × ℕ) ::
          (farWL u' (c + 1) ws' ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)])).length + 0 := by
        rw [hU0]; simp only [List.length_cons, List.length_append, List.length_singleton, List.length_nil]; omega
      rw [hl] at h'
      unfold lev at h'
      rw [hU0, show ((0, c, 0) : ℕ × ℕ × ℕ) :: ((farWL u' (c + 1) ws' ++
          [((1, c + 1, 1) : ℕ × ℕ × ℕ)]) ++ [((2, c + 1, 1) : ℕ × ℕ × ℕ)])
        = (((0, c, 0) : ℕ × ℕ × ℕ) :: (farWL u' (c + 1) ws' ++ [((1, c + 1, 1) : ℕ × ℕ × ℕ)])) ++
  ...
  rw [eO, hU0, oper_zcone hP (show 1 ≤ 2 by omega) hcone (m' + 1), farWL_flat hRu m' c hbc]
  have eS : shiftr01 1 0 (((0, c, 0) : ℕ × ℕ × ℕ) :: towWL u' ws' (c + 1) m')
      = ((1, c, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (towWL u' ws' (c + 1) m') := by
    simp [shiftr01]
  rw [eS]
  have hD := towWL_GpT hC (addF g g') m' S o F u' hfF (by omega) (RawWsAL_mono hu hR0) hSA hA hA1
    ho hKF hKoF
  rw [← hkk, ← hc, ← hws'] at hD
