/-
HcO.lean: 子の位置の節点の族 NX の状態の層 RN と文脈の公理（HcL の写し）。

    RN A k H b X := ∀ g b', b ≤ b' → NX A k (H+g) b' (reliftX b' H g A (mlift X b (b' − b)))

- FarP の場: 塊の底の節点は、埋め込み先の級で FarP_GpT_ge（行 0 の深さ d+2）。h2 の子の節点は、
  再持ち上げした族 H+g+g' の NX に、低い子の並び（族によらない）と GoodChuX の埋め込みを渡す。
- RN_node: 子の級 GC A H τ の並びを持つ節点を置く（GyF.R_child）。
-/
import HcN
...
open HaA HaC HaE HaF HbD HbM HbP HbS HcA HcE HcF HcI HcJ HcK HcL HcM HcN
...
theorem mapu_zero (c : ℕ) (H : ℕ → ℕ) (A : List ℕ) (Lds : List (List (Option TrioSeq))) :
    Lds.map (fun us => us.map (Option.map (reliftX c H (fun _ => 0) A))) = Lds := by
  have e : reliftX c H (fun _ => 0) A = id := funext (fun X => reliftX_zero c H A X)
  simp [e]
...
theorem GoodChuX_congr {A : List ℕ} {k : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A, H a = H' a) {u : ℕ}
    {Lds : List (List (Option TrioSeq))} (h : GoodChuX A k H u Lds) : GoodChuX A k H' u Lds := by
  refine ⟨fun us hus => by rw [← reOff_congrH hH]; exact h.1 us hus,
    fun g S o f b hE hub ws hC hR => ?_⟩
  have e : reliftX u H' g A = reliftX u H g A :=
...
theorem NX_congr {A : List ℕ} {k : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A, H a = H' a) {c : ℕ}
    {X : TrioSeq} (h : NX A k H c X) : NX A k H' c X :=
  ⟨h.1, by rw [← reOff_congrH hH]; exact h.2.1, fun c' hc' us hus Lds hL =>
    GoodChuX_congr hH (h.2.2 c' hc' us hus Lds (GoodChuX_congr (fun a ha => (hH a ha).symm) hL))⟩

theorem NX_nil {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k)
    (H : ℕ → ℕ) (c : ℕ) : NX A k H c [] := by
  refine ⟨fun h => absurd rfl h, LowC_nil _, fun c' hc' us hus Lds hL => ?_⟩
  refine NX_snoc_of hA01 hk1 hL hus hc' Fr_nil (fun h => absurd rfl h) (LowC_nil _)
    (fun g S o f b hE hub ws hC hR => ?_)
  have := (GoodChuX_snoc_low hL hus hA01 hk1 hAk).2 g S o f b hE hub ws hC hR
  rw [farWu_snoc, List.map_append, List.map_singleton, mapu_tie_inv hA01 hus.1 H g, FTLu_snoc] at this
  dsimp only at this
...
def RN (A : List ℕ) (k : ℕ) (H : ℕ → ℕ) (b : ℕ) (X : TrioSeq) : Prop :=
  ∀ g b', b ≤ b' → NX A k (addF H g) b' (reliftX b' H g A (mlift X b (b' - b)))

theorem RN_lift {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} {b : ℕ} {X : TrioSeq}
    (h : RN A k H b X) (g : ℕ → ℕ) : RN A k (addF H g) b (reliftX b H g A X) := by
  intro g' b' hb'
  rw [mlift_reliftX, show b + (b' - b) = b' by omega, reliftX_comp, addF_assoc]
  exact h (addF g g') b' hb'

theorem RN_congr {A : List ℕ} {k : ℕ} {H H' : ℕ → ℕ} (hH : ∀ a ∈ A, H a = H' a)
    {b : ℕ} {X : TrioSeq} (h : RN A k H b X) : RN A k H' b X := by
  intro g b' hb'
  have e : reliftX b' H' g A (mlift X b (b' - b)) = reliftX b' H g A (mlift X b (b' - b)) :=
    reliftX_congr b' (fun a ha => (hH a ha).symm) (fun _ _ => rfl) _
  rw [e]
  exact NX_congr (fun a ha => by simp [addF, hH a ha]) (h g b' hb')

theorem RN_nil {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k)
    (H : ℕ → ℕ) (b : ℕ) : RN A k H b [] := by
  intro g b' _
  rw [mlift_nil]
  have e : reliftX b' H g A [] = [] := by unfold reliftX; exact slift_nil _
  rw [e]
  exact NX_nil hA01 hk1 hAk _ _

theorem RN_okW {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} {b : ℕ} {X : TrioSeq}
    (h : RN A k H b X) : NX A k H b X := by
  have := h (fun _ => 0) b le_rfl
  rwa [addF_zero, Nat.sub_self, mlift_zero, reliftX_zero] at this

/-- ★ 状態の層の差し込み口の公理。 -/
theorem RN_ax {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (H : ℕ → ℕ) :
    SlotAx (RN A k H) where
  lift := by
  ...
  oper := by
    ...
    refine (NX_ax hA01 hk1 (addF H g)).oper b' _ _
  ...
  orph := by
    ...
    have eU2 : reliftX b' H g A (mlift (U ++ [((h, j, 0) : ℕ × ℕ × ℕ)]) u (b' - u))
      ...
      rw [eU]; rfl
    rw [eU2]
    refine (NX_ax hA01 hk1 (addF H g)).orph b' _ _ h j
  ...
  tie := by
    ...
    refine (NX_ax hA01 hk1 (addF H g)).tie b' _ _ x
  ...
  flat := by
    ...
    rw [e]
    exact (NX_ax hA01 hk1 (addF H g)).flat b' _ (Fr_reliftX (Fr_mlift hW _ _) _ _ _ _)
      (h g b' hb')

/-! ## FarP の場 -/

/-- 子の段が塊の段 b1 と同じ場合。 -/
theorem FarP_RN_core {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k)
    {P1 : TrioSeq} {b1 s1 d : ℕ} {H1 : ℕ → ℕ} (hP1Fr : Fr P1) (hd : 1 ≤ d)
    (hbotP1 : BotGe P1 d (b1 + s1)) (hs12 : 2 ≤ s1) (hsK : s1 ≤ reOff (fun _ => 0) H1 A k)
    (hP1ok : NX A k H1 b1 P1) (hHd : Hd (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)]))
    (h2' : ∀ (g2 : ℕ → ℕ) b', b1 ≤ b' → ∀ τ L, 1 ≤ τ → τ < reOff H1 g2 A s1 → Fr L →
      GC A (addF H1 g2) τ b' L →
      NX A k (addF H1 g2) b' (reliftX b' H1 g2 A (mlift P1 b1 (b' - b1)) ++
        shiftr01 (d - 1) 0 (((1, b' + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L)))
    (us : List (Option TrioSeq)) (hus : GoodLow b1 us)
    (Lds : List (List (Option TrioSeq))) (hL : GoodChuX A k H1 b1 Lds) :
    GoodChuX A k H1 b1 (Lds ++ [us ++ [some (P1 ++ [((d, b1 + s1, 0) : ℕ × ℕ × ℕ)])]]) := by
  ...
  rw [← e00]
  refine NX_snoc_of hA01 hk1 hL hus le_rfl (Fr_append hP1Fr (GzF.Fr_single hd _ _)) hHd
  ...
  have eF : fwH bb r (FTLu bb r b1 (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A))))) bb
        (((1, r, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (chF bb r b1 us ++ (P3 ++ [((d, bb + s', 0) : ℕ × ℕ × ℕ)])))
      = fwH bb r (FTLu bb r b1 (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A))))) bb
          (((1, r, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (chF bb r b1 us ++ P3)) ++
        [((d + 2, bb + s', 0) : ℕ × ℕ × ℕ)] := by
  ...
  have hQF : Fr (farWu bb r ws ++ fwH bb r (FTLu bb r b1
      (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A))))) bb
      (((1, r, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (chF bb r b1 us ++ P3))) :=
  ...
  have hbotC : BotGe (chF bb r b1 us ++ P3) d (bb + s') := by
    rcases P1 with _ | ⟨x, P1t⟩
    · have hd1 : d = 1 := by have := hHd (by simp); simpa [entry] using this
      subst hd1
      exact BotGe_one (Fr_append (Fr_chF _ _ _ hus.1) hP3F) _
    · refine BotGe_append_left (Fr_chF _ _ _ hus.1)
  ...
  have hbotQQ : BotGe (farWu bb r ws ++ fwH bb r (FTLu bb r b1
      (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A))))) bb
      (((1, r, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (chF bb r b1 us ++ P3))) (d + 2) (bb + s') := by
    unfold fwH
    simp only [Nat.sub_self, mlift_zero]
    exact BotGe_node (Fr_farWu _ _ _) (by omega) (BotGe_node (Fr_FTLu _ _ _ _) (by omega) hbotC)
  have hW1R : ∀ us' ∈ Lds ++ [us ++ [some (mlift P1 b1 (b1 - b1))]],
      RawUc (b1 + reOff (fun _ => 0) H1 A k) us' := by
    intro us' hus'
    ...
    · rw [List.mem_singleton] at hus'; rw [hus']
      ...
      · rw [List.mem_singleton, Option.some.injEq] at hY
        ...
        exact ⟨hP1Fr, hP1ok.1, hP1ok.2.1⟩
  have eQ : farWu bb r ws ++ fwH bb r (FTLu bb r b1
      (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A))))) bb
      (((1, r, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (chF bb r b1 us ++ P3))
      = farWu bb r (ws ++ [((Lds ++ [us ++ [some (mlift P1 b1 (b1 - b1))]]).map
          (fun us => us.map (Option.map (reliftX b1 H1 g A))), b1, [])]) := by
    rw [farWu_lastX hA01 H1 g le_rfl hub ws Lds hus.1 P1, hP3]
  have hRn : RawWsAu (S ++ A) o f bb (ws ++ [((Lds ++ [us ++ [some (mlift P1 b1 (b1 - b1))]]).map
      (fun us => us.map (Option.map (reliftX b1 H1 g A))), b1, [])]) :=
    RawWsAu_snoc hR ⟨hub, RawLds_emb hE hW1R, Fr_nil, fun h => absurd rfl h, LowC_nil _⟩
  have eP : ∀ (g' : ℕ → ℕ) b', bb ≤ b' →
      reliftX b' f g' (S ++ A) (mlift (farWu bb r ws ++ fwH bb r (FTLu bb r b1
        (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 g A))))) bb
        (((1, r, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (chF bb r b1 us ++ P3))) bb (b' - bb))
      = farWu b' (b' + liftOff (addF f g') (S ++ A) o + 1) (relWsu (S ++ A) f g' ws) ++
        fwH b' (b' + liftOff (addF f g') (S ++ A) o + 1)
          (FTLu b' (b' + liftOff (addF f g') (S ++ A) o + 1) b1
            (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 (addF g g') A))))) b'
          (((1, b' + liftOff (addF f g') (S ++ A) o + 1, 0) : ℕ × ℕ × ℕ) ::
            shiftr01 1 0 (chF b' (b' + liftOff (addF f g') (S ++ A) o + 1) b1 us ++
              reliftX b' H1 (addF g g') A (mlift P1 b1 (b' - b1)))) := by
    intro g' b' hb'
    rw [eQ, hr, mlift_farWu_self hRn hb', reliftX_farWu_self hA b' g' (RawWsAu_mono hb' hRn),
      relWsu_snoc, e0, EmbU_relLds hE b1 g' hW1R,
      farWu_lastX hA01 H1 (addF g g') le_rfl (by omega) _ Lds hus.1 P1]
  ...
  · -- h1
    rw [eP g' b' hb']
    exact NX_snoc_elim (hP1ok.2.2 b1 le_rfl us hus Lds hL) hA01 hus.1 le_rfl (addF g g') S o
      (addF f g') b' (EmbU_relift hE g') (by omega) _ (FarCAu_mono hb' (FarCAu_relift hC hRw g'))
  ...
  · -- h2
    ...
    have hL2 : GoodChuX A k (addF H1 (addF g g')) b1
        (Lds.map (fun us => us.map (Option.map (reliftX b1 H1 (addF g g') A)))) :=
      GoodChuX_emb hL (EmbU_self hA01 hk1 hAk H1 (addF g g'))
    have hL3 := GoodChuX_lift hL2 (show b1 ≤ b' by omega)
    have hus3 := GoodLow_lift hus (show b1 ≤ b' by omega)
    have hE3 : EmbU A k (addF H1 (addF g g')) (fun _ => 0) S o (addF f g') :=
      EmbU_shift (EmbU_relift hE g')
    have := NX_snoc_elim (hY.2.2 b' le_rfl _ hus3 _ hL3) hA01 hus3.1 le_rfl (fun _ => 0) S o
      (addF f g') b' hE3 le_rfl _ (FarCAu_mono hb' (FarCAu_relift hC hRw g'))
      (RawWsAu_relWsu (RawWsAu_mono hb' hR) g')
    rw [reliftX_zero, Nat.sub_self, mlift_zero, mapu_zero, FTLu_rebase (show b1 ≤ b' by omega) le_rfl,
      chF_rebase (show b1 ≤ b' by omega) le_rfl, fwH_child_app _ _ _ _ _ _ hd] at this
    rw [show d + 2 - 1 = d + 1 by omega, List.append_assoc]
    exact this

theorem FarP_RN {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k)
    {H : ℕ → ℕ} {s : ℕ} (hs2 : 2 ≤ s) (hs : s ≤ liftOff H A k) :
    FarP (GC A) (fun H' => RN A k H') A H s := by
  ...
  have hP1ok : NX A k H1 b1 P1 := by
    have := RN_okW (h1 g1 b1 hb1); rwa [← hP1, ← hH1] at this
  ...
  have h2' : ∀ (g2 : ℕ → ℕ) b', b1 ≤ b' → ∀ τ L, 1 ≤ τ → τ < reOff H1 g2 A s1 → Fr L →
    ...
    rw [eH] at hGL ⊢
    have := RN_okW (h2 (addF g1 g2) b' (by omega) τ L h1τ hτ' hL hGL)
  ...
  rw [eM]
  refine FarP_RN_core hA01 hk1 hAk (Fr_mlift hP1Fr _ _) hd ?_ (by omega) hsK
    ((NX_ax hA01 hk1 H1).lift b1 P1 hP1Fr hP1ok c' hc') ?_ ?_ us hus Lds hL
...
/-- ★ RN は錨の列 A・上限 k の文脈の族。 -/
theorem RN_ctx {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k) :
    CtxP (GC A) A k (fun H => RN A k H) := fun H =>
  ⟨RN_ax hA01 hk1 H, fun _ _ _ hH h => RN_congr hH h, fun g _ _ _ h => RN_lift h g,
    fun _ hs2 hs => FarP_RN hA01 hk1 hAk hs2 hs⟩

/-- ★ 子の位置の塊に、子の級 GC A H τ の並びを持つ節点を置く。 -/
theorem RN_node {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) (hAk : ∀ a ∈ A, a < k)
    {H : ℕ → ℕ} {u τ : ℕ} {X L : TrioSeq} (hX : Fr X)
    (h : RN A k H u X) (hL : GC A H τ u L) (hτ : τ ≤ liftOff H A k) :
    RN A k H u (X ++ ((1, u + τ, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 L) :=
  R_child (R := fun H => RN A k H) (RN_ctx hA01 hk1 hAk) hAk hX h hL hτ
