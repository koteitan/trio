/-
HeE.lean: 道の集合 P の低い塊の族 NXs P の差し込み口の tie / flat と SlotAx（HdN の写し、段つきの道）。

- tie: 荷は段 b'' の族で、道 Q と Lds を段 b'' へ持ち上げて置く（PSOK の lift）。
- flat: 空の道は F のタイの複製（BotT_Fflat）、空でない道は PSDec の分解で親の集合の節点の複製
  （BotT_tieflat は段 l の節点、複製の良さは Gd_jump の繰り返し）。
-/
import HeD
...
open HaA HaC HaE HaF HbD HbM HbP HbS HcA HcE HcF HcI HcM HdA HdB HdC HdD HdE HdF HdG HdH HdI HdJ HeB HeC HeD
...
theorem chT_rep_tie (b r u : ℕ) (us0 y : List UT) (l : ℕ) : ∀ m,
    chT b r u (us0 ++ List.replicate m (UT.tie l y))
      = chT b r u us0 ++ (List.range m).flatMap
          (fun _ => ((1, r + l, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (chT b r u y))
  | 0 => by simp
  | m + 1 => by
      rw [List.replicate_succ', ← List.append_assoc, chT_append, chT_rep_tie b r u us0 y l m,
...
theorem NXs_tie {P : PS} (hP : PSOK P) {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a) (hk1 : 1 ≤ k) {H : ℕ → ℕ}
    ...
    (hload : ∀ c'', c ≤ c'' → ∀ Z ∈ Wg (2 * c''), based Z →
      NXs P A k H c'' (mlift (W ++ U) c (c'' - c) ++ shiftr01 x 0 Z)) :
    NXs P A k H c (W ++ (U ++ [((x, c + 1, 0) : ℕ × ℕ × ℕ)])) := by
  ...
  · -- 荷: 段 b'' の族
    ...
    have := NXs_snoc_elim (hL2.2.2 b'' le_rfl _ (Gd_lift hus (show c' ≤ b'' by omega))) le_rfl
      G S o f hE b'' le_rfl _ (GoodChtX_lift hL hub'') _ (hP.lift _ _ _ _ _ hQ b'' hub'') b'' le_rfl ws
      (FarCAt_mono (by omega) hC) (RawWsAt_mono (by omega) hR)
    have eL2 : Lds.map (mlTs u (b'' - u)) ++ [plugQ (mapQ (mlTs u (b'' - u)) Q) []]
    ...
    have eY : reliftX b'' H G A (mlift (W ++ U) c (b'' - c) ++ shiftr01 x 0 Z)
      ...
      rw [e]
    rw [mapQ_length, Nat.sub_self, mlift_zero, eL2, FTLt_rebase hub'' le_rfl, fwH_nil_c _ _ _ b'' u,
      eimg, chT_rebase hub'' le_rfl, eY] at this
    have hL2raw : ∀ us' ∈ Lds ++ [plugQ Q []], RawTs (u + reOff (fun _ => 0) f (S ++ A) o) us' := by
      intro us' h'
      rcases List.mem_append.mp h' with h' | h'
      · exact hL.1 us' h'
      · simp at h'; subst h'; exact RawTs_plugQ Q [] (hP.raw _ _ _ _ _ hQ) (by simp [RawTs])
    ...
    rw [mlift_farWt_self hR hb'', mlift_pathB (PathCone_fwQ hbr b u _ Lds Q (hP.raw _ _ _ _ _ hQ)) hB0,
...
theorem BotT_tieflat {C : List ℕ} {o : ℕ} {f : ℕ → ℕ} (hC : ∀ a ∈ C, a < o) (hC1 : ∀ a ∈ C, 1 ≤ a)
    (ho : 1 ≤ o) {u K : ℕ} {Lds : List (List UT)} {Q : Path} {l : ℕ} {us0 x : List UT}
    (hus0 : RawTs K us0) (hx : RawTs K x) {W1 : TrioSeq} (hW1 : Fr W1)
    (hrep : ∀ m, BotT C o f u Lds Q (us0 ++ List.replicate m (UT.tie l (x ++ [UT.ch W1])))) :
    BotT C o f u Lds Q (us0 ++ [UT.tie l (x ++ [UT.ch (W1 ++ [((1, 0, 0) : ℕ × ℕ × ℕ)])])]) := by
  intro b hub ws hFC hR
  ...
  have eT : chT b R u (us0 ++ [UT.tie l (x ++ [UT.ch (W1 ++ [((1, 0, 0) : ℕ × ℕ × ℕ)])])])
      = chT b R u us0 ++ ((1, R + l, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 (D ++ [((1, 0, 0) : ℕ × ℕ × ℕ)]) := by
    rw [chT_append, hD]
    simp only [chT, unitT, List.append_nil, Nat.add_zero]
    rw [chT_snoc_ch, mlift_snoc_flat W1 1 u (b - u) hW1, List.append_assoc]
  have eG : Yfw ++ shiftr01 d 0 (chT b R u us0 ++ ((1, R + l, 0) : ℕ × ℕ × ℕ) ::
        shiftr01 1 0 (D ++ [((1, 0, 0) : ℕ × ℕ × ℕ)]))
      = (Yfw ++ shiftr01 d 0 (chT b R u us0)) ++ shiftr01 d 0 (((1, R + l, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 D) ++
          [((d + 1 + 1, 0, 0) : ℕ × ℕ × ℕ)] := by
  ...
  · intro r' hr1 hr2
    ...
    have hr2' : w + 1 < (((1, R + l, 0) : ℕ × ℕ × ℕ) :: shiftr01 1 0 D).length := by
...
theorem NXs_flat {P : PS} (hP : PSOK P) (hD : HeC.PSDec P) {A : List ℕ} {k : ℕ} {H : ℕ → ℕ} (c : ℕ)
    (W : TrioSeq) (hW : Fr W) (h : NXs P A k H c W) : NXs P A k H c (W ++ [((1, 0, 0) : ℕ × ℕ × ℕ)]) := by
  ...
  · rw [BotT_snoc_eq]
    ...
    have hrep : Gd P' (S ++ A) o f u (us0 ++ List.replicate m
        (UT.tie l (imgT A H G c' u (us ++ [UT.ch (mlift W c (c' - c))])))) := by
      induction m with
      | zero => simpa using hus0
      | succ m ih => rw [List.replicate_succ', ← List.append_assoc]; exact Gd_jump hX hW1gt hE hcu ih
...
/-- ★ 道の集合 P の低い塊の族の差し込み口の公理。 -/
theorem NXs_ax {P : PS} (hP : PSOK P) (hD : HeC.PSDec P) {A : List ℕ} {k : ℕ} (hA01 : ∀ a ∈ A, 1 ≤ a)
    (hk1 : 1 ≤ k) (H : ℕ → ℕ) : SlotAx (NXs P A k H) where
  lift := fun u W _ h u' hu => NXs_lift u W h u' hu
  oper := fun u W U hW hU hH hlen hp hIH => NXs_oper hP u W U hW hU hH hlen hp hIH
  orph := fun u W U h j hW hU hH hj1 hj hnp hz => NXs_orph hP u W U h j hW hU hH hj1 hj hnp hz
  tie := fun u W U x hW hU hH hc hload => NXs_tie hP hA01 hk1 u W U x hW hU hH hc hload
  flat := fun u W hW h => NXs_flat hP hD u W hW h
