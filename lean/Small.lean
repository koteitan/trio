...
/-! ### ★★★★★★★★★ 荷 `Ls = (0,0,0)(1,0,0)(2,0,0)(2,0,0)`（`Ls⟦n+1⟧ = Vk n`）

指数は `ω^(ω²) = owG (ow 2 1) 1`（まだ `Bwx` の中）。 -/

def Ls : TrioSeq :=
  [((0, 0, 0) : ℕ × ℕ × ℕ), ((1, 0, 0) : ℕ × ℕ × ℕ),
    ((2, 0, 0) : ℕ × ℕ × ℕ), ((2, 0, 0) : ℕ × ℕ × ℕ)]

theorem Ls_len : Ls.length = 4 := by simp [Ls]

theorem Flat_Ls : Flat Ls := by
  intro c hc
  simp only [Ls, List.mem_cons, List.not_mem_nil, or_false] at hc
  rcases hc with rfl | rfl | rfl | rfl <;> exact ⟨rfl, rfl⟩

theorem Bok_Ls : Bok Ls := Bok_flat Flat_Ls (by simp [Ls, entry])

theorem Ls_srow (i : ℕ) : srow Ls i = 0 := by
  simp [srow, (Flat_entry Flat_Ls i).1, (Flat_entry Flat_Ls i).2]

theorem Ls_hasParent : hasParent Ls 0 3 := by
  rw [hasParent_zero_iff (by rw [Ls_len]; omega)]
  exact ⟨1, by omega, by simp [Ls, entry]⟩

theorem Ls_parent : parent Ls 0 3 = 1 := by
  have h := parent_nextR Ls_hasParent
  rw [nextR, if_pos rfl] at h
  obtain ⟨-, -, hlt, hval, hmid⟩ := h
  rcases Nat.lt_or_ge (parent Ls 0 3) 1 with hp | hp
  · have := hmid 1 ⟨by omega, by omega⟩
    simp [Ls, entry] at this
  · rcases Nat.lt_or_ge (parent Ls 0 3) 2 with hp2 | hp2
    · omega
    · rw [show parent Ls 0 3 = 2 from by omega] at hval
      simp [Ls, entry] at hval

theorem hasParent_Ls_last :
    hasParent Ls (srow Ls (Ls.length - 1)) (Ls.length - 1) := by
  rw [Ls_len, Ls_srow]
  exact Ls_hasParent

theorem oper_Ls (n : ℕ) : Ls⟦n + 1⟧ = Vk n := by
  have h3 : Ls.length - 1 = 3 := by rw [Ls_len]
  simp only [oper, h3, Ls_srow, Ls_parent]
  rw [if_neg (by omega), if_neg (by simp [Ls, entry]),
    if_neg (by rw [h3, Ls_srow]; exact not_not_intro Ls_hasParent)]
  simp only [Nat.lt_irrefl, show ¬ ((1 : ℕ) < 0) from by omega, if_false,
    Nat.mul_zero, Nat.add_zero, ite_self]
  have e1 : (List.range' 1 (3 - 1)).map
      (fun j => ((entry Ls 0 j, entry Ls 1 j, entry Ls 2 j) : ℕ × ℕ × ℕ))
      = [((1, 0, 0) : ℕ × ℕ × ℕ), ((2, 0, 0) : ℕ × ℕ × ℕ)] := by
    simp [Ls, entry, List.range']
  rw [show (Ls.take 1 : TrioSeq) = [((0, 0, 0) : ℕ × ℕ × ℕ)] from rfl, e1, Vk_eq]
  rfl

theorem AtLd_Ls : AtLd (α := Bwx) Ls (owG (ow 2 1) 1) := by
  refine AtLd_fam (F := fun n => Vk n) (qf := fun n => owG (ow 1 (n + 1)) 1)
    Bok_Ls (by rw [Ls_len]; omega) hasParent_Ls_last
    (fun n l Z => by rw [oper_Ls]) AtLd_Vk ?_ PwsX.add_lt
  intro n
  exact owG_ltL (ow_ltL (by omega : 1 < 2) (n + 1) (by omega)) 1 (by omega)

theorem TopLd_Ls : TopLd (α := Bwx) Ls (owG (ow 2 1) 1) := TopLd_of_AtLd AtLd_Ls

theorem RunLd_Ls : RunLd (α := Bwx) Ls (fun n => owG (ow 2 1) n) :=
  RunLd_of_TopLd Bok_Ls (by rw [owG_zero, bot_BwG])
    (fun n => owG_add_same (ow 2 1) n) TopLd_Ls

theorem WPdw_twoLs (n : ℕ) (ks : List Bwy) :
    WPdT (owG (owG (ow 2 1) (n + 1)) 2 :: ks)
      (Jk1.two Jk1.nil (PayIt Jk1.nil Ls (n + 1))) := by
  have hR : RnG (PayIt Jk1.nil Ls n) (owG (ow 2 1) n) := by
    have h := RunLd_Ls n Jk1.nil trivial 0 (RunG_nil (0 : Bwx))
    rwa [zero_add] at h
  refine TopLd_Ls (PayIt Jk1.nil Ls n)
    (JkA_PayIt (Z := Jk1.nil) trivial Bok_Ls n) (owG (ow 2 1) n) hR
    Jk1.nil trivial ⊥ (fun c _ ks' => WPdT_nilAll _) _ ?_ ks
  rw [bot_BwG, zero_add, owG_add_same]
  exact owG_ltR (owG (ow 2 1) (n + 1)) (by omega)

def Xl (n : ℕ) : Jk1 :=
  Jk1.two Jk1.nil (Jk1.two Jk1.nil (PayIt Jk1.nil Ls (n + 1)))

theorem WPdw_Xl (n : ℕ) (ks : List Bwy) : WPdT ((⊥ : Bwy) :: ks) (Xl n) :=
  WPdT_twoOf (b := owG (owG (ow 2 1) (n + 1)) 2)
    (ne_bot_of_gt (owG_pos_succ (owG (ow 2 1) (n + 1)) 1)) trivial
    (fun q _ => WPdT_nilAll _) (WPdw_twoLs n ks)

theorem GOK_oneXl (n : ℕ) : GOK (Jk1.one Jk1.nil (Xl n)) :=
  (WPdT_bnil (Bud := Bwy) _).mp
    (WPdT_step ([] : List Bwy) (JkT_nil : FrmNT ([] : List Bwy) Jk1.nil)
      ((WPdT_bnil (Bud := Bwy) _).mpr GOK_nil) (WPdw_Xl n []))

theorem shift_Ls (l : ℕ) : shiftr01 (l + 1) 0 Ls
    = [((l + 1, 0, 0) : ℕ × ℕ × ℕ), ((l + 2, 0, 0) : ℕ × ℕ × ℕ),
        ((l + 3, 0, 0) : ℕ × ℕ × ℕ), ((l + 3, 0, 0) : ℕ × ℕ × ℕ)] := by
  show [((0 + (l + 1) : ℕ), (0 + 0 : ℕ), (0 : ℕ)),
      ((1 + (l + 1) : ℕ), (0 + 0 : ℕ), (0 : ℕ)),
...
/-- 塊 `Mc = (6,0,0)(7,0,0)(8,0,0)(8,0,0)`。 -/
def Mc : TrioSeq :=
  [((6, 0, 0) : ℕ × ℕ × ℕ), ((7, 0, 0) : ℕ × ℕ × ℕ),
    ((8, 0, 0) : ℕ × ℕ × ℕ), ((8, 0, 0) : ℕ × ℕ × ℕ)]

theorem jk1_Xl (n l : ℕ) : jk1 l (Xl n)
    = ((l + 1, 2, 0) : ℕ × ℕ × ℕ) :: ((l + 2, 2, 0) : ℕ × ℕ × ℕ)
      :: (List.range (n + 1)).flatMap (fun _ =>
          [((l + 3, 0, 0) : ℕ × ℕ × ℕ), ((l + 4, 0, 0) : ℕ × ℕ × ℕ),
            ((l + 5, 0, 0) : ℕ × ℕ × ℕ), ((l + 5, 0, 0) : ℕ × ℕ × ℕ)]) := by
  show jk1 l Jk1.nil ++ (((l + 1, 2, 0) : ℕ × ℕ × ℕ) ::
      (jk1 (l + 1) Jk1.nil ++ (((l + 2, 2, 0) : ℕ × ℕ × ℕ)
        :: jk1 (l + 2) (PayIt Jk1.nil Ls (n + 1))))) = _
  rw [jk1_PayNilG Ls (n + 1) (l + 2), shift_Ls (l + 2),
    show l + 2 + 1 = l + 3 from by omega, show l + 2 + 2 = l + 4 from by omega,
    show l + 2 + 3 = l + 5 from by omega]
  simp [jk1]

theorem R375m_Mc_rep_mem : ∀ n : ℕ,
    R375m ++ (List.range n).flatMap (fun _ => Mc) ∈ W 0
  | 0 => by simpa [R375m] using R375m_mem
  | (n + 1) => by
      have hG : GoodFb (fun a b => wordJ a b [Jk1.one Jk1.nil (Xl n)]) := by
        simpa using GOK_oneXl n [] WOk_nil GoodFb_wordJ_nil
      have hh := rowJ_mem_genF Aok_R338 hG
      have e : jk1 2 (Jk1.one Jk1.nil (Xl n))
          = ((3, 1, 0) : ℕ × ℕ × ℕ) :: ((4, 2, 0) : ℕ × ℕ × ℕ) :: ((5, 2, 0) : ℕ × ℕ × ℕ)
            :: (List.range (n + 1)).flatMap (fun _ => Mc) := by
        show jk1 2 Jk1.nil ++ (((2 + 1, 1, 0) : ℕ × ℕ × ℕ) :: jk1 (2 + 1) (Xl n)) = _
        rw [jk1_Xl n 3]
        simp [jk1, Mc]
      rw [wordJ_singleton, colJ, e] at hh
      simpa [R375m, R373, R344, R341, R338, List.append_assoc] using hh

/-- ★★★★★★★★★★★★★ `R600 (7,0,0)(8,0,0)(8,0,0)(7,0,0)`。 -/
theorem R600_7887_mem :
    R600 ++ [((7, 0, 0) : ℕ × ℕ × ℕ), ((8, 0, 0) : ℕ × ℕ × ℕ),
      ((8, 0, 0) : ℕ × ℕ × ℕ), ((7, 0, 0) : ℕ × ℕ × ℕ)] ∈ W 0 := by
  have hne : Mc ≠ [] := by simp [Mc]
  have hhead : entry Mc 0 0 < 7 := by simp [Mc, entry]
  have htail : ∀ r, 1 ≤ r → r < Mc.length → 7 ≤ entry Mc 0 r := by
    intro r h1 h2
    simp only [Mc, List.length_cons, List.length_nil] at h2
    rcases r with _ | _ | _ | _ | r
    · omega
    · simp [Mc, entry]
    · simp [Mc, entry]
    · simp [Mc, entry]
    · omega
  have hmem := flat_mem'' (Y0 := R375m) (M := Mc) (d := 7) hne hhead htail
    R375m_Mc_rep_mem
...
/-- 新しい台座 `R600 (7,0,0)(8,0,0)(8,0,0)(7,0,0)(8,0,0)(7,0,0)`。 -/
def Z6 : TrioSeq :=
  R600 ++ [((7, 0, 0) : ℕ × ℕ × ℕ), ((8, 0, 0) : ℕ × ℕ × ℕ),
    ((8, 0, 0) : ℕ × ℕ × ℕ), ((7, 0, 0) : ℕ × ℕ × ℕ),
    ((8, 0, 0) : ℕ × ℕ × ℕ), ((7, 0, 0) : ℕ × ℕ × ℕ)]

theorem Z6_eq : Z6 = [((0, 0, 0) : ℕ × ℕ × ℕ),
    ((1, 1, 1) : ℕ × ℕ × ℕ),
    ((2, 1, 0) : ℕ × ℕ × ℕ),
    ((1, 1, 0) : ℕ × ℕ × ℕ),
    ((2, 2, 1) : ℕ × ℕ × ℕ),
    ...
    ((8, 0, 0) : ℕ × ℕ × ℕ),
    ((7, 0, 0) : ℕ × ℕ × ℕ)] := by
  simp [Z6, R600, R375m, R373, R344, R341, R338]

theorem Z6_ne : Z6 ≠ [] := by rw [Z6_eq]; simp

theorem Z6_head : entry Z6 0 0 = 0 := by rw [Z6_eq]; simp [entry]

theorem Z6_tail : ∀ r, 1 ≤ r → r < Z6.length → 1 ≤ entry Z6 0 r := by
  intro r hr1 hrl
  rw [Z6_eq] at hrl ⊢
  simp only [List.length_cons, List.length_nil] at hrl
...
theorem Aok_Z6 : Aok Z6 where
  mem := R600_788787_mem
  ne := Z6_ne
  deep := ⟨Z6_head, Z6_tail⟩
  zroot := by
    rw [Z6_eq]
    intro c hc
    simp only [List.mem_cons, List.not_mem_nil, or_false] at hc
...
theorem LoopIt_Z6_mem (m : ℕ) {ws : List Jk1} (hw : WJ ws) (j n : ℕ) :
    LoopIt Z6 m ws j n ∈ W 0 := (Aok_LoopIt Aok_Z6 m hw j n).mem

/-- ★★★★★★★★★★★★★★★ 新しい台座の上に `hang6_gen` で `LoopIt` を吊るす。 -/
theorem Z6_hang6_LoopIt (m : ℕ) {ws : List Jk1} (hw : WJ ws) (j n : ℕ) :
    Z6 ++ U375a1 ++ shiftr01 6 0 (LoopIt Z6 m ws j n) ∈ W 0 :=
  hang6_gen Aok_Z6 (Aok_LoopIt Aok_Z6 m hw j n).toBok

theorem Z6_hang6_LoopIt_nil (m p j n : ℕ) :
    Z6 ++ U375a1 ++ shiftr01 6 0 (LoopIt Z6 m (List.replicate p (AltT 0)) j n)
      ∈ W 0 :=
  Z6_hang6_LoopIt m (WJ_rep_AltT 0 p) j n
...
/-! ### ★★★★★★★★★★★★ 荷の一般族 `Wgv a b k Y0 = Y0 (a,0,0)(b,0,0)^k`（`a < b`）

`Wgv 1 2 = Wg`。`(Wgv a b (k+1) Y0)⟦n⟧ = WgvIt a b k Y0 n`。
最後の `(b,0,0)` の親は必ず `(a,0,0)`（間の `(b,0,0)` が全部 `≥ b`）なので、
`Y0` の形に依らない。 -/

def Wgv (a b : ℕ) (k : ℕ) (Y0 : TrioSeq) : TrioSeq :=
  Y0 ++ (((a, 0, 0) : ℕ × ℕ × ℕ) :: List.replicate k ((b, 0, 0) : ℕ × ℕ × ℕ))

def WgvIt (a b : ℕ) (k : ℕ) (Y0 : TrioSeq) : ℕ → TrioSeq
  | 0 => Y0
  | (m + 1) => Wgv a b k (WgvIt a b k Y0 m)

theorem Wgv_len (a b k : ℕ) (Y0 : TrioSeq) :
    (Wgv a b k Y0).length = Y0.length + k + 1 := by
  simp [Wgv]; omega

theorem Flat_Wgv {Y0 : TrioSeq} (hf : Flat Y0) (a b k : ℕ) : Flat (Wgv a b k Y0) := by
  intro c hc
  rw [Wgv, List.mem_append] at hc
  rcases hc with hc | hc
  · exact hf c hc
  · simp only [List.mem_cons, List.mem_replicate] at hc
    rcases hc with rfl | ⟨-, rfl⟩ <;> exact ⟨rfl, rfl⟩

theorem entry_Wgv_lt {Y0 : TrioSeq} {i : ℕ} (h : i < Y0.length) (a b k r : ℕ) :
    entry (Wgv a b k Y0) r i = entry Y0 r i := by
  have hg : (Wgv a b k Y0).getD i ((0, 0, 0) : ℕ × ℕ × ℕ)
      = Y0.getD i ((0, 0, 0) : ℕ × ℕ × ℕ) := by
    rw [Wgv, List.getD_eq_getElem?_getD, List.getD_eq_getElem?_getD,
      List.getElem?_append_left h]
  simp only [entry, hg]

theorem entry_Wgv_mid {Y0 : TrioSeq} (a b k : ℕ) :
    entry (Wgv a b k Y0) 0 Y0.length = a := by
  have hg : (Wgv a b k Y0).getD Y0.length ((0, 0, 0) : ℕ × ℕ × ℕ)
      = ((a, 0, 0) : ℕ × ℕ × ℕ) := by
    rw [Wgv, List.getD_eq_getElem?_getD, List.getElem?_append_right (le_refl _)]
    simp
  show ((Wgv a b k Y0).getD Y0.length ((0, 0, 0) : ℕ × ℕ × ℕ)).1 = a
  rw [hg]

theorem entry_Wgv_hi {Y0 : TrioSeq} {a b k i : ℕ} (h1 : Y0.length < i)
    (h2 : i < Y0.length + k + 1) : entry (Wgv a b k Y0) 0 i = b := by
  have hg : (Wgv a b k Y0).getD i ((0, 0, 0) : ℕ × ℕ × ℕ)
      = ((b, 0, 0) : ℕ × ℕ × ℕ) := by
    rw [Wgv, List.getD_eq_getElem?_getD, List.getElem?_append_right (by omega),
      ← List.getD_eq_getElem?_getD,
      show i - Y0.length = (i - Y0.length - 1) + 1 from by omega,
      List.getD_cons_succ, getD_rep, if_pos (by omega)]
  show ((Wgv a b k Y0).getD i ((0, 0, 0) : ℕ × ℕ × ℕ)).1 = b
  rw [hg]

theorem Wgv_ne (a b k : ℕ) (Y0 : TrioSeq) : Wgv a b k Y0 ≠ [] := by
  intro hc
  have := Wgv_len a b k Y0
  rw [hc] at this
  simp at this

theorem Wgv_root {Y0 : TrioSeq} (hne : Y0 ≠ []) (hr : entry Y0 0 0 = 0) (a b k : ℕ) :
    entry (Wgv a b k Y0) 0 0 = 0 := by
  rw [entry_Wgv_lt (List.length_pos_iff.mpr hne) a b k 0, hr]

theorem Wgv_pos {Y0 : TrioSeq} (hne : Y0 ≠ [])
    (hpos : ∀ i, 1 ≤ i → i < Y0.length → 1 ≤ entry Y0 0 i) {a b : ℕ}
    (ha : 1 ≤ a) (hab : a < b) (k : ℕ) :
    ∀ i, 1 ≤ i → i < (Wgv a b k Y0).length → 1 ≤ entry (Wgv a b k Y0) 0 i := by
  intro i h1 h2
  rw [Wgv_len] at h2
  rcases Nat.lt_or_ge i Y0.length with hi | hi
  · rw [entry_Wgv_lt hi]
    exact hpos i h1 hi
  · rcases Nat.lt_or_ge Y0.length i with hi2 | hi2
    · rw [entry_Wgv_hi hi2 (by omega)]; omega
    · rw [show i = Y0.length from by omega, entry_Wgv_mid]; omega

theorem Wgv_srow {Y0 : TrioSeq} (hf : Flat Y0) (a b k i : ℕ) :
    srow (Wgv a b k Y0) i = 0 := by
  simp [srow, (Flat_entry (Flat_Wgv hf a b k) i).1,
    (Flat_entry (Flat_Wgv hf a b k) i).2]

theorem Wgv_hasParent {Y0 : TrioSeq} (hne : Y0 ≠ []) {a b : ℕ} (hab : a < b) (k : ℕ) :
    hasParent (Wgv a b (k + 1) Y0) 0 (Y0.length + k + 1) := by
  rw [hasParent_zero_iff (by rw [Wgv_len]; omega)]
  refine ⟨Y0.length, by omega, ?_⟩
  rw [entry_Wgv_mid, entry_Wgv_hi (by omega) (by omega)]
  exact hab

theorem Wgv_parent {Y0 : TrioSeq} (hne : Y0 ≠ []) {a b : ℕ} (hab : a < b) (k : ℕ) :
    parent (Wgv a b (k + 1) Y0) 0 (Y0.length + k + 1) = Y0.length := by
  have h := parent_nextR (Wgv_hasParent hne hab k)
  rw [nextR, if_pos rfl] at h
  obtain ⟨-, -, hlt, hval, hmid⟩ := h
  by_contra hneq
  rcases Nat.lt_or_ge (parent (Wgv a b (k + 1) Y0) 0 (Y0.length + k + 1)) Y0.length
    with hp | hp
  · have hmidL := hmid Y0.length ⟨hp, by omega⟩
    rw [entry_Wgv_hi (by omega) (by omega), entry_Wgv_mid] at hmidL
    omega
  · rw [entry_Wgv_hi (by omega : Y0.length < parent (Wgv a b (k + 1) Y0) 0
        (Y0.length + k + 1)) (by omega),
      entry_Wgv_hi (by omega) (by omega)] at hval
    omega

theorem Wgv_take {Y0 : TrioSeq} (a b k : ℕ) : (Wgv a b k Y0).take Y0.length = Y0 := by
  show (Y0 ++ _).take Y0.length = _
  exact List.take_left

theorem Wgv_drop {Y0 : TrioSeq} (a b k : ℕ) : (Wgv a b k Y0).drop Y0.length
    = ((a, 0, 0) : ℕ × ℕ × ℕ) :: List.replicate k ((b, 0, 0) : ℕ × ℕ × ℕ) := by
  show (Y0 ++ _).drop Y0.length = _
  exact List.drop_left

theorem WgvIt_eq {Y0 : TrioSeq} (a b k : ℕ) : ∀ m : ℕ, WgvIt a b k Y0 m
    = Y0 ++ (List.range m).flatMap
        (fun _ => ((a, 0, 0) : ℕ × ℕ × ℕ) :: List.replicate k ((b, 0, 0) : ℕ × ℕ × ℕ))
  | 0 => by simp [WgvIt]
  | (m + 1) => by
      show Wgv a b k (WgvIt a b k Y0 m) = _
      rw [Wgv, WgvIt_eq a b k m, List.range_succ, List.flatMap_append,
        List.append_assoc]
      simp

theorem oper_Wgv {Y0 : TrioSeq} (hf : Flat Y0) (hne : Y0 ≠ []) {a b : ℕ}
    (hab : a < b) (k n : ℕ) :
    (Wgv a b (k + 1) Y0)⟦n⟧ = WgvIt a b k Y0 n := by
  have h0 : 0 < Y0.length := List.length_pos_iff.mpr hne
  have h2 : (Wgv a b (k + 1) Y0).length - 1 = Y0.length + k + 1 := by
    rw [Wgv_len]; omega
  simp only [oper, h2, Wgv_srow hf, Wgv_parent hne hab]
  rw [if_neg (by omega),
    if_neg (by rw [entry_Wgv_hi (by omega) (by omega)]; simp; omega),
    if_neg (by rw [h2, Wgv_srow hf]; exact not_not_intro (Wgv_hasParent hne hab k))]
  simp only [Nat.lt_irrefl, show ¬ ((1 : ℕ) < 0) from by omega, if_false,
    Nat.mul_zero, Nat.add_zero, ite_self]
  have e1 : (List.range' Y0.length (Y0.length + k + 1 - Y0.length)).map
      (fun j => ((entry (Wgv a b (k + 1) Y0) 0 j, entry (Wgv a b (k + 1) Y0) 1 j,
        entry (Wgv a b (k + 1) Y0) 2 j) : ℕ × ℕ × ℕ))
      = ((a, 0, 0) : ℕ × ℕ × ℕ) :: List.replicate k ((b, 0, 0) : ℕ × ℕ × ℕ) := by
    rw [show Y0.length + k + 1 - Y0.length = k + 1 from by omega,
      map_range'_entry_gen (M := Wgv a b (k + 1) Y0) (p := Y0.length) (k := k + 1)
        (by rw [Wgv_len]; omega), Wgv_drop]
    show (((a, 0, 0) : ℕ × ℕ × ℕ) ::
      List.replicate (k + 1) ((b, 0, 0) : ℕ × ℕ × ℕ)).take (k + 1) = _
    rw [List.take_succ_cons, List.take_replicate]
    simp
  rw [Wgv_take, e1, WgvIt_eq]

theorem Flat_WgvIt {Y0 : TrioSeq} (hf : Flat Y0) (a b k : ℕ) :
    ∀ m : ℕ, Flat (WgvIt a b k Y0 m)
  | 0 => hf
  | (m + 1) => Flat_Wgv (Flat_WgvIt hf a b k m) a b k

theorem WgvIt_ne {Y0 : TrioSeq} (hne : Y0 ≠ []) (a b k : ℕ) :
    ∀ m : ℕ, WgvIt a b k Y0 m ≠ []
  | 0 => hne
  | (m + 1) => Wgv_ne a b k (WgvIt a b k Y0 m)

theorem WgvIt_root {Y0 : TrioSeq} (hne : Y0 ≠ []) (hr : entry Y0 0 0 = 0) (a b k : ℕ) :
    ∀ m : ℕ, entry (WgvIt a b k Y0 m) 0 0 = 0
  | 0 => hr
  | (m + 1) => Wgv_root (WgvIt_ne hne a b k m) (WgvIt_root hne hr a b k m) a b k

theorem WgvIt_pos {Y0 : TrioSeq} (hne : Y0 ≠ [])
    (hpos : ∀ i, 1 ≤ i → i < Y0.length → 1 ≤ entry Y0 0 i) {a b : ℕ}
    (ha : 1 ≤ a) (hab : a < b) (k : ℕ) :
    ∀ m : ℕ, ∀ i, 1 ≤ i → i < (WgvIt a b k Y0 m).length →
      1 ≤ entry (WgvIt a b k Y0 m) 0 i
  | 0 => hpos
  | (m + 1) => Wgv_pos (WgvIt_ne hne a b k m) (WgvIt_pos hne hpos ha hab k m) ha hab k
...
/-! ### ★★★★★★★★★★★★ 台座一般の最大の単位 `U375aH` を新台座に積む

`SmallA` には `A ++ 単位 ∈ W 0`（`A` は任意の `Aok`）の補題が 22 本あり、
一番大きいのが

    U375aH = (1,1,0)(2,2,1)(3,1,0)(4,2,0)(5,2,0)(5,1,0)(6,2,0)(7,2,0)(7,1,0)
             (8,2,0)(9,2,0)(9,1,0)(10,2,0)(11,2,0)

`MidD_U375aH` があるので `Aok_append_Mid` で `Aok` も閉じ、いくらでも積める。 -/

theorem Aok_append_U375aH {A : TrioSeq} (hA : Aok A) : Aok (A ++ U375aH) :=
  Aok_append_Mid (d := 2) (by omega) hA MidD_U375aH (U375aH_mem_gen hA)

/-- `A` の上に `U375aH` を `m` 個積む。 -/
def UHIt (A : TrioSeq) : ℕ → TrioSeq
  | 0 => A
  | (m + 1) => UHIt A m ++ U375aH

theorem Aok_UHIt {A : TrioSeq} (hA : Aok A) : ∀ m : ℕ, Aok (UHIt A m)
  | 0 => hA
  | (m + 1) => Aok_append_U375aH (Aok_UHIt hA m)

theorem UHIt_mem {A : TrioSeq} (hA : Aok A) (m : ℕ) : UHIt A m ∈ W 0 :=
  (Aok_UHIt hA m).mem

/-- ★★★★★★★★★★★★ 新台座 `Z789` の上に `U375aH` を `m` 個。 -/
theorem Z789_UHIt_mem (m : ℕ) : UHIt Z789 m ∈ W 0 := UHIt_mem Aok_Z789 m

/-- ★★★★★★★★★★★★ さらにその上に `LoopIt` を吊るす。 -/
theorem Z789_UHIt_LoopIt_mem (m k : ℕ) {ws : List Jk1} (hw : WJ ws) (j n : ℕ) :
    LoopIt (UHIt Z789 m) k ws j n ∈ W 0 :=
  (Aok_LoopIt (Aok_UHIt Aok_Z789 m) k hw j n).mem

#print axioms Z789_UHIt_mem
#print axioms Z789_UHIt_LoopIt_mem

/-- ★★★★★★★★★★★★★ `U375aH` の積み上げの極限（`flat_of_chain`）。 -/
theorem UHIt_lim_mem {A : TrioSeq} (hA : Aok A) :
    A ++ U375aH ++ [((2, 0, 0) : ℕ × ℕ × ℕ)] ∈ W 0 :=
  flat_of_chain (Y0 := A) (M := U375aH) (d := 2) (by omega) MidD_U375aH hA
    (fun n hAn => U375aH_mem_gen hAn)

theorem Z789_UH_lim_mem : Z789 ++ U375aH ++ [((2, 0, 0) : ℕ × ℕ × ℕ)] ∈ W 0 :=
  UHIt_lim_mem Aok_Z789

/-- 台座を `UHIt Z789 m` にしても同じ。 -/
theorem Z789_UHIt_lim_mem (m : ℕ) :
    UHIt Z789 m ++ U375aH ++ [((2, 0, 0) : ℕ × ℕ × ℕ)] ∈ W 0 :=
  UHIt_lim_mem (Aok_UHIt Aok_Z789 m)
...
/-! ### ★★★★★★★★★★★★★★★ `H(6,2,0)`: 単位の尻尾の 1 ずらし塔 -/

theorem TailH_eq : TailH = ((5, 1, 0) : ℕ × ℕ × ℕ) :: jk1 5 (Jk1.two Jk1.nil (TW 2)) := by
  simp only [TailH, jk1, TW, List.nil_append, List.cons_append, List.append_nil,
    List.singleton_append]

theorem tailH_shift (k : ℕ) : shiftr01 k 0 TailH
    = ((4 + k + 1, 1, 0) : ℕ × ℕ × ℕ)
      :: jk1 (4 + k + 1) (Jk1.two Jk1.nil (TW 2)) := by
  rw [TailH_eq]
  show ((5 + k, 1 + 0, 0) : ℕ × ℕ × ℕ)
    :: shiftr01 k 0 (jk1 5 (Jk1.two Jk1.nil (TW 2))) = _
  rw [jk1_shift, show 4 + k + 1 = 5 + k from by omega]

def GG : ℕ → Jk1
  | 0 => Jk1.two Jk1.nil (TW 2)
  | (n + 1) => Jk1.one (Jk1.two Jk1.nil (TW 2)) (GG n)

theorem JkA_GG : ∀ n : ℕ, JkA (GG n)
  | 0 => ⟨trivial, JkA_TW 2⟩
  | (n + 1) => ⟨⟨trivial, JkA_TW 2⟩, JkA_GG n⟩

theorem LOk_twoTW2_all (m : ℕ) : LOk (m + 1) (Jk1.two Jk1.nil (TW 2)) :=
  LOk_of_TwOk0 (TTwA_TW2 0 m Jk1.nil trivial NTw_nil (Fter_zero m))

theorem LOk_GG : ∀ (n m : ℕ), LOk (m + 1) (GG n)
  | 0, m => LOk_twoTW2_all m
  | (n + 1), m =>
      LOk_one (W := Jk1.two Jk1.nil (TW 2)) ⟨trivial, JkA_TW 2⟩
        (LOk_twoTW2_all m) (LOk_GG n (m + 1))

theorem flatMap_range_succ' {α : Type} (f : ℕ → List α) (n : ℕ) :
    (List.range (n + 1)).flatMap f = f 0 ++ (List.range n).flatMap (fun k => f (k + 1)) := by
  simp [List.range_succ_eq_map, List.flatMap_map, Function.comp_def]

theorem jk1_GG_succ (n l : ℕ) :
    jk1 l (GG (n + 1))
      = jk1 l (Jk1.two Jk1.nil (TW 2))
        ++ (((l + 1, 1, 0) : ℕ × ℕ × ℕ) :: jk1 (l + 1) (GG n)) := by
  simp only [GG, jk1]

theorem jk1_GG_tail : ∀ (n l : ℕ),
    ((l + 1, 1, 0) : ℕ × ℕ × ℕ) :: jk1 (l + 1) (GG n)
      = (List.range (n + 1)).flatMap (fun k =>
          ((l + k + 1, 1, 0) : ℕ × ℕ × ℕ)
            :: jk1 (l + k + 1) (Jk1.two Jk1.nil (TW 2)))
  | 0, l => by rw [flatMap_range_succ']; simp [GG]
  | (n + 1), l => by
      have hEq : (fun k =>
            ((l + 1 + k + 1, 1, 0) : ℕ × ℕ × ℕ)
              :: jk1 (l + 1 + k + 1) (Jk1.two Jk1.nil (TW 2)))
          = (fun k =>
            ((l + (k + 1) + 1, 1, 0) : ℕ × ℕ × ℕ)
              :: jk1 (l + (k + 1) + 1) (Jk1.two Jk1.nil (TW 2))) := by
        funext k
        rw [show l + 1 + k + 1 = l + (k + 1) + 1 from by omega]
      rw [flatMap_range_succ', jk1_GG_succ, jk1_GG_tail n (l + 1), hEq]
      simp only [Nat.add_zero, List.cons_append]

def HgD (n : ℕ) : Jk1 :=
  Jk1.one Jk1.nil (Jk1.two Jk1.nil (Jk1.one (Jk1.two Jk1.nil Jk1.nil) (GG n)))

theorem GOK_HgD (n : ℕ) : GOK (HgD n) :=
  (APd_bnil _).mp (APd_step [] (JkT_nil : FrmJ [] Jk1.nil) trivial
    ((APd_bnil _).mpr GOK_nil)
    (by
      have hT : TwoOk (Jk1.one (Jk1.two Jk1.nil Jk1.nil) (GG n)) :=
        TwoOk_of_LOk0 (LOk_one (k := 0) ⟨trivial, trivial⟩ (LOk_twoNilAll 0)
          (LOk_GG n 0))
      simpa using hT Jk1.nil trivial (fun _ _ => APd_nil _) 0 []))

theorem Mtw_TailH_gen {A : TrioSeq} (hA : Aok A) :
    ∀ n : ℕ, Mtw (A ++ U375a) TailH n ∈ W 0
  | 0 => by simpa [Mtw] using U375a_mem_gen hA
  | (n + 1) => by
      have hG : GoodFb (fun a b => wordJ a b ([] ++ [HgD n])) :=
        GOK_HgD n [] WOk_nil GoodFb_wordJ_nil
      have hG' : GoodFb (fun a b => wordJ a b [HgD n]) := by simpa using hG
      have h := rowJ_mem_genF hA hG'
      have e : jk1 2 (HgD n)
          = [((3, 1, 0) : ℕ × ℕ × ℕ), ((4, 2, 0) : ℕ × ℕ × ℕ),
              ((5, 2, 0) : ℕ × ℕ × ℕ)]
            ++ (List.range (n + 1)).flatMap (fun k => shiftr01 k 0 TailH) := by
        show jk1 2 Jk1.nil ++ (((3, 1, 0) : ℕ × ℕ × ℕ) ::
          (jk1 3 Jk1.nil ++ (((4, 2, 0) : ℕ × ℕ × ℕ) ::
            (jk1 4 (Jk1.two Jk1.nil Jk1.nil) ++ (((5, 1, 0) : ℕ × ℕ × ℕ)
              :: jk1 5 (GG n)))))) = _
        rw [jk1_twoNil 4]
        have hg := jk1_GG_tail n 4
        simp only [] at hg
        rw [show ((5, 1, 0) : ℕ × ℕ × ℕ) :: jk1 5 (GG n)
            = (List.range (n + 1)).flatMap (fun k =>
                ((4 + k + 1, 1, 0) : ℕ × ℕ × ℕ)
                  :: jk1 (4 + k + 1) (Jk1.two Jk1.nil (TW 2))) from hg]
        rw [show (List.range (n + 1)).flatMap (fun k => shiftr01 k 0 TailH)
            = (List.range (n + 1)).flatMap (fun k =>
                ((4 + k + 1, 1, 0) : ℕ × ℕ × ℕ)
                  :: jk1 (4 + k + 1) (Jk1.two Jk1.nil (TW 2))) from by
          apply List.flatMap_congr; intro k _; exact tailH_shift k]
        simp [jk1]
      rw [wordJ_singleton, colJ, e] at h
      simpa [Mtw, U375a, List.append_assoc] using h

/-- ★★★★★★★★★★★★★★★ `A ++ U375aH ++ (6,2,0)`。 -/
theorem UH_snoc62 {A : TrioSeq} (hA : Aok A) :
    A ++ U375aH ++ [((6, 2, 0) : ℕ × ℕ × ℕ)] ∈ W 0 := by
  have h := snocY_mem (Y0 := A ++ U375a) (M := TailH) (L := 5) (y := 2)
    (by simp [U375a]) MidD_TailH (by simp [TailH, entry]) (by omega)
    (Mtw_TailH_gen hA)
  rw [show (A ++ U375a) ++ TailH = A ++ U375aH from by
    rw [List.append_assoc, Ua_TailH]] at h
  exact h

theorem Z789_H_62 : Z789 ++ U375aH ++ [((6, 2, 0) : ℕ × ℕ × ℕ)] ∈ W 0 :=
  UH_snoc62 Aok_Z789
...
theorem QS_congr (S : Multiset Ld) : ∀ (ks : List ℕ) {V1 V2 : Jk1},
    (∀ l, jk1 l V1 = jk1 l V2) → QS S ks V1 → QS S ks V2 := by
  intro ks V1 V2 h hA
  rw [QS_iff] at hA ⊢
  intro ctx hc
  exact GOK_congr (jk1_plug_congr ctx h) (hA ctx hc)

theorem GOK_chainJdS (S : Multiset Ld) {ks : List ℕ} {ctx : List Frm}
    (hc : QSCtx S ks ctx) {X T : Jk1} (hXok : FrmN ks X) (hXk : QS S ks X)
    (hTok : JkA T)
    (hstep : ∀ V : Jk1, FrmN ks V → QS S ks V → QS S ks (Jk1.one V T)) :
    ∀ n, GOK (plug ctx (itJ T n X)) ∧ QS S ks (itJ T n X)
  | 0 => ⟨(QS_iff S ks X).mp hXk ctx hc, hXk⟩
  | (n + 1) => by
      obtain ⟨h1, h2⟩ := GOK_chainJdS S hc hXok hXk hTok hstep n
      have hok := FrmN_itJ ks hTok n hXok
      have h3 := hstep (itJ T n X) hok h2
      exact ⟨(QS_iff S ks _).mp h3 ctx hc, h3⟩

#print axioms QS_congr
#print axioms GOK_chainJdS

/-- `AYdW` の `QS` 版。荷は層 `S` を変えないので、A2' はそのまま通る。 -/
theorem AYdQ (S : Multiset Ld) : ∀ (Y : TrioSeq), Bok Y → ∀ (ks : List ℕ) (Z : Jk1),
    JkA Z → QS S (0 :: ks) Z →
    ∀ (X : Jk1), FrmN ks X → QS S ks X → QS S ks (Jk1.one X (Jk1.pay Z Y)) := by
  have key : W 0 ⊆ {Y : TrioSeq | Bok Y → ∀ (ks : List ℕ) (Z : Jk1), JkA Z →
    ...
    · rcases (by omega : Y.length = 0 ∨ Y.length = 1) with h0 | h1
      · have hnil0 : Y = [] := List.length_eq_zero_iff.mp h0
        subst hnil0
        exact QS_congr S ks (fun l => (jk1_one_pay_nil X Z l).symm) (QS_step S ks hX hXk hRZ)
      · obtain ⟨c, rfl⟩ := List.length_eq_one_iff.mp h1
        have hc0 : c.1 = 0 := hYb.root
        obtain ⟨hc1, hc2⟩ := hYb.zroot c (by simp) hc0
        have hcz : c = ((0, 0, 0) : ℕ × ℕ × ℕ) := Prod.ext hc0 (Prod.ext hc1 hc2)
        subst hcz
        have hpres : ∀ V : Jk1, FrmN ks V → QS S ks V →
            QS S ks (Jk1.one V (Jk1.pay Z ([] : TrioSeq))) :=
          fun V hV hVk =>
            QS_congr S ks (fun l => (jk1_one_pay_nil V Z l).symm) (QS_step S ks hV hVk hRZ)
        have e : ([((0, 0, 0) : ℕ × ℕ × ℕ)] : TrioSeq)
            = ([] : TrioSeq) ++ [((0, 0, 0) : ℕ × ℕ × ℕ)] := by simp
        rw [e, QS_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJs0 hw
          (QSCtx_JkT S ks ctx hc
            (Jk1.one X (Jk1.pay Z (([] : TrioSeq) ++ [((0, 0, 0) : ℕ × ℕ × ℕ)])))
            (FrmN_one ks X _ hX ⟨hZ, by simpa using hYb⟩))
          (by simpa using hYb) Bok_nil ?_
        intro n hn
        exact (GOK_chainJdS S (T := Jk1.pay Z ([] : TrioSeq)) hc hX hXk ⟨hZ, Bok_nil⟩
          hpres n).1 ws hw hG
    have hlen2 : 2 ≤ Y.length := by omega
    have hYne : Y ≠ [] := by intro hcc; rw [hcc] at hlen2; simp at hlen2
    rcases hY with ⟨hl, -⟩ | hnat | ⟨m, hm, -, -⟩
    ...
    · by_cases hlast : entry Y 0 (Y.length - 1) = 0
      · obtain ⟨he1, he2⟩ := Zroot_entry hYb.zroot hlast
        ...
        have hdb : Bok Y.dropLast := Bok_dropLast hYb
        have hpres : ∀ V : Jk1, FrmN ks V → QS S ks V →
            QS S ks (Jk1.one V (Jk1.pay Z Y.dropLast)) :=
          fun V hV hVk => hdl hdb ks Z hZ hRZ V hV hVk
        rw [hsplit, QS_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJs0 hw
          ...
          (by rw [← hsplit]; exact hYb) hdb ?_
        intro n hn
        exact (GOK_chainJdS S (T := Jk1.pay Z Y.dropLast) hc hX hXk ⟨hZ, hdb⟩
          hpres n).1 ws hw hG
      · have hnz : ¬ (entry Y 0 (Y.length - 1) = 0 ∧ entry Y 1 (Y.length - 1) = 0 ∧
            entry Y 2 (Y.length - 1) = 0) := fun h => hlast h.1
        have hp := hasParent_of_ZrootMono hYb.zroot hYb.mono hYb.root hlen2 hnz
        ...
        intro ctx hc ws hw hG
        refine GoodFb_snoc_innerJs0 hw
          (QSCtx_JkT S ks ctx hc (Jk1.one X (Jk1.pay Z Y))
            (FrmN_one ks X _ hX ⟨hZ, hYb⟩))
          hYb hlen2 hp ?_
        intro n hn
        have hh := hnat n hn
        simp only [Set.mem_setOf_eq] at hh
        exact (QS_iff S ks _).mp (hh (Bok_oper hYb hn) ks Z hZ hRZ X hX hXk) ctx hc ws hw hG
    · exact absurd hm (Nat.not_lt_zero m)
  intro Y hYb ks Z hZ hRZ X hX hXk
  exact key hYb.mem hYb ks Z hZ hRZ X hX hXk
...
/-- 枠 1 枚につき予算 1 個。`⊥` が 1 の枠、`i ≠ ⊥` が 2 の枠。
兄弟は「同じ位置（`p' = []`）またはそれより深い位置」に置ける。
`WPdT` と同じく**荷閉包は条件に入れない**（荷は `WNd_payA` で無条件に出る）。 -/
def WNd {Bud : Type} [PartialOrder Bud] [OrderBot Bud] [WellFoundedLT Bud] :
    List Bud → Jk1 → Prop
  | [], V => GOK V
  | (i :: ks), V =>
      (i = ⊥ → ∀ U : Jk1, FrmNT ks U → WNd ks U → WNd ks (Jk1.one U V)) ∧
      (i ≠ ⊥ → ∀ (p : List Bud), p ≠ [] → (∀ x ∈ p, x < i) →
        ∀ N : Jk1, JkA N →
        (∀ (p' : List Bud), (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) N) →
...
def WNtx {Bud : Type} [PartialOrder Bud] [OrderBot Bud] [WellFoundedLT Bud] :
    List Bud → List Frm → Prop
  | [], ctx => ctx = []
  | (i :: ks), ctx =>
      (i = ⊥ → ∃ (ctx' : List Frm) (U : Jk1), ctx = ctx' ++ [Frm.fone U] ∧
        WNtx ks ctx' ∧ FrmNT ks U ∧ WNd ks U) ∧
      (i ≠ ⊥ → ∃ (p : List Bud) (_ : p ≠ []) (_ : ∀ x ∈ p, x < i)
        (ctx' : List Frm) (N : Jk1),
        ctx = ctx' ++ [Frm.ftwo N] ∧ WNtx (p ++ ks) ctx' ∧ JkA N ∧
        (∀ (p' : List Bud), (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) N))
termination_by ks _ => ((ks : List Bud) : Multiset Bud)
decreasing_by
  all_goals
...
theorem WNtx_bnil (ctx : List Frm) : WNtx ([] : List Bud) ctx ↔ ctx = [] := by rw [WNtx]

theorem WNtx_c0 (ks : List Bud) (ctx : List Frm) :
    WNtx ((⊥ : Bud) :: ks) ctx ↔ ∃ (ctx' : List Frm) (U : Jk1),
      ctx = ctx' ++ [Frm.fone U] ∧ WNtx ks ctx' ∧ FrmNT ks U ∧ WNd ks U := by
  rw [WNtx]
  constructor
  · exact fun h => h.1 rfl
  · exact fun h => ⟨fun _ => h, fun hne => absurd rfl hne⟩

theorem WNtx_cb {i : Bud} (hi : i ≠ ⊥) (ks : List Bud) (ctx : List Frm) :
    WNtx (i :: ks) ctx ↔ ∃ (p : List Bud) (_ : p ≠ []) (_ : ∀ x ∈ p, x < i)
      (ctx' : List Frm) (N : Jk1),
      ctx = ctx' ++ [Frm.ftwo N] ∧ WNtx (p ++ ks) ctx' ∧ JkA N ∧
      (∀ (p' : List Bud), (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) N) := by
  rw [WNtx]
  constructor
  · exact fun h => h.2 hi
  · exact fun h => ⟨fun he => absurd he hi, fun _ => h⟩

theorem FrmNT_ne (a : Bud) (l : List Bud) {X : Jk1} (h : JkA X) : FrmNT (a :: l) X := h

theorem FrmNT_app_ne {p ks : List Bud} (hp : p ≠ []) {X : Jk1} (h : JkA X) :
    FrmNT (p ++ ks) X := by
  obtain ⟨a, p₀, rfl⟩ : ∃ a p₀, p = a :: p₀ := by
    cases p with
    | nil => exact absurd rfl hp
    | cons a p₀ => exact ⟨a, p₀, rfl⟩
  exact h

theorem WNtx_JkT : ∀ (ks : List Bud) (ctx : List Frm), WNtx ks ctx → ∀ X : Jk1,
    FrmNT ks X → JkT (plug ctx X)
  | [], ctx, h, X, hX => by
      rw [WNtx_bnil] at h; subst h; exact hX
  | (i :: ks), ctx, h, X, hX => by
      by_cases hi : i = ⊥
      · subst hi
        rw [WNtx_c0] at h
        obtain ⟨ctx', U, rfl, hc', hU, -⟩ := h
        rw [plug_snoc]
        exact WNtx_JkT ks ctx' hc' (Jk1.one U X) (FrmNT_one ks U X hU hX)
      · rw [WNtx_cb hi] at h
        obtain ⟨p, hpne, hp, ctx', N, rfl, hc', hJN, -⟩ := h
        rw [plug_snoc2]
        exact WNtx_JkT (p ++ ks) ctx' hc' (Jk1.two N X)
          (FrmNT_app_ne hpne ⟨hJN, FrmNT_JkA (i :: ks) X hX⟩)
termination_by ks _ => ((ks : List Bud) : Multiset Bud)
decreasing_by
  all_goals
    first
...
theorem WNd_iff : ∀ (ks : List Bud) (V : Jk1),
    WNd ks V ↔ ∀ ctx : List Frm, WNtx ks ctx → GOK (plug ctx V)
  | [], V => by
      rw [WNd_bnil]
      constructor
      · intro h ctx hc
        rw [WNtx_bnil] at hc; subst hc; exact h
      · intro h
        exact h [] ((WNtx_bnil []).mpr rfl)
  | (i :: ks), V => by
      by_cases hi : i = ⊥
      · subst hi
        rw [WNd_c0]
        ...
        · intro h ctx hc
          ...
          obtain ⟨ctx', U, rfl, hc', hU, hUk⟩ := hc
          rw [plug_snoc]
          exact (WNd_iff ks (Jk1.one U V)).mp (h U hU hUk) ctx' hc'
        · intro h U hU hUk
          refine (WNd_iff ks (Jk1.one U V)).mpr ?_
          intro ctx' hc'
          rw [← plug_snoc]
          exact h (ctx' ++ [Frm.fone U])
            ((WNtx_c0 ks _).mpr ⟨ctx', U, rfl, hc', hU, hUk⟩)
      · rw [WNd_ck hi]
        constructor
        · intro h ctx hc
          rw [WNtx_cb hi] at hc
          obtain ⟨p, hpne, hp, ctx', N, rfl, hc', hJN, hNt⟩ := hc
          rw [plug_snoc2]
          exact (WNd_iff (p ++ ks) _).mp (h p hpne hp N hJN hNt) ctx' hc'
        · intro h p hpne hp N hJN hNt
          refine (WNd_iff (p ++ ks) _).mpr ?_
          intro ctx' hc'
          rw [← plug_snoc2]
          exact h (ctx' ++ [Frm.ftwo N])
            ((WNtx_cb hi ks _).mpr ⟨p, hpne, hp, ctx', N, rfl, hc', hJN, hNt⟩)
termination_by ks _ => ((ks : List Bud) : Multiset Bud)
decreasing_by
  all_goals
    first
...
#print axioms WNtx_JkT
#print axioms WNd_iff

theorem WNd_step (ks : List Bud) {V W : Jk1} (hV : FrmNT ks V) (hVk : WNd ks V)
    (hW : WNd ((⊥ : Bud) :: ks) W) : WNd ks (Jk1.one V W) :=
  (WNd_c0 ks W).mp hW V hV hVk

/-- ブロックの 2 の枠。走りの長さは予算リストの長さに現れるので上限が無い。
新しい枠の予算 `i` は `i' < i` を満たす限り**いくらでも大きく取れる**。 -/
theorem WNd_twoOf {i i' : Bud} (hi : i ≠ ⊥) (hlt : i' < i) {ks : List Bud}
    {V N : Jk1} (hJN : JkA N)
    (hNt : ∀ p' : List Bud, (∀ x ∈ p', x < i) → WNd (p' ++ (i' :: ks)) N)
    (hV : WNd (i :: ks) V) : WNd (i' :: ks) (Jk1.two N V) := by
  have h := (WNd_ck hi ks V).mp hV [i'] (by simp)
    (by intro x hx; simp at hx; subst hx; exact hlt) N hJN
    (by simpa using hNt)
  simpa using h

theorem WNd_congr : ∀ (ks : List Bud) {V1 V2 : Jk1}, (∀ l, jk1 l V1 = jk1 l V2) →
    WNd ks V1 → WNd ks V2 := by
  intro ks V1 V2 h hA
  rw [WNd_iff] at hA ⊢
  intro ctx hc
  exact GOK_congr (jk1_plug_congr ctx h) (hA ctx hc)

theorem WNtx_split (ks : List Bud) (ctx : List Frm)
    (h : WNtx ((⊥ : Bud) :: ks) ctx) :
    ∃ (ctx0 : List Frm) (V : Jk1), ctx = ctx0 ++ [Frm.fone V] ∧ WNtx ks ctx0 ∧
      FrmNT ks V ∧ GOK (plug ctx0 V) := by
  rw [WNtx_c0] at h
  obtain ⟨ctx', U, rfl, hc', hU, hUk⟩ := h
  exact ⟨ctx', U, rfl, hc', hU, (WNd_iff ks U).mp hUk ctx' hc'⟩

theorem WNd_nilE : WNd ([] : List Bud) Jk1.nil := (WNd_bnil _).mpr GOK_nil

/-! ### `WNd` の荷は無条件（`WPdT` の荷の開発をそのまま移す） -/

theorem WNd_payE (V : Jk1) (hV : JkT V) (hVk : WNd ([] : List Bud) V)
    (C : TrioSeq) (hC : Bok C) : WNd ([] : List Bud) (Jk1.pay V C) :=
  (WNd_bnil _).mpr (AY0 C hC V hV ((WNd_bnil V).mp hVk))

theorem GOK_chainJdWN {ks : List Bud} {ctx : List Frm} (hc : WNtx ks ctx) {X T : Jk1}
    (hXok : FrmNT ks X) (hXk : WNd ks X) (hTok : JkA T)
    (hstep : ∀ V : Jk1, FrmNT ks V → WNd ks V → WNd ks (Jk1.one V T)) :
    ∀ n, GOK (plug ctx (itJ T n X)) ∧ WNd ks (itJ T n X)
  | 0 => ⟨(WNd_iff ks X).mp hXk ctx hc, hXk⟩
  | (n + 1) => by
      obtain ⟨h1, h2⟩ := GOK_chainJdWN hc hXok hXk hTok hstep n
      have hok := FrmNT_itJ ks hTok n hXok
      have h3 := hstep (itJ T n X) hok h2
      exact ⟨(WNd_iff ks _).mp h3 ctx hc, h3⟩

/-- 予算 `⊥` の荷（1 の枠の直下）。 -/
theorem AYdWN : ∀ (Y : TrioSeq), Bok Y → ∀ (ks : List Bud) (Z : Jk1), JkA Z →
    WNd ((⊥ : Bud) :: ks) Z →
    ∀ (X : Jk1), FrmNT ks X → WNd ks X → WNd ks (Jk1.one X (Jk1.pay Z Y)) := by
  have key : W 0 ⊆ {Y : TrioSeq | Bok Y → ∀ (ks : List Bud) (Z : Jk1), JkA Z →
      WNd ((⊥ : Bud) :: ks) Z →
      ∀ (X : Jk1), FrmNT ks X → WNd ks X → WNd ks (Jk1.one X (Jk1.pay Z Y))} := by
    refine A2' ?_
    ...
    · rcases (by omega : Y.length = 0 ∨ Y.length = 1) with h0 | h1
      · have hnil0 : Y = [] := List.length_eq_zero_iff.mp h0
        subst hnil0
        exact WNd_congr ks (fun l => (jk1_one_pay_nil X Z l).symm) (WNd_step ks hX hXk hRZ)
      · obtain ⟨c, rfl⟩ := List.length_eq_one_iff.mp h1
        have hc0 : c.1 = 0 := hYb.root
        obtain ⟨hc1, hc2⟩ := hYb.zroot c (by simp) hc0
        have hcz : c = ((0, 0, 0) : ℕ × ℕ × ℕ) := Prod.ext hc0 (Prod.ext hc1 hc2)
        subst hcz
        have hpres : ∀ V : Jk1, FrmNT ks V → WNd ks V →
            WNd ks (Jk1.one V (Jk1.pay Z ([] : TrioSeq))) :=
          fun V hV hVk =>
            WNd_congr ks (fun l => (jk1_one_pay_nil V Z l).symm) (WNd_step ks hV hVk hRZ)
        have e : ([((0, 0, 0) : ℕ × ℕ × ℕ)] : TrioSeq)
            = ([] : TrioSeq) ++ [((0, 0, 0) : ℕ × ℕ × ℕ)] := by simp
        rw [e, WNd_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJs0 hw
          ...
          (by simpa using hYb) Bok_nil ?_
        intro n hn
        exact (GOK_chainJdWN (T := Jk1.pay Z ([] : TrioSeq)) hc hX hXk ⟨hZ, Bok_nil⟩
          hpres n).1 ws hw hG
    have hlen2 : 2 ≤ Y.length := by omega
    have hYne : Y ≠ [] := by intro hcc; rw [hcc] at hlen2; simp at hlen2
    rcases hY with ⟨hl, -⟩ | hnat | ⟨m, hm, -, -⟩
    ...
    · by_cases hlast : entry Y 0 (Y.length - 1) = 0
      · obtain ⟨he1, he2⟩ := Zroot_entry hYb.zroot hlast
        ...
        have hdb : Bok Y.dropLast := Bok_dropLast hYb
        have hpres : ∀ V : Jk1, FrmNT ks V → WNd ks V →
            WNd ks (Jk1.one V (Jk1.pay Z Y.dropLast)) :=
          fun V hV hVk => hdl hdb ks Z hZ hRZ V hV hVk
        rw [hsplit, WNd_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJs0 hw
          ...
          (by rw [← hsplit]; exact hYb) hdb ?_
        intro n hn
        exact (GOK_chainJdWN (T := Jk1.pay Z Y.dropLast) hc hX hXk ⟨hZ, hdb⟩
          hpres n).1 ws hw hG
      · have hnz : ¬ (entry Y 0 (Y.length - 1) = 0 ∧ entry Y 1 (Y.length - 1) = 0 ∧
            entry Y 2 (Y.length - 1) = 0) := fun h => hlast h.1
        have hp := hasParent_of_ZrootMono hYb.zroot hYb.mono hYb.root hlen2 hnz
        ...
        intro ctx hc ws hw hG
        refine GoodFb_snoc_innerJs0 hw
          (WNtx_JkT ks ctx hc (Jk1.one X (Jk1.pay Z Y)) (FrmNT_one ks X _ hX ⟨hZ, hYb⟩))
          hYb hlen2 hp ?_
        intro n hn
        have hh := hnat n hn
        simp only [Set.mem_setOf_eq] at hh
        exact (WNd_iff ks _).mp (hh (Bok_oper hYb hn) ks Z hZ hRZ X hX hXk) ctx hc ws hw hG
    · exact absurd hm (Nat.not_lt_zero m)
  intro Y hYb ks Z hZ hRZ X hX hXk
  exact key hYb.mem hYb ks Z hZ hRZ X hX hXk

theorem WNd_payT (ks : List Bud) (V : Jk1) (hV : JkA V)
    (hVk : WNd ((⊥ : Bud) :: ks) V) (C : TrioSeq) (hC : Bok C) :
    WNd ((⊥ : Bud) :: ks) (Jk1.pay V C) :=
  (WNd_c0 ks _).mpr (fun U hU hUk => AYdWN C hC ks V hV hVk U hU hUk)

#print axioms AYdWN

/-- 水平鎖。予算 `i` を使い回し、尻尾だけ伸びる（`WPdT_chainT` と同じ）。 -/
theorem WNd_chainT {i : Bud} {p ks : List Bud} {ctx : List Frm}
    (hc : WNtx (p ++ ks) ctx) {N T : Jk1} (hJN : JkA N)
    (hNall : ∀ p' : List Bud, (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) N)
    (hT : JkA T)
    (hstep : ∀ N' : Jk1, JkA N' →
      (∀ p' : List Bud, (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) N') →
      ∀ p' : List Bud, (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) (Jk1.two N' T)) :
    ∀ n, GOK (plug ctx (twoIt N T n)) ∧ JkA (twoIt N T n) ∧
      (∀ p' : List Bud, (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) (twoIt N T n))
  | 0 => ⟨(WNd_iff (p ++ ks) N).mp (by simpa using hNall [] (by simp)) ctx hc, hJN, hNall⟩
  | (n + 1) => by
      obtain ⟨-, h2, h3⟩ := WNd_chainT hc hJN hNall hT hstep n
      have h4 := hstep (twoIt N T n) h2 h3
      exact ⟨(WNd_iff (p ++ ks) _).mp (by simpa using h4 [] (by simp)) ctx hc,
        ⟨h2, hT⟩, h4⟩

/-- 鎖の 1 段。予算 `i` は動かず、尻尾に `p'` が積まれるだけ。 -/
theorem AYdTWN_hstep {i : Bud} (hi : i ≠ ⊥) {ks p : List Bud} (hpne : p ≠ [])
    (hp : ∀ x ∈ p, x < i) {T : Jk1} (hTk : WNd (i :: ks) T) :
    ∀ N' : Jk1, JkA N' →
      (∀ p' : List Bud, (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) N') →
      ∀ p' : List Bud, (∀ x ∈ p', x < i) → WNd (p' ++ (p ++ ks)) (Jk1.two N' T) := by
  intro N' hN' hN'all p' hp'
  have hall : ∀ x ∈ p' ++ p, x < i := by
    intro x hx
    rcases List.mem_append.mp hx with h1 | h1
    · exact hp' x h1
    · exact hp x h1
  have h := (WNd_ck hi ks T).mp hTk (p' ++ p)
    (by intro hcc; exact hpne (List.append_eq_nil_iff.mp hcc).2) hall N' hN' ?_
  · rw [List.append_assoc] at h
    exact h
  · intro p'' hp''
    have h2 := hN'all (p'' ++ p') (by
      intro x hx
      rcases List.mem_append.mp hx with h1 | h1
      · exact hp'' x h1
      · exact hp' x h1)
...
/-- 予算 `i ≠ ⊥` の荷（2 の枠の直下）。予算は動かない。 -/
theorem AYdTWN : ∀ (Y : TrioSeq), Bok Y → ∀ (i : Bud), i ≠ ⊥ →
    ∀ (ks : List Bud) (Z : Jk1), JkA Z →
    WNd (i :: ks) Z → WNd (i :: ks) (Jk1.pay Z Y) := by
  have key : W 0 ⊆ {Y : TrioSeq | Bok Y → ∀ (i : Bud), i ≠ ⊥ →
      ∀ (ks : List Bud) (Z : Jk1), JkA Z →
      WNd (i :: ks) Z → WNd (i :: ks) (Jk1.pay Z Y)} := by
    refine A2' ?_
    intro Y hY
    simp only [Set.mem_setOf_eq]
    intro hYb i hi ks Z hZ hZk
    by_cases hshort : Y.length ≤ 1
    · rcases (by omega : Y.length = 0 ∨ Y.length = 1) with h0 | h1
      · have hnil0 : Y = [] := List.length_eq_zero_iff.mp h0
        subst hnil0
        exact WNd_congr (i :: ks) (fun l => (jk1_pay_nil l Z).symm) hZk
      · obtain ⟨c, rfl⟩ := List.length_eq_one_iff.mp h1
        have hc0 : c.1 = 0 := hYb.root
        obtain ⟨hc1, hc2⟩ := hYb.zroot c (by simp) hc0
        have hcz : c = ((0, 0, 0) : ℕ × ℕ × ℕ) := Prod.ext hc0 (Prod.ext hc1 hc2)
        subst hcz
        have hZnil : WNd (i :: ks) (Jk1.pay Z ([] : TrioSeq)) :=
          WNd_congr (i :: ks) (fun l => (jk1_pay_nil l Z).symm) hZk
        have e : ([((0, 0, 0) : ℕ × ℕ × ℕ)] : TrioSeq)
            = ([] : TrioSeq) ++ [((0, 0, 0) : ℕ × ℕ × ℕ)] := by simp
        rw [e, WNd_ck hi]
        intro p hpne hp N hJN hNt
        rw [WNd_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJt0 hw
          (WNtx_JkT (p ++ ks) ctx hc
            (Jk1.two N (Jk1.pay Z (([] : TrioSeq) ++ [((0, 0, 0) : ℕ × ℕ × ℕ)])))
            (FrmNT_app_ne hpne ⟨hJN, hZ, by simpa using hYb⟩)) ?_
        intro n hn
        exact (WNd_chainT (i := i) (T := Jk1.pay Z ([] : TrioSeq)) hc hJN hNt
          ⟨hZ, Bok_nil⟩ (AYdTWN_hstep hi hpne hp hZnil) n).1 ws hw hG
    have hlen2 : 2 ≤ Y.length := by omega
    have hYne : Y ≠ [] := by intro hcc; rw [hcc] at hlen2; simp at hlen2
    rcases hY with ⟨hl, -⟩ | hnat | ⟨mm, hm, -, -⟩
    · exact absurd hl hshort
    · by_cases hlast : entry Y 0 (Y.length - 1) = 0
      · obtain ⟨he1, he2⟩ := Zroot_entry hYb.zroot hlast
        ...
        simp only [Set.mem_setOf_eq] at hdl
        have hdb : Bok Y.dropLast := Bok_dropLast hYb
        have hprev : WNd (i :: ks) (Jk1.pay Z Y.dropLast) := hdl hdb i hi ks Z hZ hZk
        rw [hsplit, WNd_ck hi]
        intro p hpne hp N hJN hNt
        rw [WNd_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJt0 hw
          (WNtx_JkT (p ++ ks) ctx hc
            (Jk1.two N (Jk1.pay Z (Y.dropLast ++ [((0, 0, 0) : ℕ × ℕ × ℕ)])))
            (FrmNT_app_ne hpne ⟨hJN, hZ, by rw [← hsplit]; exact hYb⟩)) ?_
        intro n hn
        exact (WNd_chainT (i := i) (T := Jk1.pay Z Y.dropLast) hc hJN hNt
          ⟨hZ, hdb⟩ (AYdTWN_hstep hi hpne hp hprev) n).1 ws hw hG
      · have hnz : ¬ (entry Y 0 (Y.length - 1) = 0 ∧ entry Y 1 (Y.length - 1) = 0 ∧
            entry Y 2 (Y.length - 1) = 0) := fun h => hlast h.1
        ...
        intro ctx hc ws hw hG
        refine GoodFb_snoc_innerJt0 hw
          (WNtx_JkT (p ++ ks) ctx hc (Jk1.two N (Jk1.pay Z Y))
            (FrmNT_app_ne hpne ⟨hJN, hZ, hYb⟩))
          hlen2 hp2 ?_
        intro n hn
        have hh := hnat n hn
        simp only [Set.mem_setOf_eq] at hh
        have hh2 := hh (Bok_oper hYb hn) i hi ks Z hZ hZk
        exact (WNd_iff (p ++ ks) _).mp ((WNd_ck hi ks _).mp hh2 p hpne hp N hJN hNt)
          ctx hc ws hw hG
    · exact absurd hm (Nat.not_lt_zero mm)
  intro Y hYb i hi ks Z hZ hZk
  exact key hYb.mem hYb i hi ks Z hZ hZk

/-- ★★★★★★ `WNd` 層の荷（どの予算でも、無条件）。 -/
theorem WNd_payA : ∀ (ks : List Bud) (V : Jk1), FrmNT ks V → WNd ks V →
    ∀ C : TrioSeq, Bok C → WNd ks (Jk1.pay V C)
  | [], V, hV, hVk, C, hC => WNd_payE V hV hVk C hC
  | (i :: ks), V, hV, hVk, C, hC => by
      by_cases hi : i = ⊥
      · subst hi
        exact WNd_payT ks V (FrmNT_JkA _ V hV) hVk C hC
      · exact AYdTWN C hC i hi ks V (FrmNT_JkA _ V hV) hVk

#print axioms AYdTWN
#print axioms WNd_payA

/-- 枠木の荷閉包が無条件に出たので `one V nil` も無条件。 -/
theorem WNd_oneNil (ks : List Bud) (V : Jk1) (hV : FrmNT ks V) (hVk : WNd ks V) :
    WNd ks (Jk1.one V Jk1.nil) := by
  rw [WNd_iff]
  intro ctx hc
  refine APnil_gen0 ctx V
    (WNtx_JkT ks ctx hc (Jk1.one V Jk1.nil) (FrmNT_one ks V Jk1.nil hV trivial))
    ((WNd_iff ks V).mp hVk ctx hc) ?_
  intro C hC
  exact (WNd_iff ks _).mp (WNd_payA ks V hV hVk C hC) ctx hc

theorem WNd_nilT (ks : List Bud) : WNd ((⊥ : Bud) :: ks) Jk1.nil :=
  (WNd_c0 ks _).mpr (fun U hU hUk => WNd_oneNil ks U hU hUk)

#print axioms WNd_nilT

#print axioms WNd_twoOf
...
/-! ### ★★★★★★★★★★★★ `WQd` を一般の予算型に持ち上げる（`WQt`）

`SmallA` の `WQd` は節の入り目 `k+1` が「幅 `Ns.length ≤ k+1` のブロック
`RunP Ns V`」を一度に張る。幅が予算で縛られているのが `Pay2` で外したのと同じ形。
予算型を一般にして `Bml = Multiset Ld` を入れれば幅の上限が消える。
幅は `Scale` で測る（`S.nb Ns.length < b`）。 -/

section BudQ

variable {Bud : Type} [PartialOrder Bud] [OrderBot Bud] [WellFoundedLT Bud]

def WQt {Bud : Type} [PartialOrder Bud] [OrderBot Bud] [WellFoundedLT Bud]
    (S : Scale Bud) : List Bud → Jk1 → Prop
  | [], V => GOK V
  | (b :: ks), V =>
      (b = ⊥ → ∀ U : Jk1, FrmNT ks U → WQt S ks U →
        (∀ C : TrioSeq, Bok C → WQt S ks (Jk1.pay U C)) →
        WQt S ks (Jk1.one U V)) ∧
      (b ≠ ⊥ → ∀ (r : List Bud), (∀ x ∈ r, x < b) →
        ∀ (U : Jk1) (Ns : List Jk1), Ns ≠ [] → S.nb Ns.length < b →
        FrmNT (r ++ ks) U → WQt S (r ++ ks) U →
        (∀ C : TrioSeq, Bok C → WQt S (r ++ ks) (Jk1.pay U C)) →
        (∀ N ∈ Ns, JkA N) →
        (∀ N ∈ Ns, ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
          WQt S (q ++ (r ++ ks)) N) →
        (∀ N ∈ Ns, ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
          ∀ C : TrioSeq, Bok C → WQt S (q ++ (r ++ ks)) (Jk1.pay N C)) →
        WQt S (r ++ ks) (Jk1.one U (RunP Ns V)))
termination_by ks _ => ((ks : List Bud) : Multiset Bud)
decreasing_by
  all_goals
    first
...
theorem WQt_cons (S : Scale Bud) (b : Bud) (ks : List Bud) (V : Jk1) :
    WQt S (b :: ks) V ↔
      ((b = ⊥ → ∀ U : Jk1, FrmNT ks U → WQt S ks U →
        (∀ C : TrioSeq, Bok C → WQt S ks (Jk1.pay U C)) →
        WQt S ks (Jk1.one U V)) ∧
      (b ≠ ⊥ → ∀ (r : List Bud), (∀ x ∈ r, x < b) →
        ...
        (∀ N ∈ Ns, ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
          ∀ C : TrioSeq, Bok C → WQt S (q ++ (r ++ ks)) (Jk1.pay N C)) →
        WQt S (r ++ ks) (Jk1.one U (RunP Ns V)))) := by
  rw [WQt]

theorem WQt_bnil (S : Scale Bud) (V : Jk1) : WQt S ([] : List Bud) V ↔ GOK V := by
  rw [WQt]

theorem WQt_c0 (S : Scale Bud) (ks : List Bud) (V : Jk1) :
    WQt S ((⊥ : Bud) :: ks) V ↔ ∀ U : Jk1, FrmNT ks U → WQt S ks U →
      (∀ C : TrioSeq, Bok C → WQt S ks (Jk1.pay U C)) →
      WQt S ks (Jk1.one U V) := by
  rw [WQt_cons]
  constructor
  · exact fun h => h.1 rfl
  · exact fun h => ⟨fun _ => h, fun hne => absurd rfl hne⟩

theorem WQt_ck (S : Scale Bud) {b : Bud} (hb : b ≠ ⊥) (ks : List Bud) (V : Jk1) :
    WQt S (b :: ks) V ↔ ∀ (r : List Bud), (∀ x ∈ r, x < b) →
      ∀ (U : Jk1) (Ns : List Jk1), Ns ≠ [] → S.nb Ns.length < b →
      FrmNT (r ++ ks) U → WQt S (r ++ ks) U →
      (∀ C : TrioSeq, Bok C → WQt S (r ++ ks) (Jk1.pay U C)) →
      (∀ N ∈ Ns, JkA N) →
...
/-- `WQt` の文脈族。 -/
def WQxt {Bud : Type} [PartialOrder Bud] [OrderBot Bud] [WellFoundedLT Bud]
    (S : Scale Bud) : List Bud → List Frm → Prop
  | [], ctx => ctx = []
  | (b :: ks), ctx =>
      (b = ⊥ → ∃ (ctx' : List Frm) (U : Jk1), ctx = ctx' ++ [Frm.fone U] ∧
        WQxt S ks ctx' ∧ FrmNT ks U ∧ WQt S ks U ∧
        (∀ C : TrioSeq, Bok C → WQt S ks (Jk1.pay U C))) ∧
      (b ≠ ⊥ → ∃ (r : List Bud) (_ : ∀ x ∈ r, x < b) (ctx' : List Frm)
        (U : Jk1) (Ns : List Jk1),
        Ns ≠ [] ∧ S.nb Ns.length < b ∧
        ctx = ctx' ++ ([Frm.fone U] ++ Ns.map Frm.ftwo) ∧
        WQxt S (r ++ ks) ctx' ∧ FrmNT (r ++ ks) U ∧ WQt S (r ++ ks) U ∧
        (∀ C : TrioSeq, Bok C → WQt S (r ++ ks) (Jk1.pay U C)) ∧
        (∀ N ∈ Ns, JkA N) ∧
        (∀ N ∈ Ns, ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
          WQt S (q ++ (r ++ ks)) N) ∧
...
theorem WQxt_bnil (S : Scale Bud) (ctx : List Frm) :
    WQxt S ([] : List Bud) ctx ↔ ctx = [] := by rw [WQxt]

theorem WQxt_c0 (S : Scale Bud) (ks : List Bud) (ctx : List Frm) :
    WQxt S ((⊥ : Bud) :: ks) ctx ↔ ∃ (ctx' : List Frm) (U : Jk1),
      ctx = ctx' ++ [Frm.fone U] ∧ WQxt S ks ctx' ∧ FrmNT ks U ∧ WQt S ks U ∧
      (∀ C : TrioSeq, Bok C → WQt S ks (Jk1.pay U C)) := by
  rw [WQxt]
  constructor
  · exact fun h => h.1 rfl
  · exact fun h => ⟨fun _ => h, fun hne => absurd rfl hne⟩

theorem WQxt_cb (S : Scale Bud) {b : Bud} (hb : b ≠ ⊥) (ks : List Bud) (ctx : List Frm) :
    WQxt S (b :: ks) ctx ↔ ∃ (r : List Bud) (_ : ∀ x ∈ r, x < b) (ctx' : List Frm)
      (U : Jk1) (Ns : List Jk1),
      Ns ≠ [] ∧ S.nb Ns.length < b ∧
      ctx = ctx' ++ ([Frm.fone U] ++ Ns.map Frm.ftwo) ∧
      WQxt S (r ++ ks) ctx' ∧ FrmNT (r ++ ks) U ∧ WQt S (r ++ ks) U ∧
      ...
      (∀ N ∈ Ns, ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
        WQt S (q ++ (r ++ ks)) N) ∧
      (∀ N ∈ Ns, ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
        ∀ C : TrioSeq, Bok C → WQt S (q ++ (r ++ ks)) (Jk1.pay N C)) := by
  rw [WQxt]
  constructor
  · exact fun h => h.2 hb
  · exact fun h => ⟨fun he => absurd he hb, fun _ => h⟩

theorem WQxt_JkT (S : Scale Bud) : ∀ (ks : List Bud) (ctx : List Frm),
    WQxt S ks ctx → ∀ X : Jk1, FrmNT ks X → JkT (plug ctx X)
  | [], ctx, h, X, hX => by
      rw [WQxt_bnil] at h; subst h; exact hX
  | (b :: ks), ctx, h, X, hX => by
      by_cases hb : b = ⊥
      · subst hb
        rw [WQxt_c0] at h
        obtain ⟨ctx', U, rfl, hc', hU, -, -⟩ := h
        rw [plug_snoc]
        exact WQxt_JkT S ks ctx' hc' (Jk1.one U X) (FrmNT_one ks U X hU hX)
      · rw [WQxt_cb S hb] at h
        obtain ⟨r, hr, ctx', U, Ns, -, -, rfl, hc', hU, -, -, hJNs, -, -⟩ := h
        rw [plug_blk]
        exact WQxt_JkT S (r ++ ks) ctx' hc' (Jk1.one U (RunP Ns X))
          (FrmNT_one _ U (RunP Ns X) hU (JkA_RunP Ns hJNs hX))
termination_by ks _ => ((ks : List Bud) : Multiset Bud)
decreasing_by
  all_goals
    first
...
theorem WQt_iff (S : Scale Bud) : ∀ (ks : List Bud) (V : Jk1),
    WQt S ks V ↔ ∀ ctx : List Frm, WQxt S ks ctx → GOK (plug ctx V)
  | [], V => by
      rw [WQt_bnil]
      constructor
      · intro h ctx hc
        rw [WQxt_bnil] at hc; subst hc; exact h
      · intro h
        exact h [] ((WQxt_bnil S []).mpr rfl)
  | (b :: ks), V => by
      by_cases hb : b = ⊥
      · subst hb
        rw [WQt_c0]
        ...
        · intro h ctx hc
          ...
          rw [plug_snoc]
          exact (WQt_iff S ks (Jk1.one U V)).mp (h U hU hUk hUp) ctx' hc'
        · intro h U hU hUk hUp
          refine (WQt_iff S ks (Jk1.one U V)).mpr ?_
          intro ctx' hc'
          rw [← plug_snoc]
          exact h (ctx' ++ [Frm.fone U])
            ((WQxt_c0 S ks _).mpr ⟨ctx', U, rfl, hc', hU, hUk, hUp⟩)
      · rw [WQt_ck S hb]
        constructor
        · intro h ctx hc
          rw [WQxt_cb S hb] at hc
          obtain ⟨r, hr, ctx', U, Ns, hNe, hNl, rfl, hc', hU, hUk, hUp, hJNs, hNs, hNp⟩ := hc
          rw [plug_blk]
          exact (WQt_iff S (r ++ ks) _).mp
            (h r hr U Ns hNe hNl hU hUk hUp hJNs hNs hNp) ctx' hc'
        · intro h r hr U Ns hNe hNl hU hUk hUp hJNs hNs hNp
          refine (WQt_iff S (r ++ ks) _).mpr ?_
          intro ctx' hc'
          rw [← plug_blk]
          exact h (ctx' ++ ([Frm.fone U] ++ Ns.map Frm.ftwo))
            ((WQxt_cb S hb ks _).mpr ⟨r, hr, ctx', U, Ns, hNe, hNl, rfl, hc', hU, hUk, hUp,
              hJNs, hNs, hNp⟩)
termination_by ks _ => ((ks : List Bud) : Multiset Bud)
decreasing_by
  all_goals
...
theorem WQt_step (S : Scale Bud) (ks : List Bud) {V W : Jk1} (hV : FrmNT ks V)
    (hVk : WQt S ks V) (hVp : ∀ C : TrioSeq, Bok C → WQt S ks (Jk1.pay V C))
    (hW : WQt S ((⊥ : Bud) :: ks) W) : WQt S ks (Jk1.one V W) :=
  (WQt_c0 S ks W).mp hW V hV hVk hVp

theorem WQt_congr (S : Scale Bud) : ∀ (ks : List Bud) {V1 V2 : Jk1},
    (∀ l, jk1 l V1 = jk1 l V2) → WQt S ks V1 → WQt S ks V2 := by
  intro ks V1 V2 h hA
  rw [WQt_iff] at hA ⊢
  intro ctx hc
  exact GOK_congr (jk1_plug_congr ctx h) (hA ctx hc)
...
#print axioms WQt_iff

section BudQ3

variable {Bud : Type} [PartialOrder Bud] [OrderBot Bud] [WellFoundedLT Bud]

/-- ブロックの階段（一般予算）。`j` は「`Bs` を差すのに要る入り目」。 -/
theorem WQt_runStair (S : Scale Bud) {b : Bud} {Bs : List Jk1} {Bl : Jk1} (hJBl : JkA Bl)
    (j : Bud) (hj : j < b)
    (hjrun : ∀ (B'' : List Bud) (X : Jk1),
      (∀ N ∈ Bs, ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) → WQt S (q ++ B'') N) →
      (∀ N ∈ Bs, ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
        ∀ C : TrioSeq, Bok C → WQt S (q ++ B'') (Jk1.pay N C)) →
      WQt S (j :: B'') X → WQt S ((⊥ : Bud) :: B'') (RunP Bs X)) :
    ∀ (n : ℕ) (B'' : List Bud),
      (∀ N ∈ Bs ++ [Bl], ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
        WQt S (q ++ B'') N) →
      (∀ N ∈ Bs ++ [Bl], ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
        ∀ C : TrioSeq, Bok C → WQt S (q ++ B'') (Jk1.pay N C)) →
      WQt S ((⊥ : Bud) :: B'') (RunP Bs (appJ Bl (UtwP Bs Bl n)))
  | 0, B'', hs, hp => by
  ...
  | (n + 1), B'', hs, hp => by
      refine hjrun B'' _ (fun N hN => hs N (List.mem_append_left _ hN))
        (fun N hN => hp N (List.mem_append_left _ hN)) ?_
      show WQt S (j :: B'') (Jk1.one Bl (RunP Bs (appJ Bl (UtwP Bs Bl n))))
      refine WQt_step S (j :: B'') (hJBl : FrmNT (j :: B'') Bl) ?_ ?_ ?_
      · have h := hs Bl (List.mem_append_right _ (by simp)) [j] (by simp) (by simpa using hj)
        simpa using h
      · intro C hC
        ...
        simpa using h
      · refine WQt_runStair S hJBl j hj hjrun n (j :: B'') ?_ ?_
        · intro N hN q hq hqk
...
/-- ★★★★★★ `nil` はどの予算の枠でも差せる（一般予算、幅の上限なし）。 -/
theorem WQt_nilF (S : Scale Bud) {b : Bud} (hb : b ≠ ⊥) (ks : List Bud) :
    WQt S (b :: ks) Jk1.nil := by
  rw [WQt_ck S hb]
  ...
  have key : ∀ (B'' : List Bud),
      ...
      (∀ N ∈ Bs ++ [Bl], ∀ q : List Bud, q ≠ [] → (∀ x ∈ q, x < b) →
        ∀ C : TrioSeq, Bok C → WQt S (q ++ B'') (Jk1.pay N C)) →
      ∀ n : ℕ, WQt S ((⊥ : Bud) :: B'') (RunP Bs (appJ Bl (UtwP Bs Bl n))) := by
    cases Bs with
    | nil =>
        intro B'' hs hp n
        exact WQt_runStair S (Bs := []) hJBl (⊥ : Bud) hbot (fun _ _ _ _ h => h) n B'' hs hp
    | cons C Cs =>
        intro B'' hs hp n
        have hjb : S.nb (Cs.length + 1 + 1) < b := by
          rw [hlen] at hNl
          simpa using hNl
        have hjne : S.nb (Cs.length + 1 + 1) ≠ (⊥ : Bud) := Scale.nb_ne_bot S (Cs.length + 1)
        refine WQt_runStair S hJBl (S.nb (Cs.length + 1 + 1)) hjb ?_ n B'' hs hp
        intro B3 X hsib hsibp hX
        rw [WQt_c0]
        intro U3 hU3 hU3k hU3p
        have h2 := (WQt_ck S hjne B3 X).mp hX [] (by simp) U3 (C :: Cs) (by simp) ?_
          (by simpa using hU3) (by simpa using hU3k) (by simpa using hU3p) ?_ ?_ ?_
        · simpa using h2
        · show S.nb (C :: Cs).length < S.nb (Cs.length + 1 + 1)
        ...
        · intro N hN q hq hqk C0 hC0
          have := hsibp N hN q hq (fun x hx => lt_trans (hqk x hx) hjb) C0 hC0
          simpa using this
  refine (WQt_c0 S B _).mp ?_ U hU hUk hUp
  rw [WQt_iff]
  intro ctx hc
  obtain ⟨ctx0, V, rfl, hc0, hV, hVk, hVp⟩ := (WQxt_c0 S B ctx).mp hc
  have hJT : JkT (plug (ctx0 ++ [Frm.fone V]) (RunP (Bs ++ [Bl]) Jk1.nil)) :=
    WQxt_JkT S ((⊥ : Bud) :: B) _ hc (RunP (Bs ++ [Bl]) Jk1.nil)
      (JkA_RunP (Bs ++ [Bl]) hJNs trivial : FrmNT ((⊥ : Bud) :: B) (RunP (Bs ++ [Bl]) Jk1.nil))
  have hGV : GOK (plug ctx0 V) := (WQt_iff S B V).mp hVk ctx0 hc0
  rw [plug_snoc] at hJT ⊢
  ...
  | succ n =>
      ...
      rw [← plug_snoc]
      exact (WQt_iff S ((⊥ : Bud) :: B) _).mp (key B hNs hNp n) _ hc

theorem WQt_oneNil (S : Scale Bud) (ks : List Bud) (V : Jk1) (hV : FrmNT ks V)
    (hVk : WQt S ks V) (hVp : ∀ C : TrioSeq, Bok C → WQt S ks (Jk1.pay V C)) :
    WQt S ks (Jk1.one V Jk1.nil) := by
  rw [WQt_iff]
  intro ctx hc
  refine APnil_gen0 ctx V
    (WQxt_JkT S ks ctx hc (Jk1.one V Jk1.nil) (FrmNT_one ks V Jk1.nil hV trivial))
    ((WQt_iff S ks V).mp hVk ctx hc) ?_
  intro C hC
  exact (WQt_iff S ks _).mp (hVp C hC) ctx hc

theorem WQt_nilT (S : Scale Bud) (ks : List Bud) : WQt S ((⊥ : Bud) :: ks) Jk1.nil :=
  (WQt_c0 S ks _).mpr (fun U hU hUk hUp => WQt_oneNil S ks U hU hUk hUp)

/-- ★★★★★★ 空木はどの形でも差せる（`WQt` 層、無条件）。 -/
theorem WQt_nilAll (S : Scale Bud) : ∀ ks : List Bud, WQt S ks Jk1.nil
  | [] => (WQt_bnil S _).mpr GOK_nil
  | (b :: ks) => by
      by_cases hb : b = ⊥
      · subst hb; exact WQt_nilT S ks
      · exact WQt_nilF S hb ks

end BudQ3

#print axioms WQt_nilAll

section BudQ4

variable {Bud : Type} [PartialOrder Bud] [OrderBot Bud] [WellFoundedLT Bud]

/-- `nil` に荷を吊るせれば、走りのブロックが差せる（一般予算）。 -/
theorem WQt_stk_succ (S : Scale Bud)
    (hpay : ∀ (ks : List Bud) (C : TrioSeq), Bok C → WQt S ks (Jk1.pay Jk1.nil C))
    (p : ℕ) (ks : List Bud) (Y : Jk1) (hY : WQt S (S.nb (p + 1 + 1) :: ks) Y) :
    WQt S ((⊥ : Bud) :: ks) (stkP (p + 1) Y) := by
  rw [WQt_c0]
  intro U hU hUk hUp
  rw [← RunP_rep_nil (p + 1) Y]
  have h := (WQt_ck S (Scale.nb_ne_bot S (p + 1)) ks Y).mp hY [] (by simp) U
    (List.replicate (p + 1) Jk1.nil) (by simp) ?_
    (by simpa using hU) (by simpa using hUk) (by simpa using hUp)
    (fun N hN => by rw [List.eq_of_mem_replicate hN]; trivial)
    (fun N hN q hq hqk => by
      rw [List.eq_of_mem_replicate hN]
      exact WQt_nilAll S _)
    (fun N hN q hq hqk C hC => by
      rw [List.eq_of_mem_replicate hN]
      exact hpay _ C hC)
  · simpa using h
  · show S.nb (List.replicate (p + 1) Jk1.nil).length < S.nb (p + 1 + 1)
    rw [List.length_replicate]
    exact S.nbmono (by omega)

theorem WQt_Utw (S : Scale Bud)
    (hpay : ∀ (ks : List Bud) (C : TrioSeq), Bok C → WQt S ks (Jk1.pay Jk1.nil C)) :
    ∀ (i p : ℕ) (ks : List Bud), WQt S ks (Utw p i)
  | 0, _, ks => WQt_nilAll S ks
  | (i + 1), 0, ks => by
      refine WQt_step S ks (FrmNT_nilA ks) (WQt_nilAll S ks) (fun C hC => hpay ks C hC) ?_
      exact WQt_Utw S hpay i 0 ((⊥ : Bud) :: ks)
  | (i + 1), (p + 1), ks => by
      refine WQt_step S ks (FrmNT_nilA ks) (WQt_nilAll S ks) (fun C hC => hpay ks C hC) ?_
      exact WQt_stk_succ S hpay p ks (Utw (p + 1) i)
        (WQt_Utw S hpay i (p + 1) (S.nb (p + 1 + 1) :: ks))

/-- ★★★★★★ 行376 は「`nil` に荷を吊るす」1 本から出る（`WQt` 版、一般予算）。 -/
theorem R376_of_QtPnil (S : Scale Bud)
    (hpay : ∀ (ks : List Bud) (C : TrioSeq), Bok C → WQt S ks (Jk1.pay Jk1.nil C)) :
    R373 ++ [((5, 3, 0) : ℕ × ℕ × ℕ)] ∈ W 0 :=
  R376_of_UtwAll (fun p n => (WQt_bnil S _).mp (WQt_Utw S hpay n p []))

end BudQ4

#print axioms R376_of_QtPnil

/-! ### ★★★★★★★★★★★★★★ 深さだけの予算で荷閉包を持たない族 `WEd`

`WBd`（予算 `List (ℕ × ℕ)`、順序は深さ `encE (m,i) = i` だけ）の `m` は順序に
現れないので落とし、予算を**深さのリスト `List ℕ`**にする。枠 1 枚が予算 1 個で、
`0` が 1 の枠、`i+1` が 2 の枠。2 の枠は深さを 1 下げ、詰め物 `q`（深さ `≤ i`、
長さ無制限）を足す。これで

- 走りの階段: 兄弟 `B`（深さ `d`）の条件が「深さ `≤ d` の詰め物を何枚でも」なので、
  ブロックを内側に足したときの詰め物（そのブロックの深さ `< d` の枠）が入る。
- 荷の複製鎖: 鎖の各段は同じ深さ `i` に並ぶので深さが動かない。
  `(i+1) :: ks` で開き直すだけで済む（`AYdTWE_hstep`）。

の両方が通る。`WBd` で壁だったのは**族の定義に荷閉包が入っていたこと**だけ。
`WPdT` / `WNd` と同じく荷閉包を外し、荷は `WEd_payA` で無条件に出す。 -/

def WEd : List ℕ → Jk1 → Prop
  | [], V => GOK V
  | (0 :: ks), V => ∀ U : Jk1, FrmN ks U → WEd ks U → WEd ks (Jk1.one U V)
  | ((i + 1) :: ks), V => ∀ (q : List ℕ), (∀ x ∈ q, x ≤ i) → ∀ N : Jk1, JkA N →
      (∀ (q' : List ℕ), (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) N) →
      WEd (i :: (q ++ ks)) (Jk1.two N V)
termination_by ks _ => (ks : Multiset ℕ)
decreasing_by
  all_goals
    first
      | exact dm_cons0 ks
      | (rw [show i :: (q ++ ks) = (i :: q) ++ ks from rfl]
         exact dm_app ks (i :: q)
           (by
             intro x hx
             rcases List.mem_cons.mp hx with h0 | h0
             · omega
             · exact (by assumption : ∀ x ∈ q, x ≤ i) x h0))
      | (rw [show i :: (q' ++ (q ++ ks)) = (i :: (q' ++ q)) ++ ks from by simp]
         exact dm_app ks (i :: (q' ++ q))
           (by
             intro x hx
             rcases List.mem_cons.mp hx with h0 | h0
             · omega
             · rcases List.mem_append.mp h0 with h1 | h1
               · exact (by assumption : ∀ x ∈ q', x ≤ i) x h1
               · exact (by assumption : ∀ x ∈ q, x ≤ i) x h1))

theorem WEd_bnil (V : Jk1) : WEd [] V ↔ GOK V := by rw [WEd]

theorem WEd_c0 (ks : List ℕ) (V : Jk1) :
    WEd (0 :: ks) V ↔ ∀ U : Jk1, FrmN ks U → WEd ks U → WEd ks (Jk1.one U V) := by
  rw [WEd]

theorem WEd_ck (i : ℕ) (ks : List ℕ) (V : Jk1) :
    WEd ((i + 1) :: ks) V ↔ ∀ (q : List ℕ), (∀ x ∈ q, x ≤ i) → ∀ N : Jk1, JkA N →
      (∀ (q' : List ℕ), (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) N) →
      WEd (i :: (q ++ ks)) (Jk1.two N V) := by
  rw [WEd]

def WEtx : List ℕ → List Frm → Prop
  | [], ctx => ctx = []
  | (0 :: ks), ctx => ∃ (ctx' : List Frm) (U : Jk1), ctx = ctx' ++ [Frm.fone U] ∧
      WEtx ks ctx' ∧ FrmN ks U ∧ WEd ks U
  | ((i + 1) :: ks), ctx => ∃ (q : List ℕ) (_ : ∀ x ∈ q, x ≤ i) (ctx' : List Frm) (N : Jk1),
      ctx = ctx' ++ [Frm.ftwo N] ∧ WEtx (i :: (q ++ ks)) ctx' ∧ JkA N ∧
      (∀ (q' : List ℕ), (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) N)
termination_by ks _ => (ks : Multiset ℕ)
decreasing_by
  all_goals
...
theorem WEtx_bnil (ctx : List Frm) : WEtx [] ctx ↔ ctx = [] := by rw [WEtx]

theorem WEtx_c0 (ks : List ℕ) (ctx : List Frm) :
    WEtx (0 :: ks) ctx ↔ ∃ (ctx' : List Frm) (U : Jk1),
      ctx = ctx' ++ [Frm.fone U] ∧ WEtx ks ctx' ∧ FrmN ks U ∧ WEd ks U := by
  rw [WEtx]

theorem WEtx_ck (i : ℕ) (ks : List ℕ) (ctx : List Frm) :
    WEtx ((i + 1) :: ks) ctx ↔ ∃ (q : List ℕ) (_ : ∀ x ∈ q, x ≤ i)
      (ctx' : List Frm) (N : Jk1),
      ctx = ctx' ++ [Frm.ftwo N] ∧ WEtx (i :: (q ++ ks)) ctx' ∧ JkA N ∧
      (∀ (q' : List ℕ), (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) N) := by
  rw [WEtx]

theorem WEtx_JkT : ∀ (ks : List ℕ) (ctx : List Frm), WEtx ks ctx → ∀ X : Jk1,
    FrmN ks X → JkT (plug ctx X)
  | [], ctx, h, X, hX => by
      rw [WEtx_bnil] at h; subst h; exact hX
  | (0 :: ks), ctx, h, X, hX => by
      rw [WEtx_c0] at h
      obtain ⟨ctx', U, rfl, hc', hU, -⟩ := h
      rw [plug_snoc]
      exact WEtx_JkT ks ctx' hc' (Jk1.one U X) (FrmN_one ks U X hU hX)
  | ((i + 1) :: ks), ctx, h, X, hX => by
      rw [WEtx_ck] at h
      obtain ⟨q, hq, ctx', N, rfl, hc', hJN, -⟩ := h
      rw [plug_snoc2]
      exact WEtx_JkT (i :: (q ++ ks)) ctx' hc' (Jk1.two N X)
        (⟨hJN, FrmN_JkA _ X hX⟩ : FrmN (i :: (q ++ ks)) (Jk1.two N X))
termination_by ks _ => (ks : Multiset ℕ)
decreasing_by
  all_goals
    first
...
theorem WEd_iff : ∀ (ks : List ℕ) (V : Jk1),
    WEd ks V ↔ ∀ ctx : List Frm, WEtx ks ctx → GOK (plug ctx V)
  | [], V => by
      rw [WEd_bnil]
      constructor
      · intro h ctx hc
        rw [WEtx_bnil] at hc; subst hc; exact h
      · intro h
        exact h [] ((WEtx_bnil []).mpr rfl)
  | (0 :: ks), V => by
      rw [WEd_c0]
      constructor
      · intro h ctx hc
        ...
        obtain ⟨ctx', U, rfl, hc', hU, hUk⟩ := hc
        rw [plug_snoc]
        exact (WEd_iff ks (Jk1.one U V)).mp (h U hU hUk) ctx' hc'
      · intro h U hU hUk
        refine (WEd_iff ks (Jk1.one U V)).mpr ?_
        intro ctx' hc'
        rw [← plug_snoc]
        exact h (ctx' ++ [Frm.fone U]) ((WEtx_c0 ks _).mpr ⟨ctx', U, rfl, hc', hU, hUk⟩)
  | ((i + 1) :: ks), V => by
      rw [WEd_ck]
      constructor
      · intro h ctx hc
        ...
        rw [plug_snoc2]
        exact (WEd_iff (i :: (q ++ ks)) _).mp (h q hq N hJN hNt) ctx' hc'
      · intro h q hq N hJN hNt
        refine (WEd_iff (i :: (q ++ ks)) _).mpr ?_
        intro ctx' hc'
        rw [← plug_snoc2]
        exact h (ctx' ++ [Frm.ftwo N])
          ((WEtx_ck i ks _).mpr ⟨q, hq, ctx', N, rfl, hc', hJN, hNt⟩)
termination_by ks _ => (ks : Multiset ℕ)
decreasing_by
  all_goals
    first
...
#print axioms WEtx_JkT
#print axioms WEd_iff

theorem WEd_step (ks : List ℕ) {V W : Jk1} (hV : FrmN ks V) (hVk : WEd ks V)
    (hW : WEd (0 :: ks) W) : WEd ks (Jk1.one V W) :=
  (WEd_c0 ks W).mp hW V hV hVk

theorem WEd_congr : ∀ (ks : List ℕ) {V1 V2 : Jk1}, (∀ l, jk1 l V1 = jk1 l V2) →
    WEd ks V1 → WEd ks V2 := by
  intro ks V1 V2 h hA
  rw [WEd_iff] at hA ⊢
  intro ctx hc
  exact GOK_congr (jk1_plug_congr ctx h) (hA ctx hc)
...
theorem WEd_payE (V : Jk1) (hV : JkT V) (hVk : WEd [] V) (C : TrioSeq) (hC : Bok C) :
    WEd [] (Jk1.pay V C) :=
  (WEd_bnil _).mpr (AY0 C hC V hV ((WEd_bnil V).mp hVk))

theorem GOK_chainJdWE {ks : List ℕ} {ctx : List Frm} (hc : WEtx ks ctx) {X T : Jk1}
    (hXok : FrmN ks X) (hXk : WEd ks X) (hTok : JkA T)
    (hstep : ∀ V : Jk1, FrmN ks V → WEd ks V → WEd ks (Jk1.one V T)) :
    ∀ n, GOK (plug ctx (itJ T n X)) ∧ WEd ks (itJ T n X)
  | 0 => ⟨(WEd_iff ks X).mp hXk ctx hc, hXk⟩
  | (n + 1) => by
      obtain ⟨h1, h2⟩ := GOK_chainJdWE hc hXok hXk hTok hstep n
      have hok := FrmN_itJ ks hTok n hXok
      have h3 := hstep (itJ T n X) hok h2
      exact ⟨(WEd_iff ks _).mp h3 ctx hc, h3⟩

theorem AYdWE : ∀ (Y : TrioSeq), Bok Y → ∀ (ks : List ℕ) (Z : Jk1), JkA Z →
    WEd (0 :: ks) Z →
    ∀ (X : Jk1), FrmN ks X → WEd ks X → WEd ks (Jk1.one X (Jk1.pay Z Y)) := by
  have key : W 0 ⊆ {Y : TrioSeq | Bok Y → ∀ (ks : List ℕ) (Z : Jk1), JkA Z →
      WEd (0 :: ks) Z →
      ∀ (X : Jk1), FrmN ks X → WEd ks X → WEd ks (Jk1.one X (Jk1.pay Z Y))} := by
    refine A2' ?_
    ...
    · rcases (by omega : Y.length = 0 ∨ Y.length = 1) with h0 | h1
      · have hnil0 : Y = [] := List.length_eq_zero_iff.mp h0
        subst hnil0
        exact WEd_congr ks (fun l => (jk1_one_pay_nil X Z l).symm) (WEd_step ks hX hXk hRZ)
      · obtain ⟨c, rfl⟩ := List.length_eq_one_iff.mp h1
        have hc0 : c.1 = 0 := hYb.root
        obtain ⟨hc1, hc2⟩ := hYb.zroot c (by simp) hc0
        have hcz : c = ((0, 0, 0) : ℕ × ℕ × ℕ) := Prod.ext hc0 (Prod.ext hc1 hc2)
        subst hcz
        have hpres : ∀ V : Jk1, FrmN ks V → WEd ks V →
            WEd ks (Jk1.one V (Jk1.pay Z ([] : TrioSeq))) :=
          fun V hV hVk =>
            WEd_congr ks (fun l => (jk1_one_pay_nil V Z l).symm) (WEd_step ks hV hVk hRZ)
        have e : ([((0, 0, 0) : ℕ × ℕ × ℕ)] : TrioSeq)
            = ([] : TrioSeq) ++ [((0, 0, 0) : ℕ × ℕ × ℕ)] := by simp
        rw [e, WEd_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJs0 hw
          ...
          (by simpa using hYb) Bok_nil ?_
        intro n hn
        exact (GOK_chainJdWE (T := Jk1.pay Z ([] : TrioSeq)) hc hX hXk ⟨hZ, Bok_nil⟩
          hpres n).1 ws hw hG
    have hlen2 : 2 ≤ Y.length := by omega
    have hYne : Y ≠ [] := by intro hcc; rw [hcc] at hlen2; simp at hlen2
    rcases hY with ⟨hl, -⟩ | hnat | ⟨m, hm, -, -⟩
    ...
    · by_cases hlast : entry Y 0 (Y.length - 1) = 0
      · obtain ⟨he1, he2⟩ := Zroot_entry hYb.zroot hlast
        ...
        have hdb : Bok Y.dropLast := Bok_dropLast hYb
        have hpres : ∀ V : Jk1, FrmN ks V → WEd ks V →
            WEd ks (Jk1.one V (Jk1.pay Z Y.dropLast)) :=
          fun V hV hVk => hdl hdb ks Z hZ hRZ V hV hVk
        rw [hsplit, WEd_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJs0 hw
          ...
          (by rw [← hsplit]; exact hYb) hdb ?_
        intro n hn
        exact (GOK_chainJdWE (T := Jk1.pay Z Y.dropLast) hc hX hXk ⟨hZ, hdb⟩
          hpres n).1 ws hw hG
      · have hnz : ¬ (entry Y 0 (Y.length - 1) = 0 ∧ entry Y 1 (Y.length - 1) = 0 ∧
            entry Y 2 (Y.length - 1) = 0) := fun h => hlast h.1
        have hp := hasParent_of_ZrootMono hYb.zroot hYb.mono hYb.root hlen2 hnz
        ...
        intro ctx hc ws hw hG
        refine GoodFb_snoc_innerJs0 hw
          (WEtx_JkT ks ctx hc (Jk1.one X (Jk1.pay Z Y)) (FrmN_one ks X _ hX ⟨hZ, hYb⟩))
          hYb hlen2 hp ?_
        intro n hn
        have hh := hnat n hn
        simp only [Set.mem_setOf_eq] at hh
        exact (WEd_iff ks _).mp (hh (Bok_oper hYb hn) ks Z hZ hRZ X hX hXk) ctx hc ws hw hG
    · exact absurd hm (Nat.not_lt_zero m)
  intro Y hYb ks Z hZ hRZ X hX hXk
  exact key hYb.mem hYb ks Z hZ hRZ X hX hXk

theorem WEd_payT (ks : List ℕ) (V : Jk1) (hV : JkA V) (hVk : WEd (0 :: ks) V)
    (C : TrioSeq) (hC : Bok C) : WEd (0 :: ks) (Jk1.pay V C) :=
  (WEd_c0 ks _).mpr (fun U hU hUk => AYdWE C hC ks V hV hVk U hU hUk)

#print axioms AYdWE

/-- 水平鎖。深さ `i` は動かず、詰め物 `q'` が積まれるだけ。 -/
theorem WEd_chainT {i : ℕ} {q ks : List ℕ} {ctx : List Frm}
    (hc : WEtx (i :: (q ++ ks)) ctx) {N T : Jk1} (hJN : JkA N)
    (hNall : ∀ q' : List ℕ, (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) N)
    (hT : JkA T)
    (hstep : ∀ N' : Jk1, JkA N' →
      (∀ q' : List ℕ, (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) N') →
      ∀ q' : List ℕ, (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) (Jk1.two N' T)) :
    ∀ n, GOK (plug ctx (twoIt N T n)) ∧ JkA (twoIt N T n) ∧
      (∀ q' : List ℕ, (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) (twoIt N T n))
  | 0 => ⟨(WEd_iff (i :: (q ++ ks)) N).mp (by simpa using hNall [] (by simp)) ctx hc,
      hJN, hNall⟩
  | (n + 1) => by
      obtain ⟨-, h2, h3⟩ := WEd_chainT hc hJN hNall hT hstep n
      have h4 := hstep (twoIt N T n) h2 h3
      exact ⟨(WEd_iff (i :: (q ++ ks)) _).mp (by simpa using h4 [] (by simp)) ctx hc,
        ⟨h2, hT⟩, h4⟩

/-- 鎖の 1 段。深さ `i+1` で開き直すだけ（予算の降下が要らない）。 -/
theorem AYdTWE_hstep {i : ℕ} {ks q : List ℕ} (hq : ∀ x ∈ q, x ≤ i) {T : Jk1}
    (hTk : WEd ((i + 1) :: ks) T) :
    ∀ N' : Jk1, JkA N' →
      (∀ q' : List ℕ, (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) N') →
      ∀ q' : List ℕ, (∀ x ∈ q', x ≤ i) → WEd (i :: (q' ++ (q ++ ks))) (Jk1.two N' T) := by
  intro N' hN' hN'all q' hq'
  have hall : ∀ x ∈ q' ++ q, x ≤ i := by
    intro x hx
    rcases List.mem_append.mp hx with h1 | h1
    · exact hq' x h1
    · exact hq x h1
  have h := (WEd_ck i ks T).mp hTk (q' ++ q) hall N' hN' ?_
  · rw [List.append_assoc] at h
    exact h
  · intro q'' hq''
...
theorem AYdTWE : ∀ (Y : TrioSeq), Bok Y → ∀ (i : ℕ) (ks : List ℕ) (Z : Jk1), JkA Z →
    WEd ((i + 1) :: ks) Z → WEd ((i + 1) :: ks) (Jk1.pay Z Y) := by
  have key : W 0 ⊆ {Y : TrioSeq | Bok Y → ∀ (i : ℕ) (ks : List ℕ) (Z : Jk1), JkA Z →
      WEd ((i + 1) :: ks) Z → WEd ((i + 1) :: ks) (Jk1.pay Z Y)} := by
    refine A2' ?_
    intro Y hY
    simp only [Set.mem_setOf_eq]
    intro hYb i ks Z hZ hZk
    by_cases hshort : Y.length ≤ 1
    · rcases (by omega : Y.length = 0 ∨ Y.length = 1) with h0 | h1
      · have hnil0 : Y = [] := List.length_eq_zero_iff.mp h0
        subst hnil0
        exact WEd_congr ((i + 1) :: ks) (fun l => (jk1_pay_nil l Z).symm) hZk
      · obtain ⟨c, rfl⟩ := List.length_eq_one_iff.mp h1
        have hc0 : c.1 = 0 := hYb.root
        obtain ⟨hc1, hc2⟩ := hYb.zroot c (by simp) hc0
        have hcz : c = ((0, 0, 0) : ℕ × ℕ × ℕ) := Prod.ext hc0 (Prod.ext hc1 hc2)
        subst hcz
        have hZnil : WEd ((i + 1) :: ks) (Jk1.pay Z ([] : TrioSeq)) :=
          WEd_congr ((i + 1) :: ks) (fun l => (jk1_pay_nil l Z).symm) hZk
        have e : ([((0, 0, 0) : ℕ × ℕ × ℕ)] : TrioSeq)
            = ([] : TrioSeq) ++ [((0, 0, 0) : ℕ × ℕ × ℕ)] := by simp
        rw [e, WEd_ck]
        ...
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJt0 hw
          (WEtx_JkT (i :: (q ++ ks)) ctx hc
            (Jk1.two N (Jk1.pay Z (([] : TrioSeq) ++ [((0, 0, 0) : ℕ × ℕ × ℕ)])))
            (⟨hJN, hZ, by simpa using hYb⟩ :
              FrmN (i :: (q ++ ks)) (Jk1.two N (Jk1.pay Z _)))) ?_
        intro n hn
        exact (WEd_chainT (T := Jk1.pay Z ([] : TrioSeq)) hc hJN hNt
          ⟨hZ, Bok_nil⟩ (AYdTWE_hstep hq hZnil) n).1 ws hw hG
    have hlen2 : 2 ≤ Y.length := by omega
    have hYne : Y ≠ [] := by intro hcc; rw [hcc] at hlen2; simp at hlen2
    rcases hY with ⟨hl, -⟩ | hnat | ⟨mm, hm, -, -⟩
    · exact absurd hl hshort
    · by_cases hlast : entry Y 0 (Y.length - 1) = 0
      · obtain ⟨he1, he2⟩ := Zroot_entry hYb.zroot hlast
        ...
        simp only [Set.mem_setOf_eq] at hdl
        have hdb : Bok Y.dropLast := Bok_dropLast hYb
        have hprev : WEd ((i + 1) :: ks) (Jk1.pay Z Y.dropLast) :=
          hdl hdb i ks Z hZ hZk
        rw [hsplit, WEd_ck]
        intro q hq N hJN hNt
        rw [WEd_iff]
        intro ctx hc ws hw hG
        refine GoodFb_snoc_dupJt0 hw
          (WEtx_JkT (i :: (q ++ ks)) ctx hc
            (Jk1.two N (Jk1.pay Z (Y.dropLast ++ [((0, 0, 0) : ℕ × ℕ × ℕ)])))
            (⟨hJN, hZ, by rw [← hsplit]; exact hYb⟩ :
              FrmN (i :: (q ++ ks)) (Jk1.two N (Jk1.pay Z _)))) ?_
        intro n hn
        exact (WEd_chainT (T := Jk1.pay Z Y.dropLast) hc hJN hNt
          ⟨hZ, hdb⟩ (AYdTWE_hstep hq hprev) n).1 ws hw hG
      · have hnz : ¬ (entry Y 0 (Y.length - 1) = 0 ∧ entry Y 1 (Y.length - 1) = 0 ∧
            entry Y 2 (Y.length - 1) = 0) := fun h => hlast h.1
        have hp2 := hasParent_of_ZrootMono hYb.zroot hYb.mono hYb.root hlen2 hnz
        rw [WEd_ck]
        ...
        intro ctx hc ws hw hG
        refine GoodFb_snoc_innerJt0 hw
          (WEtx_JkT (i :: (q ++ ks)) ctx hc (Jk1.two N (Jk1.pay Z Y))
            (⟨hJN, hZ, hYb⟩ : FrmN (i :: (q ++ ks)) (Jk1.two N (Jk1.pay Z Y))))
          hlen2 hp2 ?_
        intro n hn
        have hh := hnat n hn
        simp only [Set.mem_setOf_eq] at hh
        have hh2 := hh (Bok_oper hYb hn) i ks Z hZ hZk
        exact (WEd_iff (i :: (q ++ ks)) _).mp ((WEd_ck i ks _).mp hh2 q hq N hJN hNt)
          ctx hc ws hw hG
    · exact absurd hm (Nat.not_lt_zero mm)
  intro Y hYb i ks Z hZ hZk
  exact key hYb.mem hYb i ks Z hZ hZk

/-- ★★★★★★ `WEd` 層の荷（どの深さでも、無条件）。 -/
theorem WEd_payA : ∀ (ks : List ℕ) (V : Jk1), FrmN ks V → WEd ks V →
    ∀ C : TrioSeq, Bok C → WEd ks (Jk1.pay V C)
  | [], V, hV, hVk, C, hC => WEd_payE V hV hVk C hC
  | (0 :: ks), V, hV, hVk, C, hC => WEd_payT ks V (FrmN_JkA _ V hV) hVk C hC
  | ((i + 1) :: ks), V, hV, hVk, C, hC => AYdTWE C hC i ks V (FrmN_JkA _ V hV) hVk

#print axioms AYdTWE
#print axioms WEd_payA

theorem WEd_oneNil (ks : List ℕ) (V : Jk1) (hV : FrmN ks V) (hVk : WEd ks V) :
    WEd ks (Jk1.one V Jk1.nil) := by
  rw [WEd_iff]
  intro ctx hc
  refine APnil_gen0 ctx V
    (WEtx_JkT ks ctx hc (Jk1.one V Jk1.nil) (FrmN_one ks V Jk1.nil hV trivial))
    ((WEd_iff ks V).mp hVk ctx hc) ?_
  intro C hC
  exact (WEd_iff ks _).mp (WEd_payA ks V hV hVk C hC) ctx hc

theorem WEd_nilT (ks : List ℕ) : WEd (0 :: ks) Jk1.nil :=
  (WEd_c0 ks _).mpr (fun U hU hUk => WEd_oneNil ks U hU hUk)
...
/-! ### ★★★ 汎用: 単位 `U` の上に高さ 2 から 2 行の列を継ぐ（`bump_mem2` の単位版）

`A ++ U ++ shiftr01 2 0 C`（`C` は行 2 ≡ 0・Zroot・根が高さ 0）。根の像 `(2,0,0)` の
親は必ず `U` の頭なので段の条件は要らない。非零の末尾は `oper_append_right_of`
（親が末尾側にあれば前置 `A ++ U` は展開に関与しない）と `oper_shiftr01`。 -/

theorem Aok_shift2U {U : TrioSeq} (hMid : MidD 2 U)
    (hU : ∀ A : TrioSeq, Aok A → A ++ U ∈ W 0) :
    ∀ C ∈ W 0, Z2 C → Zroot C → entry C 0 0 = 0 →
  ...
  have hsh2m : ∀ (C : TrioSeq), Z2 C → Mono (shiftr01 2 0 C) := by
    intro C hz c hc
    simp only [shiftr01, List.mem_map] at hc
    obtain ⟨x, hx, rfl⟩ := hc; simp [hz x hx]
  have hcol : ∀ C : TrioSeq, ∀ c ∈ U ++ shiftr01 2 0 C, 1 ≤ c.1 := by
    intro C c hc
    rcases List.mem_append.mp hc with h1 | h1
    · exact hUc c h1
    · have := hsh2c C c h1; omega
  have hmono : ∀ C : TrioSeq, Z2 C → Mono (U ++ shiftr01 2 0 C) := by
    intro C hz c hc
    rcases List.mem_append.mp hc with h1 | h1
    · exact hUm c h1
    · exact hsh2m C hz c h1
  have key : W 0 ⊆ {C : TrioSeq | Z2 C → Zroot C → entry C 0 0 = 0 →
      ∀ A : TrioSeq, Aok A → Aok (A ++ U ++ shiftr01 2 0 C)} := by
    refine A2' ?_
    intro C hC
    simp only [Set.mem_setOf_eq]
    intro hz2 hzr hroot A hA
    have hfin : A ++ U ++ shiftr01 2 0 C ∈ W 0 → Aok (A ++ U ++ shiftr01 2 0 C) := by
      intro h
      rw [List.append_assoc] at h ⊢
      exact Aok_of_mem_col hA (hcol C) (hmono C hz2) h
    have hflat2 : ∀ C' : TrioSeq, Z2 C' →
        (∀ A' : TrioSeq, Aok A' → Aok (A' ++ U ++ shiftr01 2 0 C')) →
        A ++ U ++ shiftr01 2 0 C' ++ [((2, 0, 0) : ℕ × ℕ × ℕ)] ∈ W 0 := by
      intro C' hz' hIH'
      have hM : MidD 2 (U ++ shiftr01 2 0 C') :=
        MidD_append hMid (hsh2c C') (hsh2m C' hz')
    ...
    · by_cases hlast : entry C 0 (C.length - 1) = 0
      · obtain ⟨he1, he2⟩ := Zroot_entry hzr hlast
        ...
        have hdl0 : entry C.dropLast 0 0 = 0 := by
          ...
          exact hroot
        have hz2' : Z2 C.dropLast := fun c hc => hz2 c (List.dropLast_subset _ hc)
        have hIH' : ∀ A' : TrioSeq, Aok A' → Aok (A' ++ U ++ shiftr01 2 0 C.dropLast) :=
          fun A' hA' => hdl hz2' (fun c hc => hzr c (List.dropLast_subset _ hc)) hdl0 A' hA'
        apply hfin
        have hbs : shiftr01 2 0 C = shiftr01 2 0 C.dropLast ++ [((2, 0, 0) : ℕ × ℕ × ℕ)] := by
          conv_lhs => rw [hsplit]
          simp [shiftr01]
        rw [hbs, ← List.append_assoc]
        exact hflat2 C.dropLast hz2' hIH'
      · have hnz : ¬ (entry C 0 (C.length - 1) = 0 ∧ entry C 1 (C.length - 1) = 0 ∧
        ...
        exact (hnat n hn hz2'' hzr'' (by rw [Wset.oper_head_eq hn]; exact hroot) A hA).mem
    · exact absurd hm (Nat.not_lt_zero m)
  intro C hC hz2 hzr hroot A hA
  exact key hC hz2 hzr hroot A hA

theorem Aok_U61_shift2 {C : TrioSeq} (hz2 : Z2 C) (hzr : Zroot C) (hroot : entry C 0 0 = 0)
    {A : TrioSeq} (hA : Aok A) : Aok (A ++ U375a61 ++ shiftr01 2 0 C) :=
  Aok_shift2U MidD_U375a61 (fun A hA => U375a61_mem_gen hA) C (Zroot_mem_W hz2 hzr hroot)
    hz2 hzr hroot A hA

/-- ★ 行 `R375m (6,1,0)(2,0,0)(3,1,0)(4,1,0)`。 -/
theorem R375m61_203141_mem :
    R375m ++ [((6, 1, 0) : ℕ × ℕ × ℕ), ((2, 0, 0) : ℕ × ℕ × ℕ), ((3, 1, 0) : ℕ × ℕ × ℕ),
      ((4, 1, 0) : ℕ × ℕ × ℕ)] ∈ W 0 := by
  have h := (Aok_U61_shift2 (C := [((0, 0, 0) : ℕ × ℕ × ℕ), ((1, 1, 0) : ℕ × ℕ × ℕ),
      ((2, 1, 0) : ℕ × ℕ × ℕ)])
    (by intro c hc; simp only [List.mem_cons, List.not_mem_nil, or_false] at hc
        rcases hc with rfl | rfl | rfl <;> rfl)
    (by intro c hc h0; simp only [List.mem_cons, List.not_mem_nil, or_false] at hc
        rcases hc with rfl | rfl | rfl <;> simp at h0 ⊢)
    (by simp [entry]) Aok_R338).mem
  simpa [shiftr01, U375a61, U375a, R375m, R373, R344, R341, R338, List.append_assoc] using h

/-- ★★ 行 `R375m (6,1,0)(2,0,0)(3,1,0)(4,2,0)(5,3,0)(6,4,0)(7,5,0)`。 -/
theorem R375m61_diag2_mem :
    R375m ++ [((6, 1, 0) : ℕ × ℕ × ℕ), ((2, 0, 0) : ℕ × ℕ × ℕ), ((3, 1, 0) : ℕ × ℕ × ℕ),
      ((4, 2, 0) : ℕ × ℕ × ℕ), ((5, 3, 0) : ℕ × ℕ × ℕ), ((6, 4, 0) : ℕ × ℕ × ℕ),
      ((7, 5, 0) : ℕ × ℕ × ℕ)] ∈ W 0 := by
  have h := (Aok_U61_shift2 (C := [((0, 0, 0) : ℕ × ℕ × ℕ), ((1, 1, 0) : ℕ × ℕ × ℕ),
      ((2, 2, 0) : ℕ × ℕ × ℕ), ((3, 3, 0) : ℕ × ℕ × ℕ), ((4, 4, 0) : ℕ × ℕ × ℕ),
      ((5, 5, 0) : ℕ × ℕ × ℕ)])
    (by intro c hc; simp only [List.mem_cons, List.not_mem_nil, or_false] at hc
        rcases hc with rfl | rfl | rfl | rfl | rfl | rfl <;> rfl)
    (by intro c hc h0; simp only [List.mem_cons, List.not_mem_nil, or_false] at hc
        rcases hc with rfl | rfl | rfl | rfl | rfl | rfl <;> simp at h0 ⊢)
    (by simp [entry]) Aok_R338).mem
  simpa [shiftr01, U375a61, U375a, R375m, R373, R344, R341, R338, List.append_assoc] using h
