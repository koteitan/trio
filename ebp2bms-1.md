...
BM4-Analysis シート（ラベルを `tools/normalize_sheet.py` で形式正規化したもの）と
全数照合済み。
...
加算ユニットの中身（アンカー・根・乗算ユニット）の区切りは例の節の underbrace を参照。
...
加算ユニット 3 つ。ア = アンカー。桁と原始数列埋め込みの組は
さらに underbrace で括って**乗算ユニット**とし、ラベルはその値 $`\omega^{\gamma_{ij}}`$:
...
加算ユニット 1 つ。$`\mathrm{P}(\gamma) = \mathrm{PrSS}(\gamma)`$（原始数列埋め込み）。
乗算ユニット $`\omega^\omega`$ = 桁 + $`\mathrm{P}(\omega)`$:
...
$`\beta = \omega^2 + \omega^2`$ は加算項 2 つなので乗算ユニットが 2 つ。
各乗算ユニットの指数 $`\gamma = 2`$ は $`\mathrm{PrSS}`$ の節 2 つ（兄弟、同じ $`x`$）。
...
乗算ユニット 1 の原始数列埋め込みの入れ子（列は全部 $`y = z = 0`$ なので $`x`$ だけ書く）:
