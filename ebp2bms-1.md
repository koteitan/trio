...
ユニット 1 つの値は、根の $`\omega`$ とサブユニットたちの因子の積になる:
...
サブユニット 1 つが因子 1 つで、その因子の大きさ $`\omega^{\omega^{\gamma_{ij}}}`$ に現れる
$`\gamma_{ij}`$ を埋め込みが担う。例: $`\omega^3`$ は根 $`\omega`$ ＋
$`\gamma = 0`$ のサブユニット 2 つで $`\omega \cdot \omega \cdot \omega`$。
...
**ユニットの個数が加法**（行 1 のレベル階段 1 段ずつ）、
**サブユニットの個数が乗法**（因子 1 つずつ）、
