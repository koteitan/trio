...
| $`\omega^\omega = \psi_0(\omega)`$ | the limit of 1-row matrices (primitive sequences) | $`\begin{pmatrix}0 & 1 & 2\cr 0 & 0 & 0\cr 0 & 0 & 0\end{pmatrix}`$ |
...
| $`\varepsilon_0 = \psi_0(\Omega)`$ | the proof-theoretic ordinal of PA; the first matrix that uses row 1 | $`\begin{pmatrix}0 & 1\cr 0 & 1\cr 0 & 0\end{pmatrix}`$ |
...
| $`\Gamma_0 = \psi_0(\Omega^\Omega)`$ | the Feferman-Schutte ordinal (ATR_0) | $`\begin{pmatrix}0 & 1 & 2 & 3\cr 0 & 1 & 1 & 1\cr 0 & 0 & 0 & 0\end{pmatrix}`$ |
...
| $`\varphi(1,0,0,0) = \psi_0(\Omega^{\Omega^2})`$ | the Ackermann ordinal | $`\begin{pmatrix}0 & 1 & 2 & 3 & 3\cr 0 & 1 & 1 & 1 & 1\cr 0 & 0 & 0 & 0 & 0\end{pmatrix}`$ |
| $`\psi_0(\Omega^{\Omega^\omega})`$ | the small Veblen ordinal (SVO) | $`\begin{pmatrix}0 & 1 & 2 & 3 & 4\cr 0 & 1 & 1 & 1 & 0\cr 0 & 0 & 0 & 0 & 0\end{pmatrix}`$ |
| $`\psi_0(\Omega^{\Omega^\Omega})`$ | the large Veblen ordinal (LVO) | $`\begin{pmatrix}0 & 1 & 2 & 3 & 4\cr 0 & 1 & 1 & 1 & 1\cr 0 & 0 & 0 & 0 & 0\end{pmatrix}`$ |
| $`\psi_0(\Omega_2)`$ | the Bachmann-Howard ordinal (ID_1 / KP) | $`\begin{pmatrix}0 & 1 & 2\cr 0 & 1 & 2\cr 0 & 0 & 0\end{pmatrix}`$ |
| $`\psi_0(\Omega_3)`$ |  | $`\begin{pmatrix}0 & 1 & 2 & 3\cr 0 & 1 & 2 & 3\cr 0 & 0 & 0 & 0\end{pmatrix}`$ |
| $`\psi_0(\Omega_\omega)`$ | **the limit of two rows** (of pair sequences); the first matrix that uses row 2 | $`\begin{pmatrix}0 & 1\cr 0 & 1\cr 0 & 1\end{pmatrix}`$ |
