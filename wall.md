...
### 大小の梯子（`bms -c` 実測、下ほど大きい）
    ...
    R600 (5,2,0)                 ★済（予算 ω）
    R600 (6,0,0)                 ★済（予算 ω²）
    R600 (6,0,0) の輪の族        ★済（台座を差し替えただけ）← いまのシート証明済み
    R600 (7,1,1)                 ← いまの証明中
    RB = R375m ++ R375m↑6
    R375m (6,1,0)
...
### 抜けた壁（2026-09-12）: 予算の型を上に伸ばす

    WPd ((k+1) :: ks) M0t          M0t = two nil (pay nil [(0,0,0)])

原因は「荷の重複鎖が幅 `m` の走りを作るが兄弟の予算が `≤ k`」。
予算の型を整礎全順序 `Bud` に広げ、`ℕ` の段 `nb 0 < nb 1 < ...` の上に `t` を置くと届く。

    WPdT_twoAZ_top (S : Scale Bud) (ht : ∀ m, S.nb m < t) (hJA) (hJZ)
      (hchain : ∀ m c, S.nb m < c → ∀ ks, WPdT (c::ks) (twoIt A Z m)) :
      ∀ ks, WPdT (t::ks) (two A (pay Z [(0,0,0)]))            ★緑

    A = Z = nil          → M0t が ω に        → R600 (5,2,0)
    A = FLz k, Z = nil   → FLz (k+1) が ω·(k+1) に
    A = nil, Z = M1t     → M0u が ω² に       → R600 (6,0,0)

予算型の在庫（`inferInstance` 実測）: `ℕ`(ω) / `ℕ ×ₗ ℕ`(ω²) / `ℕ ×ₗ (ℕ ×ₗ ℕ)`(ω³) /
`Colex (ℕ →₀ ℕ)`(ω^ω、`import Mathlib.Data.Finsupp.WellFounded` が要る)。
`Lex (ℕ →₀ ℕ)` は整礎でない。

### 予算の道はここで止まる（追記382）

この手が効くのは**荷が `[(0,0,0)]` だけ**のとき（鎖の上が `pay Z []` で語が `Z` と同じ）。
一般の荷 `C` だと鎖が `twoIt A (pay Z C') m` になり、1 段ごとに ω 消費、
段数の上限は `W 0` の階数 ＝ 行列に順序数を割り当てることそのもの。避けたい方向。
...
`SCtx` は 2 の枠（兄弟 `nil`）を続けて積める文脈族なので、**縦の走り**を表せる。
`SHtow` は「2 の記録の兄弟に、荷 `V` の水平鎖 `VCh V N` を置ける」。
`V = nil` の場合は `SHtow_nil`（ただし `SNil` を仮定）。

同値・同根（全部 1 点）:

    SNil : ∀ ks, SG ks nil          「空木はどの `SCtx` 形にも差せる」
    SNilT / SPayF / SNilF
    RNil : ∀ D, RCx D → GOK (plug D nil)      （`RCx` 版。`RNil ≒ SNilT`）
    RHang2 / StkBlk2 / RunBdA / RunNilR
...
`WPdT` の文脈 `WCtxU (b::ks)` は `ctx' ++ [fone U, ftwo N]`。
つまり `ftwo` の間に必ず `fone` が入るので、**`ftwo` が 2 つ続く文脈を表せない**。
縦の走りはまさにそれ。予算を伸ばしても直らない。
`SCtx` の `false` の節（`D' ++ [ftwo nil]`）はそれを表せる。
次の一手は `SCtx` 側で `SHtow` をやるか、`WPdT` の文脈に走りの長さを持たせるか。
