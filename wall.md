...
### 大小の梯子（`bms -c` 実測、下ほど大きい）
    ...
    R600 (5,1,0) の輪の族        ← 旧・証明済みの上限
    R600 (5,2,0)                 ★済（予算 ω で抜けた）
    R600 (5,2,0) の輪の族        ★済（台座を差し替えただけ）
    R600 (6,0,0)
    R600 (7,1,1)
    RB = R375m ++ R375m↑6
    R375m (6,1,0)
    R375m (6,2,0) = R373 (5,2,0)(6,2,0)   ← いまの証明中（= `stk 3`）
...
**原因**: 荷の重複鎖が幅 `m` の横の走りを作るが、兄弟の予算が `≤ k` に
抑えられていて `m > k` に届かない。予算の上限は DM 測度の整礎性のために要る。

**直し方**: 予算の型を `ℕ` から整礎全順序 `Bud` に広げ、`ℕ` の段
`nb 0 < nb 1 < ...` の**上**に予算 `t` を置く。`t` の節の兄弟は全部の `nb m` で
来るので、どの幅にも届く。`Bud` が整礎なら DM 測度も整礎のまま。
実体化は `Bud2 = ℕ ×ₗ ℕ`（順序型 ω²）、`nb m = (0,m)`、`t = (1,0) = ω`。

    WPdT_M0t_top   : ∀ks, WPdT ((1,0)::ks) M0t                 ★緑
    WPdT2_twoM0nil : ∀ks, WPdT ((1,1)::ks) (two M0t nil)       ★緑
    R600520_mem    : R600 ++ [(5,2,0)] ∈ W 0                   ★緑
...
「縦の走り `stk j` の上に荷を吊る」。目標行376 はここ 1 本から出る
（`R376_of_RHang2`）。`C = []` の底は済（`RHang2_nil`）。

同値・同根の言い方:

    RNil     : ∀ D, RCx D → GOK (plug D nil)     （`RNil_of_RHang2`）
    StkBlk2  : ∀k ks, 2 ≤ k → WPd ((k+1)::ks) (stk 2)
    RunBdA / RunNilR / RHang

走りの長さの帰納法（`GOK_stk_step` / `GOK_stk_RCx`）は `RCx` 文脈で緑なので、
残りは底の `RNil` だけ。

### なぜ止まるか

`WPdT` 層の文脈 `WCtxU` は `[fone U, ftwo N]` の繰り返しなので、
**`ftwo` が 2 つ続く文脈を表せない**。縦の走り（`stk 2` を 2 の枠の直上に置く）は
まさにそれ。予算を上に伸ばす手は「兄弟の幅」には効いたが、
「文脈に走りが要る」形にはそのままでは効かない。
