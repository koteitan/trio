...
### 続き127 追記4: ★★★ 実測。**木の 2 の記録の junk は高々 1 列**

4 個の種（`D_3` / `D_4` / #17 / `(0,0,0)(1,1,1)(2,2,1)(3,2,0)`）から
`bms` の展開を 6 段まで回して標準形 **2017 個**のコーパスを作り、数えた
（展開は標準形を保つ。先頭 50 個を `bms -s` で確認して全部 1）。

```
(x,1,0) の連続長ごとの個数:              {1: 832, 2: 295, 3: 46, 4: 9}
そのうち直後が (x,2,0)（木の 2 の記録）:  {1: 764}      ← 長さ 2,3,4 は **0 件**
```

**読み方:** 1 の記録の連なりは長さ 4 まで実在する。しかしその直後が
**木の 2 の記録 `(x,2,0)` である場合は、連なりの長さが必ず 1**。
つまり `jk1` の木で言えば
```
two N M の N（2 の記録の junk）は、実際の標準形では高々 1 段。
...
「4 段の junk を持つ 2 の記録の入れ子」まで `GOK` を要求してしまう。
...
1. コーパスを広げて（種を増やして）上の 0 件が保たれるか確認する。
2. 保たれるなら、`APd` の `(k+1)` の場合の `N` に
   「junk は高々 1 段」相当の条件を入れて測る。
   停止性は `N` に条件が増えるだけなので `msr` は変わらない。
   **これは「枠を制限する」のではなく「木の junk の深さを制限する」ので、
   案(iv)（枠 = nil）が壊した鎖（`twoIt N T n`）は壊さない**
   — ただし鎖の要素は入れ子なので、この条件も鎖に効く。そこを先に確認すること。
3. `jk1` で junk が 1 段のとき `two N M` の N がどう描かれるかを確認してから条件を書く。

**再現コマンド**: コーパス生成は notes のこの節のスクリプトと同じ手順
（`bms "<m>[n]"` を n=1,2,3 で 6 段、frontier 300 打ち切り）。
