...
for pairs,dest in ((br,NEG),(new,POS)):
    for A,n,m in pairs:
        ...
        bA=rows3w2.b2d3wc(list(A), sites=set())[0]; bE=rows3w2.b2d3wc(list(E), sites=set())[0]
        tgt=tuple(expand(bA,m))
        for off,_,_ in rows3w2.b2d3wc(list(E))[1]:
            if (rows3w2.b2d3wc(list(E), sites={off})[0]!=tgt)==(dest is NEG): dest.add((E,off))
        for off,_,_ in rows3w2.b2d3wc(list(A))[1]:
            if (bE!=tuple(expand(rows3w2.b2d3wc(list(A), sites={off})[0],m)))==(dest is NEG): dest.add((A,off))
print('壊れた %d 組 -> 負例 %d / 新しく一致 %d 組 -> 正例 %d'%(len(br),len(NEG),len(new),len(POS)))
pickle.dump(sorted(NEG), open('/tmp/h1work/h17wcneg_%s.pkl'%TAG,'wb'))
pickle.dump(sorted(POS), open('/tmp/h1work/h17wcpos_%s.pkl'%TAG,'wb'))
