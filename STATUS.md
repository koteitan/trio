...
## ★★★★★★ **頂点は 1 文。`GraftAll` は降りた**

`Final.lean:875`（team-lead が `leanman build` **809 jobs / exit 0** で検算）:

    **`TRIO_terminates_of_mTowerClosedRow2 (h : L105.MTowerClosedRow2) : WellFounded stepRel`**
    **`sorry` … 自作ファイル全体で 0**

```lean
def MTowerClosedRow2 : Prop :=                                -- L105Cap:5794
...
| | 量化 | 前提 |
|---|---|---|
| `GraftAll` ＝ `CoreCap` ＝ `CoreSingleton`（**半年の残核**） | 7 | 5 |
| **`MTowerClosedRow2`** | 6 | **3** |

**`graft` も `y ∈ W u` の全称も `GX` も出てこない。**

## ★★ 証明の形は **素の 2 重帰納 1 本**（§94 `mTowerClosed_of_snocStep`、`:7027`）

    **`P j := mTower Q d e n ++ (block n).take j ∈ W u`**
    `P 0` ＝ `n` の帰納法の仮定 ／ `P |Q|` ＝ 目標（`mTower_succ`）
    段 ＝ **1 列の snoc。列は完全に決まっている**（`block_getD`、`:7051`）:
      **`(entry Q 0 j + d*n,  entry Q 1 j + (錐なら e*n),  entry Q 2 j)`**

**`catBlock` も `A2'` もリフト装備も消えた。**
**段で無料なのは 2 つだけ: `snoc_orphan_W`（孤児）／`snoc_flat_root`（`srow=0` かつ親＝根）。**

## ⟹ 残っている枝（4 つ）

    **F2a**（`srow=2` ∧ 錐の外）… **F1 と同じ骨。道具は揃っている** ← L3 が実装中
    **`j = 0`**（ブロック境界）… §95 で「無料の 2 枝に入らない」と**証明ずみ**
                              ⟹ **`oper_eq_gexp_gen` を `j0=(n-1)|Q|` で読む**（R132）
    **`j >= 1`** … **誰も見ていない**
...
## ★★★ 今日 (D) から始まった連鎖（全部緑）
    ...
    §85 `le1_mTower_block` ／ §86 行 0 の壁 ／ §87 行 1 の壁（錐の中）
    §93 **行 2 の壁は `¬F2b` だけ**（`mTower` は行 2 を動かさないので `+k*d2` が無い）
    §98 `gexp_blockRoot_anc`（**§84 の `q=0` ＋ `le1_zero_iff`**）
...
| 枝 | R2 の実測 | いま |
|---|---|---|
| (D) | 24,510（R2）＋ 152,208（H12） | **定理**（§84） |
| **F1** | **376,164・破れ 0** | **定理**（§100） |
| F2a | 933,768・破れ 0 | 同じ骨。実装中 |
| **F2b** | **1,812（0.50%）** | **残る 1 点。しかも「命題が偽」** |

**⟹ 測定が「証明すべき文」を特定し、証明がその測定を不要にする —— 今日 4 回起きた。**

## ★★ 道具: **`lean/LEMMA-INDEX.tsv`**（2,932 件、`python3 tools/dbms/mkindex.py`）

**L3 が 9 回、既存の補題を書き直したので team-lead が作った。**
**すぐ元が取れた —— §98 は「索引で `le1_zero_iff` の型を見て、
§97 の『足りない 1 文』が既にあると気づいた」ことで生まれた。**
...
⚠ **索引は型しか持たない。docstring は入っていない**（`nextR_src_ge` のように
docstring が本質を語るものは、名前を見つけたあと必ずファイルを開くこと）。

## ⚠ team-lead の今日の外し（6 件。全部エージェントが捕まえた）

    1 「`srow=2, z=1` は起きない」… 死んだコード（自分で気づいた）
    2 「塔を 1 段進めれば `v` が変わる」… `oper` は第 1 列を落とさない
    **3** 「`srow=0` は `snoc_flat_root` で無料」… **主語違い**
    4 「`Aop` があるほうが強い前提」… **`Aop` は弱い前提**
    **5** 「`hblk` 破れ ⟹ `lift_oper_of_noParent`」… **主語違い**
    **6** 「行 2 の上限を 3 段振れ」（`le1` は行 0/1 しか見ない）… **課題設計の誤り**

**当たったもの: `srow=1` で `e=0` ⟹ 段が動かない／`j` で帰納する案（§94 になった）／
`oper_eq_gexp_gen` を `j0=(n-1)|Q|` で読む（R132 で検証中）。**

> **⟹ 定義を開いたものは当たり、開かずに構造から推測したものは 6 件とも外れた。**

**そして中継で但し書きを落としたのが 4 回**（`LiftFlatMapLocal` の箱／`snoc_flat_root` の主語／
R2 の教訓 14／§79 の 3 形のうち F2b）。**⟹ 教訓 52: 元の報告の ⚠ 行はそのまま貼る。要約しない。**
