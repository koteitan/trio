...
### 残る核は **2 本**（§89 の訂正）
    ...
    **`LiftTie`**       タイのある根での (WL)。⟸ `Row1DownLocal` ＋ `Row1DownRoot0`
                       （どちらも **`Row1Mono` より弱い**ことが証明ずみ、§90）
    **`GraftFromExp`**  `Aop` の節 2 から節 3 を作ること ＝ **連結（`WCat` / `WSnoc`）そのもの**
                       （`TowerExp` の正体、§93）
...
さらに **`D_1` .. `D_12` も覆う**。**対照（strict）は 4 行・`D_1` のみ**（§91）。
