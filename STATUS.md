...
## ★★★ 残核の中心は **「`v = 0`・`z = 0`・段 0・タイあり」**（§207、H12 の最終結果）
    ...
    **`v = 0` は `z = 0` を強制する**（箱 2 つ × 長さ 3 つ × **94334 件で例外 0**）
    ⟹ `lev Q 0 = 2·0 + 0 = 0` ⟹ **段は `a = 0`**

⚠ **機構は未証明**（H12 は `nextrel2` の `le1` 条件を追い切っていない）。**L3 に確かめさせている。**
...
## ★ 次の一手（課題 L135）
    ...
    **3** その特徴づけで `v=0, z=0` の塔（＝ (δ) の 7 割）が扱えるか
    ...
    R113 … **`mTower = liftTower`（マスクがブロック局所か）**。真なら (2) をブロックごとに扱える
