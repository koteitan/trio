...
## ★★★★★★ **核は 2 本**（`TowerExpBig` の完全な分解、§206）
    ...
    **(1) `LiftTieCore`**  … ブロック 1 個を持ち上げる（**経路 C と D で同じ命題**、`block_mem_of_liftTieCore`）
    **(2) `ShTower2Self`** … 等差シフト塔が `Wself` に閉じる（経路 D 固有、**段はどこにも現れない**）
...
**単調増加で頭打ちなし。** R2 の別測定（`zle1` 後）では **`|R|=6` で 97.2%**、
無料になる割合は **48.3% → 2.80%**。
...
## ★ 次に測ること: **(δ) を `v = 0` / `v >= 1` で割る**

`liftTie_case_tieFree`（`L53Subst.lean:2615`）は **`1 <= v` を要求**するので
**`v = 0` では使えない** ⟹ (δ) は `v=0` と `v>=1` で別の道具が要る。
`v = 0` なら根は `(0,0,z)` で `lev = z <= 1` ⟹ **段がほぼゼロ**。
`mem_iff_lev_le`（`Wchar.lean:106`、緑）が効く形になっていないか。
