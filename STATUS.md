...
## ⚠ `WSnoc` 路線は循環している —— 理由まで確定した（§131）

    塔 shTower Q e n の 1 段追加  =  shTower Q e n ++ shiftr01 (n*e) 0 Q
    W_add の側条件 rsum は  entry Q 0 0 + n*e <= entry Q 0 0  を要求 ⟹ **n*e <= 0**
    ⟹ n>=1, e>=1 では**必ず破れる。塔は W_add で組めない**（算術。測定不要）
...
ただし L3 の副産物「**`CoreCap` の段リフト `t` は自由変数**。`t=0`/`t>=1` の場合分け不要」は残る。

## 逃げ道: `CtxOK` は `WSnoc` より真に弱い前提を配っている（課題 L106）

    CtxOK M v z  =  ∀ k < |M|, ∀ a t, 2(v+t)+z <= a → Lift1 ((0,v,z) :: M.take k) t ∈ W a
                    ^^^^^^^^^ **接頭辞 k を全称している。使ったのは k=|M|-1 の 1 本だけ**

⟹ `CoreCap` の snoc は「任意の `C ∈ W u` に 1 列」(=`WSnoc`) **ではない**。
「接頭辞の鎖を丸ごと持つ土台に 1 列」。**`WSnocCtx` が `WCat` を出さなければ循環は切れる。**

## 分岐を決める 1 個の測定（課題 R89）

`argOK M`（`Wset.lean:1314`）= `M` の全列が行 0 >= 1。根 `(0,v,z)` は行 0 = 0。
⟹ cap 列は**必ず親を持つ**。親が根（`j0=0`, 塔の場合）になるのは
**`M` の行 0 が末尾で非増加**のときだけ（＋行 1・行 2 の祖先条件）。

    j0 >= 1 が常に  ⟹ コピー塊は `M` の接尾辞（根を含まない）
                      ⟹ **`CtxOK` の接頭辞が直接土台になる。軽い路線**
    j0 = 0 が起きる  ⟹ コピー塊が根を含む ⟹ **`W_add`（連結）では組めない**
                      ⟹ ただし **`Aop` は選言。節 3（graft）が残る**。路線は死なない
                      ⟹ 次に測るのは **`domT S m` を満たす `m < a` の有無**（節 3 が使えるか）
                      ⟹ および **`d0` の分布**（`d0 = 0` なら根込みでも rsum は 0<=0 で通る）

⚠ **私は一度「`j0=0` ⟹ 路線を捨てる根拠」と書いたが誤り**（2026-08-30 に訂正）。
`Aop` は選言なので、連結が死んでも graft が残る。**「連結が死ぬ」と「路線が死ぬ」は別**。
