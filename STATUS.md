...
> **塔（またはその接頭辞）に**決まった 1 列**を足す**

**5 方向すべてがここに集約する**（L3 §110-§125。**`CORES.md` 冒頭に詳細**）:

    **§110** `TowerSnocRoot` ＝ `MTowerClosedS`（**塔を消しても同じ文**）
    **§112** `MTowerClosedS0` ＝ `ShiftTowerClosedS`（**`z=0` では同一**）
    **§121** (T1) の**親＝根**は **`(shTower Q e n ++ [r])⟦m⟧ = shTower Q e (n*m)`**（**塔の塔**）
    **§122** (T1) の**親＝ブロッカー**は **接頭辞との連結**
    **§124/§125** `v=0` は**接頭辞つきコピー塔**、その節 2 も**接頭辞 ＋ 1 列**に戻る

⚠ **§78.2 の「素直な分解は `WSnoc` に落ちる」は、遠回りしたあとで見ると正しかった。**
**ただし §90 で底の列は**決まっている**ので `WSnoc` そのものではない。**

## ⟹ 残核は 2 本

    **(a) 「塔（接頭辞）に決まった 1 列を足す」** … **5 方向の合流点。取っ手が無い**
        別表記: `(TOW)` ＝ `ShiftTowerClosedS` ＝ `MTowerClosedS` ＝ `TowerSnocRoot`
    **(c) F2b（`z = 1`）** … §93 で「復活しない路線は閉じた（命題が偽）」
        ★ **R2 が §R143 で新しい取っ手を出した**（下）

## ★★★★★ **F2b に新しい道が開いた**（R2 §R143、いまの最優先）

    **再帰は必ず有限。深さ ＝ `|Q| − 2`**
      最大例の族 **`Q = (0,0,1)(1,1,0)(2,1,1)^k`** で **`|Q|`=3..8 → 深さ 1..6 に厳密一致**
      （`d,e` を振っても不変、打ち切り 0 件。**R2 が最初の箱の「深さ 2」を自分で壊した**）
    **⟹ `|Q|` に関する整礎帰納が組める。無限後退は起きない**
    **展開の末尾列の 57.8% は `srow = 1` ⟹ F1（`gexp_orphan_row1`、§102、緑）の領域へ**
    **また F2b になるのは 28.4% だけ**

⚠ **R2 の但し書き: 深さは**構造の深さ**であって所属ではない。**
**`W` の節 2 は「すべての `m >= 1`」を要求する。「深さが有限」は「`W` に入る」ではない。**

## ✅ 今日落ちたもの

    **`Final.TRIO_terminates_of_mTowerClosedRow2`**（緑、**6 量化 / 3 前提**）
      ⟸ `GraftAll`（7 量化 / 5 前提、**半年の残核**）が本線から降りた
    **(D)/(E)** `gexp_cone_mir_zero` / `le1_mTower_block`（§84-85）
    **B の 2 枝**: **F1**（§100/§102、任意の `(k,q)`）／**F2a**（§108-109）
      ＝ **R2 の実測 130 万件が定理に**
    **`z=0` の道具一式**（§115-§119、**`hd1pos` なし**）… §115 錐の外の壁／§116 同ブロック行 1／
      §117 鎖の閉じ込め／§118 橋渡し／**§119 `shTower_orphan_row1`（`e=0` 版の F1）**
    **(T1) の完全な地図**（§120-§125）… どの枝も「無料」か「接頭辞 ＋ 1 列」
      `d = 0` は全行で孤児（§121、R2 の導出）／`v=0` の親＝根は `snoc_flat_root`（§123）

## ★★★ 今日の手筋（5 回。3 回は定理、2 回は「作れない」の確定）

> **既存の証明を開いて、要らない仮定が**実際にどこで使われているか**を数え、
> そこだけ差し替える。**

    **§84**  `hj0` … 3 か所（2 つは代替、1 つは空虚）⟹ **(D) が定理に**
    **§115** `hd1pos` … **1 か所**（`gexp_outer_anc_row1`）⟹ `hlp` で置換
    **§117** `houtc` の `gexp_cone_mir_zero` ⟹ `Q` の中の推移律で置換
    **§121** `hsr : srow = 0` ⟹ **「`srow=1` 版は書けない」**（`(TOW)` そのもの）
    **§125** `W_flatMap_copies` の `A = []` ⟹ **「使っていない。使っているのは `W_add`」**
             ⟹ **接頭辞つき版は既存の一般化では作れない**

## ★★ 道具: **`lean/LEMMA-INDEX.tsv`**（2,971 件、`python3 tools/dbms/mkindex.py`）

**L3 が 9 回、既存の補題を書き直したので team-lead が作った。効いた回数:**

    §98  「足りない 1 文」が `le1_zero_iff` として既にあった
    §106 **設計を 3 段から 1 段に**（`nextrel2_append_right` が全部運ぶ）
    §114 **7 本の署名をまとめて写して、`e=0` の生死の地図を作った**
    `Gamma.le0_of_le1` が 1 回の grep で出た（10 回目の再発明が起きなかった）
    **名前を思いついた時点で grep** ⟹ **衝突を書く前に 2 回避けた**

⚠ **限界が 2 つ:**

    **索引は型しか持たない。docstring は入っていない**
      （`nextR_src_ge` / `oper_snoc_flat_root` / `W_flatMap_copies` は
       **docstring が本質を語っていた**。名前を見つけたら必ずファイルを開く）
    **⚠ 索引に無いことは「未着手」と「書けない」を区別しない**（教訓 63、§121）

## ⚠ team-lead の今日の外し（10 件。全部エージェントが捕まえた）
    ...
    **9** 「`(TOW)` へ降りると `WCat` の壁」… **`CORES.md` の列の誤読**（自分で見つけた）
    **10** 「`d0 = d`」… §101 の訂正を計算に反映していなかった

**当たったもの: `srow=1` で `e=0` ⟹ 段が動かない ／ `j` で帰納する案（§94 になった）／
`v=0` では `srow r = 0`（§120 の射程を訂正）／ `hQr` が接頭辞を許さない（§125）。**

> **⟹ 定義を開いたものは当たり、開かずに構造から推測したものは 10 件とも外れた。**

**中継で但し書きを落としたのが 4 回**（`LiftFlatMapLocal` の箱／`snoc_flat_root` の主語／
R2 の教訓 14／§79 の 3 形のうち F2b）。**⚠ 教訓 52: 元の ⚠ 行はそのまま貼る。要約しない。**
