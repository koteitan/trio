...
## ★★★ 残核の中心は **「`v = 0`・`z = 0`・段 0・タイあり」**（§207、H12 の最終結果）
    ...
    (δ) の **7 割が `v = 0`**（広い箱でも **70.5%**。**生き残る**）
    ⛔ **ただし「残核の 7 割は段 0」は誤り。正しくは 47%**（§219 で H12 が撤回）

| `|R|` | (δ) 総数 | **段 `a=0`** | `a=1` | `a=2` | `a>=3` | **段 0 の割合** |
|--:|--:|--:|--:|--:|--:|--:|
| 2 / 3 / 4 | 54 / 2100 / 51258 | 24 / 971 / 24189 | 12 / 481 / 11937 | 12 / 432 / 10088 | 6 / 216 / 5044 | **44.4% / 46.2% / 47.2%** |

**team-lead の「段 0 では `Aop` の節 3 が消える」は正しいが、効く範囲は 7 割ではなく 47%。**
...
| **行2<=2（`zle1` なし）** | 38,841 | **19,123** |

**`zle1` は核に課せない**（L131）⟹ **`v = 0, z = 1` は本物の場合。
「段 `a = 0`」は `z=0` のときだけで、`z=1` なら `a = 1`。場合分けは 2 つ。**
...
### ★★ R2 が機構まで出した（§215、commit `e7b709c`。311,688 列で数えた）

    `|R|<=6`, `n=2..7`  塔 **276,876** / ブロック **1,245,942** / 列 **6,846,876** —— **すべて 100%**
    (q2) ブロッカーがある `Q` だけに絞っても**反例なし**
    (q3) 陽性対照（マスクを 1 列ずらす）は**鳴る**（塔単位 50.00% 不一致）

    `¬le1 T 0 j ⟺ ∃ y ∈ anc0(T,j), y≠0, entry T 1 y <= v`（`le1_zero_iff`、緑）の `y` の位置:
      錐の中                                    212,598（68.21%）
      錐の外 / **同ブロック内にブロッカーあり**   99,090（31.79%）
      錐の外 / **外のブロックにしか無い**         **0 件**

> **⟹ 証明の目標は「錐の外の列は必ず自分のブロック内にブロッカーを持つ」。**
> 行 0 の祖先鎖は前のブロックへ**抜ける**（ブロック `k` の根は行 0 = `k*d` > `(k-1)*d`）が、
> **抜けた先が唯一のブロッカーになることは 1 度も無い。**

⚠ R2 の但し書き:「**実測で真 ≠ Lean で証明できる。`le1_take` は接頭辞局所性しか与えない。
