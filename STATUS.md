...
## ★★★★★★ **未証明は 2 本だけ。うち (2) は Lean の 1 文になった**

    **(1) `LiftTieCore`** … ブロック 1 個を持ち上げる（**経路 C と D で同じ命題**）
        `v = 0` は (δ) の **70.5%**。段で 2 本に分かれる:
          **`LiftTieCoreZero`**（緑）`((0,0,0)::R) ∈ W 0 ⟹ Lift1 … 1 ∈ W 2`
            前提の段 **0**（**節 3 が無い**）／結論の段 2（節 3 が使える）
          **`LiftTieCoreOne`**（緑） `((0,0,1)::R) ∈ W 1 ⟹ Lift1 … 1 ∈ W 3`
            前提の段 **1**（**節 3 が `m=0` で使える**）／結論の段 3
        どちらも結論は **`Wself` 所属と同値**（`lev` がちょうど、**余裕ゼロ**）

    **(2)** … ⛔⛔ **`LiftFlatMapLocal` は文のままでは偽**（§223。H12 が反例）
        **前提が 1 つも無い `∀ Q d e n`**。文のまま全数 111,132 組で**反例あり**
          `Q=(0,0,0)(1,0,0)`, `d=2`, `e=1`, `n=2` … 左辺 `…(2,1,0)…` / 右辺 `…(2,2,0)…`
        **`d>=1` を足しても、根が狭義最浅を足しても、両方足しても反例は残る。**
        **消えるのは「ブロッカー無し」を足したときだけ ＝ §210 で既に無料の場合**
        ⟹ **残核 (δ)（ブロッカーあり 65〜83%）ではまさに反例が出る側**
        ⚠ ただし実際の場面では `d = entry R 0 (|R|-1)`、`e = entry R 1 (|R|-1) - v`、
          `Q = (0,v,z) :: R.dropLast` で **`R` の前提（`domT` など）が効く**。
          上の反例を `R` に戻すと `R = [(1,0,0),(2,1,c)]` で **`domT R m` が破れる**
        ⟹ **「文が場面より広い」。修正は 1 手だけ**（§229、H12 が全数で決着）:

    ★★ **`LiftFlatMapLocal` を `TowerExpBigRow2` と同じ引数（`R`, `v`, `z` ＋ 同じ前提）で
       述べ直し、`Q`, `d`, `e` はその中で定義する。**
       下流（`operTower_eq_mTower` / `operTower_succ`）は `Q d e` のままでよい
       ⟹ **書き直すのは的の文だけ。**

    ⛔ **`Q, d, e` に前提を足すやり方では直らない**（H12 が全数で確認）:
       `Q[0][0]=0` ＋ `argOK` ＋ `d>=1` ＋ 根が狭義最浅 を全部足しても **反例が残る**
       生き残る反例 `Q=(0,0,0)(1,0,0)`, `d=2`, `e=1` を `R` に戻すと `R = (1,0,0)(2,1,1)` で
       **`domT R m` が成り立たない**
       ⟹ **`domT` は `Q,d,e` の言葉では書けない**（`R` の末尾列の性質だから）

    ✅ **`R` から導いた `(Q,d,e)` では反例ゼロ**（**332,752 検査、行 2 を 2 まで許した箱**）
       自由な反例 3288 件は全部どこかの前提で落ちる（**到達可能なもの 0**）

    （旧）**`def LiftFlatMapLocal`**（`L105Cap.lean:4634`）
          `Lift1 (mTower Q d e n) e = flatMap k => Lift1 (Lift1 (shiftr01 (d*k) 0 Q) (e*k)) e`
          ＝ **錐がブロック局所**
        `:4661` `operTower_eq_mTower`（**緑**）／`:4676` **`operTower_succ`（緑）**
        ⟹ **落ちれば `oper_shTower`（§48）/ `oper_shTower2`（§56）の技法がそのまま使え、
           (2) の帰納が `srow = 1` 側とまったく同じ形になる**

（team-lead 検算: `leanman build` **809 jobs / exit 0**、上の 4 行を自分で開いて確認。）

## ★ (2) の穴は **(D) 1 本**に切り分けられている（R2 §R116）

    (A) `le1 Bk 0 i ↔ le1 Q 0 i`          484,848/484,848 ← **`le1_block`（緑）**
                                          ＝ `le1_Lift1`（`:1213`）＋ `le1_shiftr01`（`:3470`）の合成
    (B) `le1 T 0 (k|Q|+i) ↔ le1 Bk 0 i`
        `k=0`  138,528/138,528 ← `le1_take`（`:908`、緑）で既済
        **`k>=1` 346,320/346,320 ← ★ 唯一の穴**
    (B) の骨: (C) 鎖のブロック内部分 ＝ 単体の鎖／**(D) 鎖のブロック外の祖先は 1 本もブロッカーでない**

⚠ **(B) が非自明なのはしきい値が違うから**（塔の根の行 1 は `v`、ブロック `k` の根は `v + k*e`）。
⚠ **(D) は空虚ではない**（R2 が先に確認）: **鎖は必ず外へ出て（100%）、
71.1% で根以外の外部祖先も含む。それでも 1 本もブロッカーにならない。**

### ★ L3 の (D) の機構の予想（R2 が測定中）

> **ブロック `k` の根の行 1 は `v + k*e > v`**（`e >= 1` は `L53.tower2_vw` が強制）
> ⟹ **外のブロックへ渡る鎖は必ず「ブロック `k` の根」を経由する**
> ⟹ **根が錐に入っていれば十分**

**正しければ `le1_zero_iff` ＋ `le1_block` ＋「根の行 1 > `v`」で閉じる。**
