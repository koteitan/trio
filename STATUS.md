...
## ⚠ 効く分割は **`srow`**（バッドルートの行）。`j0` ではない（§133）

    Cnf.lean:1060   d0 := if 0 < i1 then entry M 0 j1 - entry M 0 j0 else 0   （i1 = srow）
    ⟹ **`srow = 0` ⟺ `d0 = 0`** ⟹ 写しが行 0 でずれない ⟹ **そもそも塔が無い**

    srow = 0, j0 = 0   … **無料**。`W_flatMap_copies`（`Wset.lean:2552`、証明ずみ、同一の写し）
                         側条件は根の行 0 = 0 から自明
    srow = 0, j0 >= 1  … 接頭辞が根（行0=0）を含み `C.drop j0` は argOK で行0>=1
                         ⟹ rsum が **`1 <= 0`** を要求して必ず破れる（`not_rsum_capBase_split`、緑）
                         ⟹ ただし死ぬのは `W_add` 経路だけ。**節 3（graft）が残る**
    srow >= 1          … **唯一の本丸**。ここで初めて `d0 >= 1` の塔になる

## ★ 今日いちばん鋭い仮説（課題 L107 / R89 の (s2)(s3)、検証中）

`srow` = 「どの行で祖先が見つかるか」。cap 列の行 1 は `b`、行 2 は `c`。

> **`CoreCap` の `∀ b c` は見かけの全称ではなく、`srow >= 1` の枝を生成している当のもの。**

本当なら §130.5 の「`c <= 1` への制限」の試みは、まさにこの枝を殺そうとしていたことになる。
