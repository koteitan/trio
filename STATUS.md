...
`Final.lean:875`（`leanman build` **809 jobs / exit 0**、`sorry` 0）:
**`TRIO_terminates_of_mTowerClosedRow2 (h : L105.MTowerClosedRow2) : WellFounded stepRel`**

**核 ＝ §139 `mTowerClosed_of_snocStepPar`（緑）「足す列が**親を持つ** `j` についてだけ、塔に 1 列足す」**

### 3 分岐（全部定理。実測の割合は R2 §R154、分母 146 万）

    **(i) 孤児** … **無料**（`snoc_orphan_W`）。§139 で**場合分けにすれば前提が要らない**
                 実測 56.3〜68.4%
    **(ii) 親が**同じブロック**（`p = 0` を含む）** … **測度 `j − p` が厳密に減る**（§138 ＋ §140）
                 実測 16.7〜28.9%。**窓 `j − p < |Q|` は実測 100%（分母 341,184）**
    **(iii) 復活**（親が**手前のブロック**）… **測度なし ← 核**
                 実測 **11.9〜19.3%**

### ★ 核の場所（全部定理）

    **§141** 復活は「足す列が**自分のブロックの中で孤児**」のときだけ（`nextR_src_ge` ＋ §101）
    **§142** **錐の中の列は復活しない**（根の行 1 が狭義に小さいので根が候補親）
    **§79**  `srow = 0` も必ずブロック内に親を持つ
    **§102/§137** 錐の外で行 1 が根以下なら塔でも孤児 ⟹ **無料**

> **⟹ 復活しうるのは:**
> **「`Q` の**錐の外**で、`entry Q 1 0 < entry Q 1 j <= entry Q 1 0 + e*n`」の帯**
> **と `srow = 2`（F2b）だけ。**

### ★★ 実測が示す形（R2 §R154）

    **復活は `j = 0`（ブロックの根を足す段）に 88.9〜90.6% 集中**。末尾は **0.14〜0.21%**
      **理由は §104「1 列は自分の中に親を持てない」** ⟹ 親があれば必ず手前のブロック
    **親のブロック戻りは常に 1**（100%、分母 **300,764**）⟹ **窓は高々 2 ブロックぶん**
      ★★★ **`n = 2..7` のすべてで 100%、`n` 別の分布も完全に同一**（R2 §R156）
      ⟹ **`n=7` なら戻り 1〜7 がありえたのに 1 しか出ない。箱の産物ではない。**
      ⟹ **二重帰納（外＝ブロック番号、内＝窓）の条件が満たされている**
    ⛔ **`k*`（L3 §144-145 の下界）は 100% 正しいが tight なのは 0.10〜0.20%**
      ⟹ team-lead の「`k* = n−1` ⟹ 帯が 1 ブロックに狭まる」は**外れ**（13 回目、§496）

## ★★★★★★ **なぜここが核なのか**（L3 §134-135。`CORES.md` に 8 方向で記録）
...
**`Aop` の 3 節を全部開いて判定した:**
    ...
    **節 1**（`|M| <= 1`）… 塔からは届かない
    **節 2**（`∀ m >= 1`）… **`m >= 2` で対象が伸びる**（§134.2）
    **節 3**（graft）… **`domT` が「末尾列が孤児」を含む** ⟹ `snoc_orphan_W` より強い要求（§135）

**⟹ どの節から入っても着地先は `dropLast ∈ W u` ＝ (a)。**

## ★★★ いま出している手: **`Q` の側で `A2'` を回す**（§499、L3 の判定待ち）
...
⚠ **穴**: §77.1「末尾列が段内で孤児 ⟹ 悪根がブロックをまたぐ」の枝は `M⟦n⟧` の形でないので IH が効かない。
⟹ **「閉じる」主張ではなく「`W` の導出木という整礎構造がまだ未使用」という指摘。**
