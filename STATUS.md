...
## ★★★★★★ **両路線が 1 点で出会った**（§197）
    ...
    経路 C（`CoreCap` ⟺ `Wset.GraftAll`）… 残核は **`LiftTieCore`**
    経路 D（`TowerOK` ⟸ `TowerExpBigRow2`）
      **(1) ブロック 1 個を `W` に入れる … `LiftTieCore`（同じ命題。`block_mem_of_liftTieCore`、緑）**
      (2) ブロックを繋ぐ … `ShiftTowerClosedS` ／ `shTower2` 版（**`WCat` ではない**）

**(1) は完全に同じ命題。(2) だけが経路 D 固有**で、`L47W.shiftTowerClosed_iff_wself`
（`:155`、緑・**両向き**）で **`Wself` の閉包 1 点**に落ちる。

## ★ 塔の主語は完全に書けている（H12、291561 件・100%・機構も定義から）

> **`S⟦n⟧` = `B_0 ++ … ++ B_{n-1}`、`B_0 = (0,v,z) :: R.dropLast`（＝ 仮定の主語そのもの）、
> **`B_k = Lift1 (shiftr01 (k·δ0) 0 B_0) (k·δ1)`**
> **マスクは全ブロックで同一**（80137 件・例外 0）で **`B_0` に内在**（`Wset.le1_take`、緑）

    `Lift1_shiftr01`（L3、緑） `Lift1` と行 0 の一様シフトは**可換** ⟹ マスクが `k` に依らない理由
    `Lift1_iterate` / `Wset.le1_Lift1`（`:1213`、**錐は `Lift1` で動かない**）/ `Lift1_Lift1`（`:1230`）
      ⟹ **`Lift1^k X e = Lift1 X (k·e)`。H12 と R2 の閉じた形は同じ式の別表記だった**

## ⛔ `WCat` は使えない（H12 §214、L3 §43 が独立に）

    `B_k` の根は `(k·δ0, v + k·δ1, z)` ⟹ **`lev = 2(v+k·δ1)+z > 2v+z = a`**
    ⟹ **`k >= 1` のブロックは 1 つも `W a` に入れない**（21357 件で 100%）
    ⟹ `WCat` は**両辺が同じ段**を要求し `W_mono` は上げる向きだけ ⟹ **使えない**
    ⟹ **要るのは `ShiftTowerClosedS` の形**（「`Q` だけが `W u` にいれば塔全体が `W u`」——
       **途中のブロックが `W u` にいることを要求しない**）の**持ち上げ版**

⚠ `ShiftTowerClosedS → WCat` の逆向きは `grep` で**見つからない** ⟹
**`ShiftTowerClosedS` は `WCat` より弱い可能性が残る。弱いほうに賭ける**（L3 / H12 の一致）。

## ★ 残核の大きさ —— **4 分割で測り直し中**（§198）

ブロッカーがある場合は**さらに 3 分割される**（L3 の `block_mem_of_liftTieCore`）:

    (α) **タイ無し**（ブロッカーが全部 `< v`）… `L53.liftStage_of_noTie`（**仮定ゼロ**）で無料
    (β) タイあり ∧ `TieFree`                  … `L53.liftTie_case_tieFree` で無料
    (γ) 行 2 ≡ 0                              … `liftStage_of_zeroRow2`（**仮定ゼロ**）で無料
    **(δ) タイ ∧ `¬TieFree` ∧ 行 2 に非零      ＝ `LiftTieCore` ← 本当の残核**

⚠ **「ブロッカーあり」の率（H12 83.2% / R2 67.1% @ `|R|=4`）は (α)(β)(γ) を含む。
残核ではない。** H12 の (m1) では**ブロッカーの行 1 は 76% が `v` ちょうど、24% が `v` 未満**
⟹ **`v` 未満の 24% は (α) で仮定ゼロで落ちる。**

## ★ `zle1` を `Wset.lean` に足すことを承認（§195）

`CLAUDE.md` が「一旦 z < 2 の断片を対象とする」と明記しており、R2 が消費側を全部調査して
`zle1` を持っていることを確認済み（`zle1_oper` / `zle1_ST_TS` は緑）。L2 が抜けたので
`Wset.lean` を触るのは L3 だけ。壊れたら `leanman build` が教える。
⟹ **効果: `srow = 2` の枝が `z = 0` に限られる**（`tower2_not_z1_of_zle1`、緑）。

## 死んだ道（全部確認ずみ）

    連結（`W_add` / `rsum`）… どの枠組みでも死ぬ。**塔は深いほうを足す**ので構造的に不可
    **`|R|` の帰納**   … 節 3 の `domT` と仮定の `hasParent` が排他
    **導出の帰納**     … `Wstar` は Π 命題で導出木が無い ⟹ `Wstar_closed` と循環
    分解して組み直す   … 接頭辞でも行 0 でも `WCat` に落ちる
    **`WCat`**         … 段が上がるので使えない（上）

## 計器の教訓（`CORES.md` 冒頭。H12 の整理 ＋ R2 の 1 本）

    21 「100% の不変量」は報告前に必ず 1 段長い母集団で壊す
    22 陰性対照は壊し方を 2 種類以上、「どの葉で鳴ったか」を記録する
    23 「反例ゼロ」の前に前提の充足率（分母）を数える
    24 **候補補題は測る前に `grep` する**
    25 **「この補題で無料」と書く前に、結論の段と向きを紙に書き下す**
    （R2）**緑の補題を実測で検証するときは、前提を `file:line` から写してから測る**

**21・22・23 は「測る前に、測れているかを測れ」の 3 面（母集団 / 対照 / 分母）。
24 はその手前の「そもそも測る必要があるか」。25 と R2 の 1 本は「引いた補題を最後まで読め」。**

## 検算（team-lead 自身）

    `lean/` に **`sorry` は 1 つも無い** ／ `leanman build` **809 jobs / exit 0**
    `Final.lean:353` `TRIO_terminates_of_towerExpBigRow2`（**仮定 1 本**）
    ⚠ `SESSION-2026-08-28.md` は **2 ファイルに分裂していたので統合ずみ**（commit `95a625a`）
