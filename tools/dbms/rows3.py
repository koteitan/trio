...
# `base_sd` を使う条項。
#
# **課題 H19 で既定 off に戻した。** 順序保存（`ST_TS` 展開閉包 v<=5 len<=10、
# 416607 個）の破れが **7 -> 41** に増えるからである。得たものは
# ImgCofinalT の破れ 18 -> 17 の 1 個だけ。**1 個のために 34 件の逆転**は割に合わない。
# 原因は門 `tie_sd` の第 2 条件 `nx1_r1_lt`（**次の柱**の 行 1。無いときを真と読む）で、
# これが「行列の末尾かどうか」で綴りを変えるので接頭辞単調が壊れる。
# `RS_TIESD=1` で戻せる。
V18 = {
    'tiesd': os.environ.get('RS_TIESD', '') == '1',
}


# ---------------------------------------------------------------- v20 の旗
# 課題 H21: `top_level` に「親が `term_top`」を足す。これで **`OrderT3` が真**に
# なる（`ST_TS` 展開閉包 416607 個で (→) 0 / (←) 0）。`RS_NOTLTERM=1` で切る。
V20 = {
    'tlterm': os.environ.get('RS_NOTLTERM', '') != '1',
}
...
def top_level(Mo, j):
    ...
    v20（課題 H21）: 親が **`term_top`** のときも「根の直下」に数える。
    もとは 根 `(0,*,*)` と 写しの頭 `copy_head` だけを見ていたが、
    **アンカー `(1,1,0)` は行 1 の加算項の頭**（`term_top`）なので、
    その直下も「項の先頭に戻った」と読むのが正しい。

    これを落としていたせいで `closes_top` が**次の柱の深さについて反単調**に
    なっていた（課題 H21 の群 B、順序保存の破れ 3 件）:

        接頭辞 (0,0,0)(1,1,1)(2,X,Y)(1,1,0)(2,0,0)(3,1,1)(4,1,0)  site = 第 6 列
          次の柱 (2,0,0) … 親はアンカー -> 旧 `top_level` False -> **deep**
          次の柱 (3,0,0) … 親は写しの頭 -> True                -> **shallow**
        ＝ 次の柱が**深い**ほうが浅く綴られる ＝ `seqlex` の順序が逆転する

    足すと両方 True になって反転が消える。**`ST_TS` 展開閉包
    (v<=5, len<=10, 416607 個) の破れが 3 -> 0**、他の土俵はすべて不変か改善。
    `RS_NOTLTERM=1` で v19 に戻る。
    """
    q = par0(Mo, j)
    if q < 0 or Mo[q][0] == 0 or copy_head(Mo, q):
        return True
    return V20['tlterm'] and term_top(Mo, q)
