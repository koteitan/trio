...
V17 = {
    ...
    # 課題 H27: **既定 off に戻した**（＝ `aw_flip` を両向きに使う）。
    # 片側に制限したのは順序保存のためだったが、順序が指標に降りたので、
    # `ImgCofinalT` を買うほう（20 -> 18）を取る。`RS_AWDOWN=1` で片側に戻る。
    'awdown': os.environ.get('RS_AWDOWN', '') == '1',
}
...
# `RS_TIESD=1` で戻せる。
V18 = {
    # 課題 H31（2026-08-29）: **また既定 off に戻した（v22）。**
    # H27 で一度 on に戻したが、**`Inj3`（Lean の仮定）を偽にする**ことが分かった:
    # `ST_TS v<=5 len<=11` の 1882196 個で像の衝突 **0 -> 12**。
    # 衝突する 6 対は全部同じ形で、前 8 列が同じで末尾だけ違うのに像が同じになる:
    #   ...(4,1,1)(4,1,0)**(4,1,0)**  と  ...(4,1,1)(4,1,0)**(5,2,0)**
    # `tie_sd` が深い側 `base_sd` を使うせいで 2 つの柱が潰れる。
    # `ImgCofinalT` を 1 個（19 -> 17）買うために `Inj3` を落とすのは割に合わない。
    # `RS_TIESD=1` で戻せる（門を探すときは上の 6 対を負例にすること）。
