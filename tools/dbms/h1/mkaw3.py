...
src = src.replace("V12 = {", '''NX = {'aw2': False, 'aw3': False}
...
def aw_deep2(Mo, off):
    ...
    deep = ((not blk_ge5 and nx2_z) or (pv3_r1_eq and not zblk1))
    if deep and NX['aw3']:
        # 課題 H8: ずれ 0 の一致を 3 組壊す site を浅く戻す
        a01 = _par2(Mo, off, 0)
        a02 = _par2(Mo, a01, 0) if a01 >= 0 else -1
        a02_r1_d1 = a02 >= 0 and Mo[a02][1] == p[1] - 1
        nx2_r0_d1 = g(off + 2)[0] == p[0] - 1
        if a02_r1_d1 and nx2_r0_d1:
            deep = False
    return deep
...
open('/tmp/h1work/rows3q2.py','w').write(src)
print('ok')
