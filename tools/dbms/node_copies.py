"""RDnode の正則 regime で convC(B[n]) = copies e (convC W) n かを全数検査。"""
...
def main(maxlen=5,bdmax=2,plevmax=2,nmax=4):
    blocks=gen_blocks(maxlen,bdmax); stats=collections.Counter(); bad=[]
    cnt=0
    for B in blocks:
        cnt+=1
            ...
            for plev in range(0,plevmax+1):
                ...
                for first in (False,True):
                    ...
                    for force in (False,True):
                        ...
                        a = d+2 if lad0 else dd+1
                        fA = False if lad0 else (first and p[1]==plev)
                        ladR = bool(R) and ladOf(R[0][1], a, p[1], True, fA)
                        reg = 'contr' if ladR else 'reg'
                        ...
                        for e in range(0,8):
                            if all(tuple(convC(list(expand(tuple(B),n)),d,plev,first,force))
                                ...
                                ok=e; break
                        stats[(reg, 'e=%s'%ok)]+=1
                        if reg=='reg' and ok is None and len(bad)<8:
                            bad.append((B,d,plev,first,force))
    print('stats:')
    for k,v in sorted(stats.items(),key=lambda t:str(t)): print('   ',k,v)
    for x in bad: print('   BAD', show(x[0]), x[1:])
