"""DBMS の「読み」translateD を、264 件の正解に当てて決める。

**結論（2026-08-24）: `mk_chain4` が 264/264 で一致した。**

読み方はこう。

  * 列 `(a,b)` の添字は `b`、森は行 0（BMS と同じ）
  * ただし **`(+1,+1)` で続く連**（DBMS の対角の段）は、
    添字 c_1.1, c_2.1, …, c_k.1 のノードを**引数側に入れ子**にした鎖になる
  * 連を取るのは「親の最初の子」かつ「**底の段が親と同じ**」ときだけ
    （段が下がっていればそれは影ではなく新しい加算項の頭）
  * 連のあとに続く列は、その**行 0（深さ）に対応する段**のノードの後続になる。
    連より浅い列は連全体の後続（一番外の段に付く）
  * 連の長さが 1 で、連の兄弟が尽きたあと**同じ深さで段が下がる**列が来るときは、
    その連は BMS 側で「梯子」と「本体」の二役を兼ねている（縮約で 1 本に潰れている）
    ので、`P 影の段 (頂上から読み直したもの) (残り)` と開き直す

注意: 元データ（trio-agent 作成）はシートの生の値なので、128 行・129 行の
DBMS 列の誤記が入っている。`check_sheet` の補正を反映すると 264/264 になる。
