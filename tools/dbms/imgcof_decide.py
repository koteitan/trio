...
`preimage_try` は発見的なので「逆像が無い」の証明にならない。だが
`conv3` は 1 列につき **1 柱以上**出すので **`|conv3 B| >= |B|`**、つまり

    conv3 B = T  ならば  **|B| <= |T|**

⟹ `|T| <= 11` なら `B` は `ST_TS len<=11` の母集団に**必ず入る**（`v` は効かない。
R1 の測定で `v<=6 len<=10` は `v<=5` の +1 個だけ）。**そこで全数探索すれば確定する。**

使い方: python3 tools/dbms/imgcof_decide.py
"""
import sys, time
