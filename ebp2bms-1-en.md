...
sequence system (the z<2 fragment). Generated by `tools/build_omega_alpha.py` (whose CLI
accepts the whole range $`\alpha \lt \Lambda`$), checked
against every row of the BM4-Analysis sheet (labels format-normalized by
`tools/normalize_sheet.py`); 4 rows disagree and the orbit-law audit sides
